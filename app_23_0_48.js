...
/* ===== PHASE 23.0.48 PORTFOLIO-WIDE SCENARIO REVIEW ENGINE ===== */
(function(){
  const PHASE = "23.0.48";
  try { window.RISKTOOL_RUNTIME_VERSION = PHASE; } catch(e) {}
  ...
  function quantile(values, q){
    const arr = (values || []).map(num).filter(Number.isFinite).sort((a,b)=>a-b);
  ...
  function scenarioType(s){ return s?.mode === "complex" ? "Complex" : s?.mode === "beta" ? "Beta" : "Single"; }
  function scenarioArea(s){ return s?.primaryProduct || s?.productGroup || s?.riskDomain || s?.name || "Unassigned"; }
  function componentsForScenario(s){
    if (s?.mode === "complex" && Array.isArray(s.components) && s.components.length) return s.components;
    if (Array.isArray(s?.components) && s.components.length) return s.components;
    return [s || {}];
  }
  function normalizeComponent(component, parent){
    const c = component || {};
    const p = parent || {};
    return {
      mode: p.mode || c.mode || "single",
      id: c.componentId || c.id || p.id,
      name: c.scenarioName || c.name || p.name || "Unnamed Component",
      hardCostMin: num(c.hardCostMin || p.hardCostMin),
      hardCostLikely: num(c.hardCostLikely || p.hardCostLikely),
      hardCostMax: num(c.hardCostMax || p.hardCostMax),
      softCostMin: num(c.softCostMin ?? p.softCostMin),
      softCostLikely: num(c.softCostLikely ?? p.softCostLikely),
      softCostMax: num(c.softCostMax ?? p.softCostMax),
      mitigationCost: num(c.mitigationCost || p.mitigationCost),
      control: num(c.control ?? p.control),
      randomScenarioCount: num(c.randomScenarioCount || p.randomScenarioCount || 1000),
      items: Array.isArray(c.items) ? c.items : (Array.isArray(p.items) ? p.items : []),
      insurance: Array.isArray(c.insurance) ? c.insurance : (Array.isArray(p.insurance) ? p.insurance : []),
      hardFacts: Array.isArray(c.hardFacts) ? c.hardFacts : (Array.isArray(p.hardFacts) ? p.hardFacts : []),
      mitigations: Array.isArray(c.mitigations) ? c.mitigations : (Array.isArray(p.mitigations) ? p.mitigations : []),
      inheritedResidual: num(c.residual || p.residual),
      inheritedInherent: num(c.inherent || p.inherent)
    };
  }
  function insuranceAdjustment(component, residual){
    const list = Array.isArray(component.insurance) ? component.insurance : [];
    const coverage = list.reduce((sum,i)=>sum + num(i.coverageAmount || i.coverage || i.limit || i.policyLimit), 0);
    const deductible = list.reduce((sum,i)=>sum + num(i.deductible || i.retention), 0);
    const premium = list.reduce((sum,i)=>sum + num(i.premium || i.annualPremium), 0);
    return Math.max(0, Math.min(num(residual), Math.max(0, coverage - deductible - premium)));
  }
  function runComponentMonteCarlo(component, iterations){
    const c = component;
    const requested = Math.max(100, Math.min(10000, num(iterations || c.randomScenarioCount || 1000)));
    const payload = { ...c, randomScenarioCount: requested };
    let mc = null;
    try { if (typeof runFinancialMonteCarlo === "function") mc = runFinancialMonteCarlo(payload); } catch(e) { mc = null; }
    if (!mc || !Array.isArray(mc.randomOutcomeRows)) {
      const hard = c.hardCostLikely || c.hardCostMax || c.hardCostMin || c.inheritedInherent || 0;
      const residual = Math.max(0, hard * (1 - Math.min(100, Math.max(0, c.control)) / 100));
      const rows = Array.from({length: requested}, (_,i)=>({ scenarioNumber:i+1, residualCost:Math.round(residual), totalCost:Math.round(hard) }));
      mc = { expectedLoss: hard, residualExpectedLoss: residual, confidenceHigh: residual, residualConfidenceHigh: residual, randomOutcomeRows: rows };
    }
    const residualBeforeInsurance = num(mc.residualExpectedLoss || mc.residualMedian || mc.expectedLoss || c.inheritedResidual || 0);
    const insuranceImpact = insuranceAdjustment(c, residualBeforeInsurance);
    const residualExpectedLoss = Math.max(0, residualBeforeInsurance - insuranceImpact);
    const outcomeRows = mc.randomOutcomeRows.map((row, idx) => {
      const baseResidual = num(row.residualCost || row.totalCost || residualExpectedLoss);
      return {
        iteration: idx + 1,
        componentId: c.id || "",
        componentName: c.name || "Component",
        residualLoss: Math.max(0, Math.round(baseResidual - insuranceImpact)),
        totalLoss: Math.round(num(row.totalCost || baseResidual)),
        insuranceImpact: Math.round(insuranceImpact)
      };
    });
    return {
      component: c,
      expectedLoss: num(mc.expectedLoss || c.inheritedInherent || residualExpectedLoss),
      residualExpectedLoss,
      p95: quantile(outcomeRows.map(r=>r.residualLoss), 0.95),
      p99: quantile(outcomeRows.map(r=>r.residualLoss), 0.99),
      insuranceImpact,
      outcomeRows
    };
  }
  function evaluateSavedScenario(s){
    const comps = componentsForScenario(s).map(c => normalizeComponent(c, s));
    const iterationCount = Math.max(100, Math.min(10000, num(s?.randomScenarioCount || comps[0]?.randomScenarioCount || 1000)));
    const componentResults = comps.map(c => runComponentMonteCarlo(c, iterationCount));
    const totalRisk = componentResults.reduce((sum,r)=>sum + num(r.residualExpectedLoss), 0);
    const totalInherent = componentResults.reduce((sum,r)=>sum + num(r.expectedLoss), 0);
    const avgComponentRisk = componentResults.length ? totalRisk / componentResults.length : totalRisk;
    const outcomes = [];
    for (let i=0; i<iterationCount; i++) {
      let residualLoss = 0, totalLoss = 0, insuranceImpact = 0;
      componentResults.forEach(r => {
        const row = r.outcomeRows[i % r.outcomeRows.length] || {};
        residualLoss += num(row.residualLoss);
        totalLoss += num(row.totalLoss);
        insuranceImpact += num(row.insuranceImpact);
      });
      outcomes.push({ iteration:i+1, scenarioId:s?.id || "", scenarioName:scenarioLabel(s), residualLoss:Math.round(residualLoss), totalLoss:Math.round(totalLoss), insuranceImpact:Math.round(insuranceImpact) });
    }
    const highComponent = componentResults.slice().sort((a,b)=>num(b.residualExpectedLoss)-num(a.residualExpectedLoss))[0];
    const p95 = quantile(outcomes.map(o=>o.residualLoss), 0.95);
    const p99 = quantile(outcomes.map(o=>o.residualLoss), 0.99);
    const volatility = Math.max(0, p95 - quantile(outcomes.map(o=>o.residualLoss), 0.05));
    const status = avgComponentRisk >= 500000 ? "High" : avgComponentRisk >= 100000 ? "Elevated" : avgComponentRisk >= 25000 ? "Moderate" : "Low";
    return {
      scenario:s,
      id:s?.id || "",
      ...
      highestComponent:highComponent?.component?.name || "Scenario",
      topDriver:(highComponent?.component?.items || [])[0]?.name || (highComponent?.component?.items || [])[0]?.riskTitle || highComponent?.component?.riskDomain || s?.riskDomain || "Review probability, impact, mitigation, and insurance assumptions",
      status,
      componentResults,
      outcomes,
      narrative:`${scenarioLabel(s)} is ranked using average component residual risk of ${money(avgComponentRisk)} and total residual exposure of ${money(totalRisk)}. The highest contributing component is ${highComponent?.component?.name || "the scenario itself"}. Management should validate the probability and impact ranges, confirm mitigation effectiveness, and determine whether insurance transfer materially offsets the residual exposure.`
    };
  }
  function getPortfolioRows(){
    const saved = (typeof getSavedScenarios === "function" ? getSavedScenarios() : []) || [];
    return saved.map(evaluateSavedScenario).sort((a,b)=> (b.avgComponentRisk - a.avgComponentRisk) || (b.totalRisk - a.totalRisk));
  }
  window.rt48GetPortfolioRows = getPortfolioRows;
  window.rt48EvaluateSavedScenario = evaluateSavedScenario;

  function drawBars(canvas, rows){
    if (!canvas || !canvas.getContext) return;
    const ctx = canvas.getContext("2d");
    const w = canvas.width = canvas.clientWidth || 760;
    const h = canvas.height = Number(canvas.getAttribute("height")) || 260;
    ctx.clearRect(0,0,w,h);
    ctx.font = "12px Arial";
    ctx.fillText("Scenario ranking by Avg Component Risk (ranked) with Total Risk shown in table", 12, 18);
    const top = rows.slice(0, 10);
    const max = Math.max(1, ...top.map(r=>r.avgComponentRisk));
    const barH = Math.max(16, Math.floor((h-45)/Math.max(1,top.length))-6);
    top.forEach((r,i)=>{
      const y = 36 + i*(barH+6);
      const label = (r.name || "Scenario").slice(0, 28);
      const barW = Math.max(2, Math.round((w-260) * (r.avgComponentRisk/max)));
      ctx.fillRect(190, y, barW, barH);
      ctx.fillText(`${i+1}. ${label}`, 12, y+barH-3);
      ctx.fillText(money(r.avgComponentRisk), 200+barW, y+barH-3);
    });
  }
  function ensurePortfolioUi(){
    const reportsView = document.getElementById("view-reports");
    if (!reportsView) return null;
    let card = document.getElementById("rt48PortfolioReportCard");
    if (card) return card;
    card = document.createElement("div");
    card.className = "card";
    card.id = "rt48PortfolioReportCard";
    card.innerHTML = `
      <div class="card-header"><h3>Portfolio Scenario Review</h3><span>All Saved Scenarios</span></div>
      <div class="note-box" id="rt48PortfolioSummary">Runs every saved Single and Complex scenario, auto-generates Monte Carlo outcomes, shows both Avg Component Risk and Total Risk, and ranks by Avg Component Risk.</div>
      <div class="builder-actions" style="gap:8px; flex-wrap:wrap;">
        <button class="btn btn-primary" type="button" id="rt48RunPortfolioReportBtn">Run Portfolio Report</button>
        <button class="btn btn-secondary" type="button" id="rt48SavePortfolioReportBtn">Save Report Snapshot</button>
        <button class="btn btn-secondary" type="button" id="rt48DownloadPortfolioCsvBtn">Download Ranking CSV</button>
      </div>
      <div class="table-wrap"><table>
        <thead><tr><th>Rank</th><th>Scenario</th><th>Type</th><th>Area / Product Family</th><th>Avg Component Risk</th><th>Total Risk</th><th>P95</th><th>P99</th><th>Components</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody id="rt48PortfolioReportBody"><tr><td colspan="11">Run report to rank every saved scenario.</td></tr></tbody>
      </table></div>
      <canvas id="rt48PortfolioRankChart" height="280" width="760" style="width:100%;max-width:100%;margin-top:12px;"></canvas>
      <div class="note-box" id="rt48ScenarioNarrativeBox">Select Scenario Review to see the management concern narrative and Monte Carlo outcome preview.</div>
      <div class="table-wrap"><table>
        <thead><tr><th>Iteration</th><th>Scenario</th><th>Total Loss</th><th>Residual Loss</th><th>Insurance Impact</th></tr></thead>
        <tbody id="rt48OutcomePreviewBody"><tr><td colspan="5">Select View Outcomes to preview Monte Carlo outcome rows for a scenario.</td></tr></tbody>
      </table></div>`;
    const anchor = document.getElementById("riskPriorityCard") || reportsView.firstElementChild?.nextSibling || reportsView.firstChild;
    reportsView.insertBefore(card, anchor);
    return card;
  }
  function renderPortfolioReport(){
    ensurePortfolioUi();
    const rows = getPortfolioRows();
    window.rt48LastPortfolioRows = rows;
    const tbody = document.getElementById("rt48PortfolioReportBody");
    const summary = document.getElementById("rt48PortfolioSummary");
    if (!tbody) return rows;
    if (!rows.length) {
      tbody.innerHTML = '<tr><td colspan="11">No saved scenarios found. Save Single or Complex scenarios first.</td></tr>';
      if (summary) summary.textContent = "No saved scenarios found.";
      return rows;
    }
    tbody.innerHTML = rows.map((r,i)=>`<tr>
      <td>${i+1}</td><td><strong>${esc(r.name)}</strong><br><small>${esc(r.id)}</small></td><td>${esc(r.type)}</td><td>${esc(r.area)}</td>
      <td><strong>${money(r.avgComponentRisk)}</strong></td><td>${money(r.totalRisk)}</td><td>${money(r.p95)}</td><td>${money(r.p99)}</td><td>${r.componentCount}</td><td>${esc(r.status)}</td>
      <td><div class="rollup-actions" style="display:flex;gap:6px;flex-wrap:nowrap;"><button class="btn btn-secondary small-btn" type="button" data-rt48-review="${esc(r.id)}">Scenario Review</button><button class="btn btn-secondary small-btn" type="button" data-rt48-outcomes="${esc(r.id)}">View Outcomes</button></div></td>
    </tr>`).join("");
    if (summary) summary.textContent = `Portfolio report ran ${rows.length} saved scenario${rows.length === 1 ? "" : "s"}. Ranking is by Avg Component Risk; Total Risk is shown for exposure context.`;
    drawBars(document.getElementById("rt48PortfolioRankChart"), rows);
    return rows;
  }
  function showScenarioReview(id, showOutcomes){
    const rows = window.rt48LastPortfolioRows || renderPortfolioReport();
    const r = rows.find(x => String(x.id) === String(id)) || rows[0];
    if (!r) return;
    const box = document.getElementById("rt48ScenarioNarrativeBox");
    if (box) box.innerHTML = `<strong>${esc(r.name)}</strong><br>${esc(r.narrative)}<br><br><strong>Decision metrics:</strong> Avg Component Risk ${money(r.avgComponentRisk)} | Total Risk ${money(r.totalRisk)} | P95 ${money(r.p95)} | P99 ${money(r.p99)} | Volatility ${money(r.volatility)}.`;
    if (showOutcomes) {
      const body = document.getElementById("rt48OutcomePreviewBody");
      if (body) body.innerHTML = r.outcomes.slice(0, 100).map(o=>`<tr><td>${o.iteration}</td><td>${esc(o.scenarioName)}</td><td>${money(o.totalLoss)}</td><td>${money(o.residualLoss)}</td><td>${money(o.insuranceImpact)}</td></tr>`).join("");
      window.rt48CurrentOutcomeRows = r.outcomes;
    }
  }
  ...
  function portfolioCsv(){
    const rows = window.rt48LastPortfolioRows || renderPortfolioReport();
    return [["Rank","Scenario ID","Scenario","Type","Area/Product Family","Avg Component Risk","Total Risk","P95","P99","Components","Status","Narrative"]].concat(rows.map((r,i)=>[i+1,r.id,r.name,r.type,r.area,Math.round(r.avgComponentRisk),Math.round(r.totalRisk),Math.round(r.p95),Math.round(r.p99),r.componentCount,r.status,r.narrative]));
  }
  async function saveSnapshot(){
    const rows = window.rt48LastPortfolioRows || renderPortfolioReport();
    const text = "RiskTool Portfolio Scenario Review - " + new Date().toLocaleString() + "\n\n" + rows.map((r,i)=>`${i+1}. ${r.name} (${r.type})\nArea/Product Family: ${r.area}\nAvg Component Risk: ${money(r.avgComponentRisk)}\nTotal Risk: ${money(r.totalRisk)}\nP95/P99: ${money(r.p95)} / ${money(r.p99)}\nNarrative: ${r.narrative}\n`).join("\n");
    if (window.rtWorkspaceWriteText) { await window.rtWorkspaceWriteText("reports/portfolio-scenario-review-" + Date.now() + ".txt", text); alert("Saved portfolio scenario review to workspace reports folder."); }
    else { const blob = new Blob([text], { type:"text/plain" }); const url = URL.createObjectURL(blob); const a=document.createElement("a"); a.href=url; a.download="portfolio-scenario-review-"+Date.now()+".txt"; a.click(); setTimeout(()=>URL.revokeObjectURL(url), 1000); }
  }
  function updateAreaRollupAverageField(){
    const field = document.getElementById("complexSectionWeight");
    if (!field) return;
    const comps = Array.isArray(window.complexScenarioComponents) ? window.complexScenarioComponents : [];
    if (!comps.length) { field.value = "Calculated after components"; return; }
    const rows = comps.map(c => runComponentMonteCarlo(normalizeComponent(c, {mode:"complex"}), 500));
    const avg = rows.reduce((s,r)=>s+num(r.residualExpectedLoss),0) / rows.length;
    const high = rows.slice().sort((a,b)=>num(b.residualExpectedLoss)-num(a.residualExpectedLoss))[0];
    field.value = `${money(avg)} avg component residual risk | ${rows.length} component${rows.length === 1 ? "" : "s"} | Highest: ${high?.component?.name || "Component"}`;
  }
  function wrapRenderComplex(){
    if (typeof renderComplexProductSections === "function" && !renderComplexProductSections.__rt48Wrapped) {
      const prior = renderComplexProductSections;
      const wrapped = function(){ const out = prior.apply(this, arguments); try { updateAreaRollupAverageField(); } catch(e){} return out; };
      wrapped.__rt48Wrapped = true;
      window.renderComplexProductSections = renderComplexProductSections = wrapped;
    }
  }
  function setVersion(){
    document.querySelectorAll("#appVersion,#versionLabel,#dashboardVersion,.version,[data-version]").forEach(el => { if (el) el.textContent = PHASE; });
    document.querySelectorAll(".sidebar-note h4").forEach(el => { if ((el.textContent||"").includes("Phase")) el.textContent = "Phase " + PHASE; });
  }
  document.addEventListener("click", function(e){
    const run = e.target.closest("#rt48RunPortfolioReportBtn,#generateOverallRiskReport,#generatePortfolioRiskReport");
    if (run) { setTimeout(renderPortfolioReport, 0); return; }
    const review = e.target.closest("[data-rt48-review]");
    if (review) { showScenarioReview(review.dataset.rt48Review, false); return; }
    const outcomes = e.target.closest("[data-rt48-outcomes]");
    if (outcomes) { showScenarioReview(outcomes.dataset.rt48Outcomes, true); return; }
    if (e.target.closest("#rt48DownloadPortfolioCsvBtn")) { downloadCsv("portfolio_scenario_review_"+Date.now()+".csv", portfolioCsv()); return; }
    if (e.target.closest("#rt48SavePortfolioReportBtn")) { saveSnapshot(); return; }
  }, true);
  document.addEventListener("DOMContentLoaded", function(){
    setVersion(); ensurePortfolioUi(); wrapRenderComplex();
    setTimeout(()=>{ try { renderPortfolioReport(); updateAreaRollupAverageField(); } catch(e){ console.warn(e); } }, 400);
  });
  setTimeout(()=>{ setVersion(); wrapRenderComplex(); try { updateAreaRollupAverageField(); } catch(e){} }, 1000);
})();
/* ===== END PHASE 23.0.48 ===== */
