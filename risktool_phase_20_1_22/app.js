...
function activateView(viewName) {
  ...
  if (typeof ensurePhase2022HelpExperience === "function") {
    ensurePhase2022HelpExperience();
    refreshViewContextHelp(viewName);
  }
}
...
function renderScenarioSummary(summary) {
  ...
  if (typeof ensurePhase2022HelpExperience === "function") {
    ensurePhase2022HelpExperience();
    refreshViewContextHelp("reports");
    wireHelpInteractions(document.getElementById("view-reports"));
  }
}
...
function init() {
  ...
  if (typeof ensurePhase2022HelpExperience === "function") {
    ensurePhase2022HelpExperience();
    refreshViewContextHelp("dashboard");
  }
}
...
/* =========================
   PHASE 20.1.22
   Interactive Help Layer Through Information Menu Only
========================= */

const HELP_SECTION_MAP = {
  gettingStarted: "manual-getting-started",
  single: "manual-single-scenario",
  complex: "manual-complex-scenario",
  field: "manual-field-guidance",
  results: "manual-results",
  insurance: "manual-insurance",
...
const VIEW_HELP_CONTENT = {
  dashboard: {
    ...
    body: "Use the dashboard to monitor open scenarios, review the generated narrative, and decide whether graph output should appear for leadership. Open-scenario IDs should take the user back to the original builder so review can happen in context.",
    bullets: ["Check open scenarios that are not closed.", "Use the heat map for concentration and prioritization.", "Review the generated summary before using it in management reporting."],
    section: HELP_SECTION_MAP.results
  },
  single: {
    title: "Single Scenario Guidance",
    body: "Use Single Scenario for one focused issue, event, or control concern. Complete the core scenario fields first, then add insurance, hard facts, mitigation, and accepted-risk governance only when they apply.",
    bullets: ["Start with title, ownership, status, and description.", "Set realistic hard-cost and soft-cost ranges.", "Use evidence and insurance to improve reasonableness."],
    section: HELP_SECTION_MAP.single
  },
  complex: {
    title: "Complex Scenario Guidance",
    body: "Use Complex Scenario for a broader initiative, business line, or multi-part risk set. Each component should be clear, separately meaningful, and grouped under the same complex group ID.",
    bullets: ["Add component scenarios before relying on aggregate outputs.", "Use weighted risk items to reflect relative importance.", "Keep naming consistent so grouped reporting stays readable."],
    section: HELP_SECTION_MAP.complex
  },
  beta: {
    title: "Beta Scenario Guidance",
    body: "Use Beta Scenario when you need a projected future-state estimate built from minimum, most likely, and maximum assumptions. This is useful for planning, forecasting, and proposal review.",
    bullets: ["Set min, mode, and max from grounded assumptions.", "Use the expected value and P10/P50/P90 outputs to frame uncertainty.", "Treat beta outputs as decision support, not a historical loss record."],
    section: HELP_SECTION_MAP.results
  },
  categories: {
    title: "Category Admin Guidance",
    body: "Category Admin controls the selectable values used across scenario builders. Changes here affect consistency and should be reviewed carefully because category drift can weaken reporting quality.",
    bullets: ["Keep category names clean and standardized.", "Avoid duplicates with small wording changes.", "Restore defaults only when you truly need to reset the library."],
    section: HELP_SECTION_MAP.reference
  },
  saved: {
    title: "Saved Scenario Guidance",
    body: "Saved Scenarios acts as the browser-based working library until shared storage is added. Reopen from here when you need to update assumptions, rerun a scenario, or prepare reporting.",
    bullets: ["Use IDs to reopen the correct builder.", "Review status before including a scenario in reporting.", "Closed items can stay saved without cluttering the open dashboard table."],
    section: HELP_SECTION_MAP.reference
  },
  reports: {
    title: "Reports Guidance",
    body: "Reports translate model output into leadership-ready language. Review the narrative, decision metrics, horizon table, and Monte Carlo explanation for reasonableness before using the output externally.",
    bullets: ["Keep the Monte Carlo explanation enabled for examiner-facing output.", "Use decision metrics to compare mitigation cost with reduction value.", "Review insurance and evidence totals before finalizing a board packet."],
    section: HELP_SECTION_MAP.results
  },
  information: {
    title: "Information Center Guidance",
    body: "All manual and help content should flow through this Information view. Field help should link back here so users have one consistent reference point instead of scattered floating help entry points.",
    bullets: ["Use this page as the single source of manual guidance.", "Expand sections as needed while working in the builders.", "Linked help should return the reader to the exact manual section."],
    section: HELP_SECTION_MAP.gettingStarted
  }
};

function ensureHelpSystemStyles() {
  if (document.getElementById("phase2022-help-styles")) return;
  const style = document.createElement("style");
  style.id = "phase2022-help-styles";
  style.textContent = `
    .context-help-card { margin: 14px 0 18px 0; border: 1px solid rgba(23,58,140,.12); border-left: 4px solid #d96b1f; border-radius: 16px; background: linear-gradient(180deg, rgba(23,58,140,.04), rgba(217,107,31,.05)); padding: 16px 18px; box-shadow: 0 4px 14px rgba(0,0,0,.04); }
    .context-help-card h3 { margin: 0 0 8px 0; font-size: 16px; }
    .context-help-card p { margin: 0 0 10px 0; line-height: 1.6; }
    .context-help-card ul { margin: 0 0 12px 18px; padding: 0; line-height: 1.55; }
    .context-help-card li + li { margin-top: 4px; }
    .context-help-card .context-help-link { display: inline-flex; align-items: center; gap: 6px; font-weight: 700; color: #173a8c; text-decoration: none; }
    .help-label, .help-tip { position: relative; cursor: help; }
    .help-label::after, .help-tip::after { content: " ?"; color: #d96b1f; font-weight: 800; }
    .help-tooltip { position: fixed; z-index: 9999; max-width: 320px; border-radius: 14px; background: #172033; color: #fff; padding: 12px 14px; box-shadow: 0 12px 32px rgba(0,0,0,.22); font-size: 13px; line-height: 1.55; pointer-events: auto; opacity: 0; transform: translateY(4px); transition: opacity .14s ease, transform .14s ease; }
    .help-tooltip.open { opacity: 1; transform: translateY(0); }
    .help-tooltip-title { font-weight: 800; margin-bottom: 4px; color: #fff; }
    .help-tooltip-copy { color: rgba(255,255,255,.92); }
    .help-tooltip-link { display: inline-block; margin-top: 8px; color: #9ec2ff; font-weight: 700; text-decoration: none; }
    .manual-toggle { border-top: 1px solid rgba(0,0,0,.06); padding: 12px 0; }
    .manual-toggle:first-of-type { border-top: none; padding-top: 0; }
    .manual-toggle summary { list-style: none; cursor: pointer; display: flex; align-items: center; justify-content: space-between; gap: 10px; font-weight: 700; font-size: 18px; }
    .manual-toggle summary::-webkit-details-marker { display: none; }
    .manual-toggle summary::after { content: "+"; font-size: 22px; line-height: 1; color: #d96b1f; }
    .manual-toggle[open] summary::after { content: "−"; }
    .manual-toggle-body { padding: 10px 0 6px 0; }
    .manual-toggle-body p, .manual-toggle-body li { line-height: 1.65; }
  `;
  document.head.appendChild(style);
}

function buildCollapsibleManualHtml() {
  return `
    ${getManualLayoutStyles()}
    <div class="manual-card">
      <h4>Information / User Guide</h4>
      ...
      <details class="manual-toggle" id="manual-getting-started" open>
      ...
      <details class="manual-toggle" id="manual-single-scenario">
        <summary>Single Scenario Guide</summary>
        <div class="manual-toggle-body">
          <ol>
            <li>Enter a clear scenario title.</li>
            <li>Describe the business context, product, process, or event involved.</li>
            <li>Set severity and control assumptions that reflect operating reality.</li>
            <li>Add mitigation information for controls already in place or planned.</li>
            <li>Add evidence items when you have known losses, incidents, audit findings, or external events.</li>
            <li>Add insurance entries if coverage exists or is being evaluated.</li>
            <li>Save the scenario before running if you want to preserve the current state.</li>
      ...
      <details class="manual-toggle" id="manual-complex-scenario">
        <summary>Complex Scenario Guide</summary>
        <div class="manual-toggle-body">
          <ol>
            <li>Confirm the complex scenario grouping ID.</li>
            <li>Add each component scenario as a distinct item within the same group.</li>
            <li>Use consistent naming so components are easy to distinguish in reports.</li>
            <li>Add risk items and component assumptions before relying on aggregate outputs.</li>
            <li>Review grouped results for reasonableness before reporting.</li>
          </ol>
        </div>
      </details>

      <details class="manual-toggle" id="manual-field-guidance">
        <summary>Field Guidance</summary>
        <div class="manual-toggle-body">
          <div class="manual-field-grid">
            <div class="manual-field-item"><strong>Scenario Title</strong><div>Use a clear and specific name.</div></div>
            <div class="manual-field-item"><strong>Likelihood / Impact / Weighted Score</strong><div>Use grounded assumptions and consistent scoring discipline.</div></div>
            <div class="manual-field-item"><strong>Hard Cost</strong><div>Capture direct measurable loss such as refunds, fines, remediation, write-offs, vendor cost, or legal spend.</div></div>
            <div class="manual-field-item"><strong>Soft Cost Multiplier</strong><div>Use this to reflect secondary impact such as complaint volume, operational drag, management time, or brand pressure.</div></div>
            <div class="manual-field-item"><strong>Mitigation</strong><div>Document controls or actions that reduce exposure.</div></div>
            <div class="manual-field-item"><strong>Evidence</strong><div>Add factual loss history or supporting data.</div></div>
            <div class="manual-field-item"><strong>Insurance</strong><div>Add policy title, premium, deductible, and key terms.</div></div>
          </div>
      ...
      <details class="manual-toggle" id="manual-results">
        <summary>Understanding Results</summary>
        <div class="manual-toggle-body">
          <ul>
            <li>Expected Annual Loss is the modeled annual total cost.</li>
            <li>Residual Annual Loss is what remains after the stated control effectiveness is applied.</li>
            <li>P10 / P50 / P90 frame the distribution of likely outcomes.</li>
            <li>Decision metrics compare mitigation cost with modeled reduction value.</li>
          </ul>
        </div>
      </details>

      <details class="manual-toggle" id="manual-insurance">
        <summary>Insurance Evaluation</summary>
        <div class="manual-toggle-body">
          <p>Insurance effectiveness compares coverage cost structure against modeled protection. Review premium, deductible, and coverage amount together instead of treating insurance as a simple checklist item.</p>
        </div>
      </details>

      <details class="manual-toggle" id="manual-evidence">
        <summary>Evidence &amp; Data Usage</summary>
        <div class="manual-toggle-body">
          <p>Evidence entries should be factual and traceable. Use actual losses, incidents, audit findings, external events, and documented amounts where available.</p>
          <p>Evidence helps anchor uncertainty and improves the reasonableness of scenario outputs.</p>
        </div>
      </details>

      <details class="manual-toggle" id="manual-reference">
        <summary>Reference</summary>
        <div class="manual-toggle-body">
          <ul>
            <li>Single Scenario is best for one defined issue or event.</li>
            <li>Complex Scenario is best for grouped risks under one broader initiative.</li>
            <li>Reports should be reviewed for reasonableness before management, audit, or board use.</li>
          </ul>
        </div>
      </details>

      <details class="manual-toggle" id="manual-faq">
        <summary>FAQ</summary>
        <div class="manual-toggle-body">
          <ul>
            <li>Save preserves the scenario.</li>
...
function getPolishedManualHtml() {
  return buildCollapsibleManualHtml();
}

function renderManual() {
  const target = document.getElementById("userManualCopy");
  if (!target) return;
  target.innerHTML = buildCollapsibleManualHtml();
}

function forceManualContent() {
  const manual = document.getElementById("userManualCopy");
  if (!manual) return;
  manual.innerHTML = buildCollapsibleManualHtml();
}

function resolveHelpSectionFromElement(element) {
  if (!element) return HELP_SECTION_MAP.reference;
  if (element.dataset.helpSection) return element.dataset.helpSection;
  const text = String((element.getAttribute("aria-label") || element.textContent || element.dataset.help || "")).toLowerCase();
  const closestView = element.closest(".view")?.id || "";
  if (text.includes("insurance") || closestView.includes("insurance")) return HELP_SECTION_MAP.insurance;
  if (text.includes("evidence") || text.includes("hard fact")) return HELP_SECTION_MAP.evidence;
  if (text.includes("report") || text.includes("residual") || text.includes("expected annual") || text.includes("monte carlo") || text.includes("risk reduction") || text.includes("net benefit")) return HELP_SECTION_MAP.results;
  if (text.includes("complex") || text.includes("risk item") || text.includes("group id") || text.includes("component id") || closestView === "view-complex") return HELP_SECTION_MAP.complex;
  if (text.includes("single") || closestView === "view-single") return HELP_SECTION_MAP.single;
  if (text.includes("beta") || closestView === "view-beta") return HELP_SECTION_MAP.results;
  if (text.includes("control") || text.includes("cost") || text.includes("likelihood") || text.includes("impact") || text.includes("weight") || text.includes("scenario") || text.includes("product") || text.includes("regulation") || text.includes("owner")) return HELP_SECTION_MAP.field;
  return HELP_SECTION_MAP.reference;
}

function assignHelpSections(root = document) {
  root.querySelectorAll?.("[data-help]").forEach((element) => {
    if (!element.dataset.helpSection) {
      element.dataset.helpSection = resolveHelpSectionFromElement(element);
    }
  });
}

function ensureHelpTooltip() {
  let tooltip = document.getElementById("risktool-help-tooltip");
  if (tooltip) return tooltip;
  tooltip = document.createElement("div");
  tooltip.id = "risktool-help-tooltip";
  tooltip.className = "help-tooltip";
  tooltip.setAttribute("role", "dialog");
  tooltip.innerHTML = '<div class="help-tooltip-title">Field Help</div><div class="help-tooltip-copy"></div><a href="#" class="help-tooltip-link">Learn more in Information</a>';
  document.body.appendChild(tooltip);
  const link = tooltip.querySelector(".help-tooltip-link");
  if (link) {
    link.addEventListener("click", (event) => {
      event.preventDefault();
      const section = tooltip.dataset.helpSection || HELP_SECTION_MAP.reference;
      navigateToManualSection(section);
      hideHelpTooltip();
    });
  }
  return tooltip;
...
function positionHelpTooltip(target, tooltip) {
  if (!target || !tooltip) return;
  const rect = target.getBoundingClientRect();
  const tooltipRect = tooltip.getBoundingClientRect();
  const top = Math.min(window.innerHeight - tooltipRect.height - 12, Math.max(12, rect.bottom + 10));
  const preferredLeft = rect.left + Math.max(0, (rect.width / 2) - (tooltipRect.width / 2));
...
function showHelpTooltip(target) {
  ...
  tooltip.dataset.helpSection = target.dataset.helpSection || resolveHelpSectionFromElement(target);
  const titleEl = tooltip.querySelector(".help-tooltip-title");
  const copyEl = tooltip.querySelector(".help-tooltip-copy");
  const linkEl = tooltip.querySelector(".help-tooltip-link");
  if (titleEl) titleEl.textContent = (target.textContent || target.getAttribute("aria-label") || "Field Help").trim().replace(/\?$/,'') || "Field Help";
  if (copyEl) copyEl.textContent = target.dataset.help || "Reference available in the Information menu.";
  if (linkEl) linkEl.textContent = "Learn more in Information";
  tooltip.classList.add("open");
  positionHelpTooltip(target, tooltip);
}

function hideHelpTooltip() {
  const tooltip = document.getElementById("risktool-help-tooltip");
  if (!tooltip) return;
  tooltip.classList.remove("open");
}

function wireHelpInteractions(root = document) {
  const elements = root.querySelectorAll ? root.querySelectorAll("[data-help]") : [];
  elements.forEach((element) => {
    if (element.dataset.helpWired === "true") return;
    element.dataset.helpWired = "true";
    if (!element.hasAttribute("tabindex")) element.setAttribute("tabindex", "0");
    element.addEventListener("mouseenter", () => showHelpTooltip(element));
    element.addEventListener("mouseleave", hideHelpTooltip);
    element.addEventListener("focus", () => showHelpTooltip(element));
    element.addEventListener("blur", hideHelpTooltip);
  });
}

function ensureViewContextCard(viewName) {
  const view = document.getElementById(`view-${viewName}`);
  if (!view) return null;
  let card = view.querySelector(".context-help-card");
  if (card) return card;
  const sectionHeader = view.querySelector(".section-header, .hero");
  card = document.createElement("div");
  card.className = "context-help-card";
  if (sectionHeader && sectionHeader.parentNode) {
    if (sectionHeader.classList.contains("hero")) {
...
function refreshViewContextHelp(viewName) {
  const content = VIEW_HELP_CONTENT[viewName];
  if (!content) return;
  const card = ensureViewContextCard(viewName);
  if (!card) return;
  card.innerHTML = `
    <h3>${escapeHtml(content.title)}</h3>
    <p>${escapeHtml(content.body)}</p>
    <ul>${(content.bullets || []).map(item => `<li>${escapeHtml(item)}</li>`).join("")}</ul>
    <a href="#${escapeHtml(content.section)}" class="context-help-link" data-context-manual-link="${escapeHtml(content.section)}">Open related Information section</a>
  `;
  const link = card.querySelector("[data-context-manual-link]");
  if (link && !link.dataset.wired) {
    link.dataset.wired = "true";
    link.addEventListener("click", (event) => {
      event.preventDefault();
      navigateToManualSection(link.dataset.contextManualLink || HELP_SECTION_MAP.reference);
    });
  }
}
...
function ensurePhase2022HelpExperience() {
  ensureHelpSystemStyles();
  renderManual();
  assignHelpSections(document);
  wireHelpInteractions(document);
  Object.keys(VIEW_HELP_CONTENT).forEach(refreshViewContextHelp);
}
