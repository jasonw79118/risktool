...
/* ===== END PHASE 23.0.53 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.53 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.53 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.53 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.53 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.53 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.53 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.53 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.53 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.53 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.53";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.53 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.53 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 AREA ROLLUP + ENTERPRISE RISK REPORT ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 HUBBARD EXPECTED LOSS + PORTFOLIO AREA RANKING ===== */
(function(){
  const PHASE = "23.0.53";
...
/* ===== PHASE 23.0.53 EVIDENCE-DRIVEN RISK ENGINE + VERSION STABILIZATION ===== */
(function(){
  const PHASE="23.0.53";
  ...
  function retireSubjectiveFields(){
    ...
    Object.entries(labels).forEach(([id,text])=>{const el=document.getElementById(id),lab=el?.parentElement?.querySelector('label');if(lab){lab.textContent=text;lab.dataset.help='RiskTool 23.0.53 retires subjective scoring. Risk should be supported by internal/external historical evidence, quantified cost ranges, controls, mitigations, insurance, and Monte Carlo outcomes.';}});
...
/* === RiskTool 23.0.53 artifact table action integrity patch === */
(function(){
  const VERSION='23.0.53';
  function safeId(prefix){return prefix+'-'+Date.now()+'-'+Math.floor(Math.random()*1000000);}
  function byId(id){return document.getElementById(id);} function esc(v){return (typeof escapeHtml==='function')?escapeHtml(v??''):String(v??'');}
  function cur(v){return (typeof currency==='function')?currency(Number(v||0)):'$'+Number(v||0).toLocaleString();}
  function title(m){return m.charAt(0).toUpperCase()+m.slice(1);} function setv(id,v){const e=byId(id);if(e)e.value=v??'';} function sett(id,v){const e=byId(id);if(e)e.textContent=v??'';} function chk(id,v){const e=byId(id);if(e)e.checked=!!v;}
  function sel(id,v){if(typeof setSelectValueSafe==='function')setSelectValueSafe(id,v??'');else setv(id,v??'');} function setc(id,v){if(typeof setCurrencyFieldValue==='function')setCurrencyFieldValue(id,v||0);else setv(id,v||0);}
  const key={riskItem:'issueId',insurance:'insuranceId',hardFact:'hardFactId',mitigation:'mitigationId',acceptedRisk:'acceptedRiskId'}; const pref={riskItem:'ISS',insurance:'INS',hardFact:'HF',mitigation:'MIT',acceptedRisk:'AR'};
  function list(kind,mode){if(kind==='riskItem')return currentComplexItems;if(kind==='insurance')return mode==='single'?singleInsurance:mode==='complex'?complexInsurance:betaInsurance;if(kind==='hardFact')return mode==='single'?singleHardFacts:mode==='complex'?complexHardFacts:betaHardFacts;if(kind==='mitigation')return mode==='single'?singleMitigations:complexMitigations;if(kind==='acceptedRisk')return mode==='single'?window.singleAcceptedRisks:window.complexAcceptedRisks;return[];}
  function norm(kind,mode){const a=list(kind,mode), k=key[kind], p=pref[kind]||'REC'; if(!Array.isArray(a))return[]; a.forEach(x=>{if(x&&!x[k])x[k]=safeId(p);}); return a;}
  function replace(kind,mode,next){const a=list(kind,mode); if(Array.isArray(a)) a.splice(0,a.length,...next);}
  function actions(kind,mode,id){return '<div class="table-actions"><button type="button" class="btn btn-secondary small-btn" data-rt53-edit-kind="'+kind+'" data-rt53-edit-mode="'+mode+'" data-rt53-edit-id="'+esc(id)+'">Edit</button> <button type="button" class="btn btn-danger small-btn" data-rt53-delete-kind="'+kind+'" data-rt53-delete-mode="'+mode+'" data-rt53-delete-id="'+esc(id)+'">Delete</button></div>';}
  function refresh(kind,mode){ if(kind==='riskItem'){renderComplexItems();return;} if(kind==='insurance')renderInsuranceTable(mode+'InsuranceBody',list(kind,mode)); if(kind==='hardFact')renderHardFactsTable(mode+'HardFactsBody',list(kind,mode)); if(kind==='mitigation')renderMitigationTable(mode+'MitigationBody',list(kind,mode)); if(kind==='acceptedRisk')renderAcceptedRiskTable(mode+'AcceptedRiskBody',list(kind,mode),mode); if(mode==='complex'&&typeof renderComplexProductSections==='function')renderComplexProductSections(); }
  window.renderInsuranceTable=renderInsuranceTable=function(targetId,items){const tb=byId(targetId);if(!tb)return;const mode=targetId.replace('InsuranceBody','');norm('insurance',mode);const a=items||list('insurance',mode);if(!a.length){tb.innerHTML='<tr><td colspan="11">No insurance entries added yet.</td></tr>';return;}tb.innerHTML=a.map(x=>'<tr><td>'+esc(x.policyName)+'</td><td>'+esc(x.policyNumber)+'</td><td>'+esc(x.carrier)+'</td><td>'+esc(x.coverageType)+'</td><td>'+cur(x.premium)+'</td><td>'+cur(x.deductible)+'</td><td>'+cur(x.coverageAmount)+'</td><td>'+esc(x.coverageDates)+'</td><td>'+esc(x.notes)+'</td><td>'+esc(x.sourceLink)+'</td><td>'+actions('insurance',mode,x.insuranceId)+'</td></tr>').join('');};
  window.renderHardFactsTable=renderHardFactsTable=function(targetId,items){const tb=byId(targetId);if(!tb)return;const mode=targetId.replace('HardFactsBody','');norm('hardFact',mode);const a=items||list('hardFact',mode);if(!a.length){tb.innerHTML='<tr><td colspan="6">No hard facts / evidence entries added yet.</td></tr>';return;}tb.innerHTML=a.map(x=>'<tr><td>'+esc(x.sourceType)+'</td><td>'+cur(x.amount)+'</td><td>'+esc(x.factDate)+'</td><td>'+esc(x.description)+'</td><td>'+esc(x.sourceLink)+'</td><td>'+actions('hardFact',mode,x.hardFactId)+'</td></tr>').join('');};
  window.renderMitigationTable=renderMitigationTable=function(targetId,items){const tb=byId(targetId);if(!tb)return;const mode=targetId.replace('MitigationBody','');norm('mitigation',mode);const a=items||list('mitigation',mode);if(!a.length){tb.innerHTML='<tr><td colspan="5">No mitigation factors added yet.</td></tr>';return;}tb.innerHTML=a.map(x=>'<tr><td>'+esc(x.title)+'</td><td>'+esc(x.owner)+'</td><td>'+esc(x.status)+'</td><td>'+esc(x.attachment)+'</td><td>'+actions('mitigation',mode,x.mitigationId)+'</td></tr>').join('');};
  window.renderAcceptedRiskTable=renderAcceptedRiskTable=function(targetId,items,mode){const tb=byId(targetId);if(!tb)return;mode=mode||targetId.replace('AcceptedRiskBody','');norm('acceptedRisk',mode);const a=items||list('acceptedRisk',mode);if(!a.length){tb.innerHTML='<tr><td colspan="7">No accepted-risk records added yet.</td></tr>';return;}tb.innerHTML=a.map(x=>'<tr><td>'+(x.isAccepted?'Yes':'No')+'</td><td>'+esc(x.authority)+'</td><td>'+esc(x.acceptedBy)+'</td><td>'+esc(x.acceptanceDate)+'</td><td>'+esc(x.reviewDate)+'</td><td>'+esc(x.decisionLogic)+'</td><td>'+actions('acceptedRisk',mode,x.acceptedRiskId)+'</td></tr>').join('');};
  window.renderComplexItems=renderComplexItems=function(){const tb=byId('riskItemsTableBody');if(!tb)return;norm('riskItem','complex');if(!currentComplexItems.length)tb.innerHTML='<tr><td colspan="7">No risk items added yet.</td></tr>';else tb.innerHTML=currentComplexItems.map(x=>'<tr data-issue-id="'+esc(x.issueId)+'"><td><button type="button" class="scenario-link" data-rt53-edit-kind="riskItem" data-rt53-edit-mode="complex" data-rt53-edit-id="'+esc(x.issueId)+'">'+esc(x.name)+'</button></td><td>'+esc(x.domain)+'</td><td>'+esc(x.product)+'</td><td>'+esc(x.regulation)+'</td><td>'+(x.score??'')+'</td><td>'+(x.weight??'')+'</td><td>'+actions('riskItem','complex',x.issueId)+'</td></tr>').join(''); if(typeof refreshComplexProductSectionSelects==='function')refreshComplexProductSectionSelects(); if(typeof updateInherentScores==='function')updateInherentScores(); if(typeof renderComplexProductSections==='function')renderComplexProductSections();};
  function resetRiskEdit(){window.rt53EditingRiskItemId='';sett('addRiskItemBtn','Add Risk Item');}
  const oldAddRiskItem=addRiskItem; window.addRiskItem=addRiskItem=function(){const id=window.rt53EditingRiskItemId;if(id){const i=currentComplexItems.findIndex(x=>String(x.issueId)===String(id));const rec={issueId:id,parentScenarioMode:'complex',name:byId('riskItemName')?.value||'Unnamed Risk Item',domain:byId('riskItemDomain')?.value||'',product:byId('riskItemProduct')?.value||'',regulation:byId('riskItemReg')?.value||'',description:i>=0?(currentComplexItems[i].description||''):'',score:Number(byId('riskItemScore')?.value||0),weight:Number(byId('riskItemWeight')?.value||1)}; if(i>=0)currentComplexItems[i]=rec;else currentComplexItems.push(rec);resetRiskEdit();renderComplexItems();return;} oldAddRiskItem(); norm('riskItem','complex'); renderComplexItems();};
  function edit(kind,mode,id){norm(kind,mode);const a=list(kind,mode),k=key[kind],x=a.find(r=>String(r[k])===String(id));if(!x)return;if(kind==='riskItem'){window.rt53EditingRiskItemId=id;setv('riskItemName',x.name||'');sel('riskItemDomain',x.domain||'');sel('riskItemProduct',x.product||'');sel('riskItemReg',x.regulation||'');setv('riskItemScore',x.score??'');setv('riskItemWeight',x.weight??'');sett('addRiskItemBtn','Update Risk Item');return;} if(kind==='insurance'){setEditState('insurance',mode,id);setv(mode+'InsurancePolicyName',x.policyName||'');setv(mode+'InsurancePolicyNumber',x.policyNumber||'');setv(mode+'InsuranceCarrier',x.carrier||'');setv(mode+'InsuranceCoverageType',x.coverageType||'');setc(mode+'InsurancePremium',x.premium||0);setc(mode+'InsuranceDeductible',x.deductible||0);setc(mode+'InsuranceCoverageAmount',x.coverageAmount||0);setv(mode+'InsuranceCoverageDates',x.coverageDates||'');setv(mode+'InsuranceNotes',x.notes||'');setv(mode+'InsuranceSourceLink',x.sourceLink||'');sett('add'+title(mode)+'InsuranceBtn','Update Insurance');sett(mode+'InsuranceEditStatus','Editing selected insurance record.');} if(kind==='hardFact'){setEditState('hardFact',mode,id);sel(mode+'HardFactSourceType',x.sourceType||'Internal');setc(mode+'HardFactAmount',x.amount||0);setv(mode+'HardFactDate',x.factDate||'');setv(mode+'HardFactDescription',x.description||'');setv(mode+'HardFactSourceLink',x.sourceLink||'');sett('add'+title(mode)+'HardFactBtn','Update Hard Fact');sett(mode+'HardFactEditStatus','Editing selected hard fact / evidence.');} if(kind==='mitigation'){setEditState('mitigation',mode,id);setv(mode+'MitTitle',x.title||'');setv(mode+'MitOwner',x.owner||'');setv(mode+'MitStatus',x.status||'');setv(mode+'MitAttachment',x.attachment||'');sett('add'+title(mode)+'MitigationBtn','Update Mitigation');sett(mode+'MitigationEditStatus','Editing selected mitigation.');} if(kind==='acceptedRisk'){setEditState('acceptedRisk',mode,id);chk(mode+'AcceptedRiskFlag',x.isAccepted);sel(mode+'AcceptanceAuthority',x.authority||'');setv(mode+'AcceptedBy',x.acceptedBy||'');setv(mode+'AcceptanceDate',x.acceptanceDate||'');setv(mode+'ReviewDate',x.reviewDate||'');setv(mode+'DecisionLogic',x.decisionLogic||'');sett('add'+title(mode)+'AcceptedRiskBtn','Update Accepted Risk');sett(mode+'AcceptedRiskEditStatus','Editing selected accepted-risk record.');} if(typeof formatAllCurrencyFields==='function')formatAllCurrencyFields();}
  function del(kind,mode,id){norm(kind,mode);const a=list(kind,mode),k=key[kind],x=a.find(r=>String(r[k])===String(id));if(!x)return;const label=x.name||x.policyName||x.description||x.title||x.acceptedBy||x.authority||'selected record';if(!confirm('Delete only this '+kind+' record: "'+label+'"?'))return;replace(kind,mode,a.filter(r=>String(r[k])!==String(id))); if(kind==='riskItem')resetRiskEdit(); else if(kind==='insurance'&&typeof resetInsuranceForm==='function')resetInsuranceForm(mode); else if(kind==='hardFact'&&typeof resetHardFactForm==='function')resetHardFactForm(mode); else if(kind==='mitigation'&&typeof resetMitigationForm==='function')resetMitigationForm(mode); else if(kind==='acceptedRisk'&&typeof resetAcceptedRiskForm==='function')resetAcceptedRiskForm(mode); refresh(kind,mode);}
  window.editRecord=editRecord=edit; window.deleteRecord=deleteRecord=del; window.deleteComplexRiskItem=deleteComplexRiskItem=function(id){del('riskItem','complex',id);};
  document.addEventListener('click',function(e){const eb=e.target.closest('[data-rt53-edit-kind]');if(eb){e.preventDefault();e.stopImmediatePropagation();edit(eb.dataset.rt53EditKind,eb.dataset.rt53EditMode||'complex',eb.dataset.rt53EditId);return;}const db=e.target.closest('[data-rt53-delete-kind]');if(db){e.preventDefault();e.stopImmediatePropagation();del(db.dataset.rt53DeleteKind,db.dataset.rt53DeleteMode||'complex',db.dataset.rt53DeleteId);return;}const oe=e.target.closest('[data-edit-kind][data-edit-mode][data-edit-id]');if(oe){e.preventDefault();e.stopImmediatePropagation();edit(oe.dataset.editKind,oe.dataset.editMode,oe.dataset.editId);return;}const od=e.target.closest('[data-delete-kind][data-delete-mode][data-delete-id]');if(od){e.preventDefault();e.stopImmediatePropagation();del(od.dataset.deleteKind,od.dataset.deleteMode,od.dataset.deleteId);return;}},true);
  document.addEventListener('DOMContentLoaded',function(){['single','complex','beta'].forEach(m=>{norm('insurance',m);norm('hardFact',m);});['single','complex'].forEach(m=>{norm('mitigation',m);norm('acceptedRisk',m);});norm('riskItem','complex');setTimeout(()=>{['single','complex','beta'].forEach(m=>{refresh('insurance',m);refresh('hardFact',m);});['single','complex'].forEach(m=>{refresh('mitigation',m);refresh('acceptedRisk',m);});refresh('riskItem','complex');const v=byId('appVersion');if(v)v.textContent=VERSION;},50);});
})();
