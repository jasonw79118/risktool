...
/* === RT61 Category + Single/Beta Tab Cleanup Patch === */
(function(){
  const RT61_VERSION = '23.0.61';
  function qs(s,r=document){ return r.querySelector(s); }
  function qsa(s,r=document){ return Array.from(r.querySelectorAll(s)); }
  function esc(v){ return String(v ?? '').replace(/[&<>]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;'}[c])); }
  function inject61Styles(){ if(qs('#rt61Styles')) return; const st=document.createElement('style'); st.id='rt61Styles'; st.textContent=`#view-single .rt58-tabs,#view-beta .rt58-tabs{display:none!important}#view-single .rt60-tabs,#view-beta .rt60-tabs{display:flex!important;gap:24px!important;border-bottom:1px solid #dbe4f0!important;margin:8px 0 12px!important;padding:0 6px!important;overflow-x:auto!important}#view-single .rt60-tab,#view-beta .rt60-tab{border:0!important;background:transparent!important;padding:12px 0 10px!important;font-weight:800!important;color:#25324b!important;border-bottom:3px solid transparent!important;white-space:nowrap!important}#view-single .rt60-tab.active,#view-beta .rt60-tab.active{color:#0b5bd3!important;border-bottom-color:#0b5bd3!important}#view-single .builder-actions,#view-beta .builder-actions{justify-content:flex-start!important;align-items:center!important;gap:10px!important;padding-left:18px!important}.rt61-hidden-legacy-category{display:none!important}`; document.head.appendChild(st);}
  function setVersion61(){ try{window.RISKTOOL_RUNTIME_VERSION=RT61_VERSION}catch(e){} qsa('#appVersion,.app-version,[data-version-label]').forEach(el=>el.textContent=RT61_VERSION); if(document.title) document.title='RiskTool '+RT61_VERSION; }
  function fieldWrapper(el){ return el ? el.closest('div') : null; }
  function labelText(labelEl, text){ if(!labelEl) return; const links=Array.from(labelEl.querySelectorAll('a,button')).map(n=>n.outerHTML).join(' '); labelEl.innerHTML=esc(text)+(links?' '+links:''); }
  function afterWrapper(anchorEl, movingEl){ const a=fieldWrapper(anchorEl), m=fieldWrapper(movingEl); if(a&&m&&a!==m&&a.parentNode) a.parentNode.insertBefore(m,a.nextSibling); }
  function copyOptions(fromSel,toSel){ if(!fromSel||!toSel) return; const current=toSel.value || qs('#betaProjectOrProductName')?.value || ''; const opts=Array.from(fromSel.options||[]).map(o=>o.value||o.textContent).filter(Boolean); if(!opts.length) return; toSel.innerHTML=opts.map(v=>`<option value="${esc(v)}">${esc(v)}</option>`).join(''); if(opts.includes(current)) toSel.value=current; else if(current){const opt=document.createElement('option'); opt.value=current; opt.textContent=current; toSel.insertBefore(opt,toSel.firstChild); toSel.value=current;} }
  function ensureBetaCategorySelect(){ const betaId=qs('#betaScenarioId'), old=qs('#betaProjectOrProductName'); if(!betaId||!old) return; let sel=qs('#betaCategorySelect'); if(!sel){ const wrap=document.createElement('div'); wrap.id='betaCategoryWrapper'; wrap.innerHTML='<label class="help-label" data-help="Category value comes from Category Admin. This replaces the former Project / Product Name field for scenario classification.">Category</label><select id="betaCategorySelect"></select>'; const betaIdWrap=fieldWrapper(betaId); betaIdWrap?.parentNode?.insertBefore(wrap,betaIdWrap.nextSibling); sel=qs('#betaCategorySelect'); sel.addEventListener('change',()=>{old.value=sel.value; old.dispatchEvent(new Event('input',{bubbles:true})); old.dispatchEvent(new Event('change',{bubbles:true}));}); } copyOptions(qs('#singlePrimaryProduct')||qs('#complexPrimaryProduct'),sel); if(!old.value&&sel.value) old.value=sel.value; if(old.value&&sel.value!==old.value){ const vals=Array.from(sel.options).map(o=>o.value); if(vals.includes(old.value)) sel.value=old.value; } const oldWrap=fieldWrapper(old); if(oldWrap) oldWrap.classList.add('rt61-hidden-legacy-category'); }
  function relabelCategoryFields(){ const singleProduct=qs('#singlePrimaryProduct'), complexProduct=qs('#complexPrimaryProduct'); labelText(fieldWrapper(singleProduct)?.querySelector('label'),'Category'); labelText(fieldWrapper(complexProduct)?.querySelector('label'),'Category'); afterWrapper(qs('#singleScenarioId'),singleProduct); afterWrapper(qs('#complexGroupId'),complexProduct); ensureBetaCategorySelect(); qsa('#view-categories .stat-card span,#view-categories h3,#view-categories button,#view-categories input').forEach(el=>{ if(el.tagName==='INPUT'){ if(/Product Group/i.test(el.placeholder||'')) el.placeholder=(el.placeholder||'').replace(/Product Group(s)?/gi,'Category'); } else { if(/Product Groups/i.test(el.textContent)) el.textContent=el.textContent.replace(/Product Groups/g,'Categories'); if(/Product Group/i.test(el.textContent)) el.textContent=el.textContent.replace(/Product Group/g,'Category'); }}); qsa('.rt58-label,.rt54-label,th').forEach(el=>{ const t=(el.textContent||'').trim(); if(/Product\s*\/\s*Service(\s*\/\s*Area)?/i.test(t)) el.textContent='Category'; }); }
  function cleanupSingleBetaTabs(){ ['single','beta'].forEach(viewName=>{ const view=qs('#view-'+viewName); if(!view) return; qsa('.rt58-tabs',view).forEach(t=>t.classList.add('rt61-hidden-legacy-category')); let tabs=qs('.rt60-tabs',view); const firstCard=qs('.card',view); if(!tabs&&firstCard){tabs=document.createElement('div'); tabs.className='rt60-tabs'; firstCard.parentNode.insertBefore(tabs,firstCard);} const labels=['1. Scenario Entry','2. Costs / Losses','3. Hard Facts / Evidence','4. Mitigation / Controls','5. Insurance / Risk Transfer','6. Accepted Risk']; if(tabs){ if(tabs.dataset.rt61Fixed!=='1'){tabs.innerHTML=labels.map((label,i)=>`<button type="button" class="rt60-tab ${i===0?'active':''}" data-rt60-tab="${i}">${esc(label)}</button>`).join(''); tabs.dataset.rt61Fixed='1'; const panels=qsa('.card',view); panels.forEach((p,i)=>{p.classList.add('rt60-panel'); p.classList.toggle('rt60-hidden',i>0);}); qsa('.rt60-tab',tabs).forEach(btn=>btn.addEventListener('click',()=>{qsa('.rt60-tab',tabs).forEach(x=>x.classList.toggle('active',x===btn)); panels.forEach((p,i)=>p.classList.toggle('rt60-hidden',String(i)!==btn.dataset.rt60Tab));}));} else {qsa('.rt60-tab',tabs).forEach((btn,i)=>{if(labels[i]) btn.textContent=labels[i];});}} }); }
  function moveActionButtonsLeft(){ qsa('#view-single .builder-actions,#view-beta .builder-actions').forEach(row=>{row.style.justifyContent='flex-start'; row.style.textAlign='left';}); }
  function patchDashboardWording(){ const box=qs('#aiSummaryBox'); if(box&&/Product\/service:/i.test(box.textContent||'')) box.textContent=box.textContent.replace(/Product\/service:/gi,'Category:'); }
  function boot61(){setVersion61();inject61Styles();cleanupSingleBetaTabs();relabelCategoryFields();moveActionButtonsLeft();patchDashboardWording();}
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>{setTimeout(boot61,250);setTimeout(boot61,900);}); else {setTimeout(boot61,250);setTimeout(boot61,900);}
  document.addEventListener('click',()=>setTimeout(boot61,80),true); document.addEventListener('change',e=>{if(e.target?.closest?.('#view-single,#view-beta,#view-complex,#view-categories'))setTimeout(boot61,60);},true); document.addEventListener('input',e=>{if(e.target?.closest?.('#view-single,#view-beta,#view-complex,#view-categories'))setTimeout(boot61,60);},true);
})();
/* === END RT61 === */
