...
/* ===== END PHASE 23.0.47 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.47 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.47 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.47 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.47 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.47 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.47 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.47 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.47 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.47 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.47";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.47 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.47 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 AREA ROLLUP + ENTERPRISE RISK REPORT ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 HUBBARD EXPECTED LOSS + PORTFOLIO AREA RANKING ===== */
(function(){
  const PHASE = "23.0.47";
...
/* ===== PHASE 23.0.47 AVERAGE COMPONENT RISK + SCENARIO REVIEW MONTE CARLO ===== */
(function(){
  const PHASE = "23.0.47";
  const DEFAULT_ITERATIONS = 1000;
  const IMPACT_DEFAULTS = { low:10000, moderate:50000, medium:50000, high:250000, severe:1000000, critical:1000000 };
  ...
  function esc(v){ return (typeof escapeHtml === 'function') ? escapeHtml(v) : String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c])); }
  function riskStatus(v){ v=num(v); return v>=500000?'High':v>=100000?'Elevated':v>=25000?'Moderate':'Low'; }
  function scenarioLabel(s){ return s?.name || s?.scenarioName || s?.id || 'Unnamed Scenario'; }
  function componentLabel(c){ return c?.scenarioName || c?.componentName || c?.primaryProduct || c?.name || c?.componentId || 'Unnamed Component'; }
  function activeComponents(){ try { return Array.isArray(complexScenarioComponents) ? complexScenarioComponents : []; } catch(e){ return []; } }
  function areaName(){ try { return complexProductSections?.[0]?.productServiceArea || complexProductSections?.[0]?.name || document.getElementById('complexSectionProduct')?.value || 'Selected Area'; } catch(e){ return 'Selected Area'; } }

  function triangular(min, mode, max){
    min = Math.max(0, num(min)); mode = Math.max(min, num(mode, min)); max = Math.max(mode, num(max, mode));
    if (max <= min) return min;
    const u = Math.random(); const c = (mode - min) / (max - min || 1);
  ...
  function impactMostLikely(item){
    const direct = num(item?.impactAmount || item?.financialImpact || item?.estimatedImpact || item?.lossExposure || item?.impactDollars || item?.hardCostLikely);
    if (direct > 0) return direct;
    const sev = String(item?.severity || item?.impactLevel || item?.riskLevel || item?.impactRating || '').toLowerCase();
    return IMPACT_DEFAULTS[sev] || IMPACT_DEFAULTS.moderate;
  }
  function impactRange(item){
    const likely = impactMostLikely(item);
    const min = num(item?.impactMin || item?.hardCostMin || item?.lossMin, Math.max(1000, likely * 0.4));
    const max = num(item?.impactMax || item?.hardCostMax || item?.lossMax, Math.max(likely, likely * 2.5));
    return { min: Math.min(min, likely), likely, max: Math.max(max, likely) };
  }
  function probabilityLikely(item){
    const fields = [item?.probability, item?.likelihood, item?.eventProbability, item?.frequencyProbability];
    for (const f of fields) { const p = pct(f, NaN); if (Number.isFinite(p) && p > 0) return p; }
    const l = String(item?.likelihoodRating || item?.likelihoodLevel || item?.likelihood || '').toLowerCase();
    if (l.includes('rare') || l.includes('low')) return 0.10;
    if (l.includes('moderate') || l.includes('medium')) return 0.30;
    if (l.includes('high') || l.includes('likely')) return 0.60;
    if (l.includes('certain') || l.includes('critical')) return 0.80;
    const score = num(item?.score);
    if (score > 0) return Math.max(0.05, Math.min(0.75, score / 100));
    return 0.25;
  }
  function probabilityRange(item){
    const likely = probabilityLikely(item);
    const min = pct(item?.probabilityMin || item?.likelihoodMin, Math.max(0.01, likely * 0.5));
    const max = pct(item?.probabilityMax || item?.likelihoodMax, Math.min(0.95, likely * 1.5));
    return { min: Math.min(min, likely), likely, max: Math.max(max, likely) };
  }
  function allRiskItems(obj){
    let out = [];
    if (Array.isArray(obj?.items)) out = out.concat(obj.items);
    if (Array.isArray(obj?.riskItems)) out = out.concat(obj.riskItems);
    if (Array.isArray(obj?.components)) obj.components.forEach(c => { out = out.concat(allRiskItems(c)); });
    const seen = new Set();
    return out.filter(x => { const k = String(x?.issueId || x?.riskId || x?.id || x?.name || JSON.stringify(x)); if(seen.has(k)) return false; seen.add(k); return true; });
  }
  function mitigationEffect(obj){
    let list = [];
    if (Array.isArray(obj?.mitigations)) list = list.concat(obj.mitigations);
    if (Array.isArray(obj?.components)) obj.components.forEach(c => { if(Array.isArray(c?.mitigations)) list = list.concat(c.mitigations); });
    if (!list.length) return pct(obj?.control || obj?.controlEffectiveness, 0);
    const vals = list.map(m => pct(m?.effectiveness || m?.controlEffectiveness || m?.mitigationEffectiveness || m?.reductionPercent, 0.15));
    return Math.min(0.85, avg(vals));
  }
  function insuranceOffset(obj, residual){
    let list = [];
    if (Array.isArray(obj?.insurance)) list = list.concat(obj.insurance);
    if (Array.isArray(obj?.components)) obj.components.forEach(c => { if(Array.isArray(c?.insurance)) list = list.concat(c.insurance); });
    const coverage = list.reduce((s,i)=>s+num(i?.coverageAmount || i?.coverage || i?.limit || i?.policyLimit),0);
    const deductible = list.reduce((s,i)=>s+num(i?.deductible || i?.retention),0);
    return Math.max(0, Math.min(residual, coverage - deductible));
  }
  function deterministicRisk(obj){
    const items = allRiskItems(obj);
    let inherent = items.reduce((sum,it) => sum + probabilityLikely(it) * impactMostLikely(it), 0);
    if (!items.length) {
      const hardLikely = num(obj?.hardCostLikely || obj?.expectedLoss || obj?.residualExpectedLoss || 0);
      const softMult = num(obj?.softCostLikely || 0);
      inherent = hardLikely > 0 ? hardLikely + (hardLikely * softMult) : num(obj?.residual || obj?.inherent || 0) * 1000;
    }
    const mit = mitigationEffect(obj);
    const residualBeforeInsurance = inherent * (1 - mit);
    const insuranceImpact = insuranceOffset(obj, residualBeforeInsurance);
    const residual = Math.max(0, residualBeforeInsurance - insuranceImpact);
    const top = items.slice().sort((a,b)=>(probabilityLikely(b)*impactMostLikely(b))-(probabilityLikely(a)*impactMostLikely(a)))[0];
    return {
      inherent, residual, expectedLoss: inherent, residualExpectedLoss: residual,
      deltaRisk: Math.max(0, inherent - residual), insuranceImpact, mitigationEffect: mit,
      riskItemCount: items.length,
      highestDriver: top ? (top.name || top.title || top.issue || top.description || 'Highest expected-loss risk item') : 'No risk items saved',
      status: riskStatus(residual)
    };
  }
  function simulateObject(obj, iterations=DEFAULT_ITERATIONS){
    const items = allRiskItems(obj);
    const mit = mitigationEffect(obj);
    const rows = [];
    const losses = [];
    for (let i=0; i<iterations; i++) {
      ...
      if (items.length) {
        items.forEach(it => {
          const pr = probabilityRange(it); const ir = impactRange(it);
          inherent += triangular(pr.min, pr.likely, pr.max) * triangular(ir.min, ir.likely, ir.max);
        });
      } else {
        const likely = num(obj?.hardCostLikely || obj?.expectedLoss || obj?.residualExpectedLoss || 0) || num(obj?.residual || obj?.inherent || 0) * 1000;
        inherent = triangular(num(obj?.hardCostMin, likely*0.4), likely, num(obj?.hardCostMax, likely*2.5));
      }
      const residualBeforeInsurance = inherent * (1 - mit);
      const ins = insuranceOffset(obj, residualBeforeInsurance);
      const residual = Math.max(0, residualBeforeInsurance - ins);
      losses.push(residual);
      if (i < 1000) rows.push({ iteration:i+1, inherentExpectedLoss:Math.round(inherent), mitigationEffectiveness:Math.round(mit*100)+'%', insuranceOffset:Math.round(ins), residualExpectedLoss:Math.round(residual) });
    }
    losses.sort((a,b)=>a-b);
    return { iterations, mean:Math.round(avg(losses)), p50:Math.round(percentile(losses,.5)), p95:Math.round(percentile(losses,.95)), p99:Math.round(percentile(losses,.99)), min:Math.round(losses[0]||0), max:Math.round(losses[losses.length-1]||0), outcomeRows:rows };
  }

  window.calculateComplexComponentRollup = function(component){ return deterministicRisk(component || {}); };
  window.calculateAverageComponentRisk47 = function(){
    const comps = activeComponents();
    const rows = comps.map(c => ({ component:c, metrics:deterministicRisk(c) }));
    const avgResidual = rows.length ? Math.round(avg(rows.map(r => r.metrics.residual))) : 0;
    const avgInherent = rows.length ? Math.round(avg(rows.map(r => r.metrics.inherent))) : 0;
    const highest = rows.slice().sort((a,b)=>b.metrics.residual-a.metrics.residual)[0] || null;
    return { areaName:areaName(), rows, avgResidual, avgInherent, highest, status:riskStatus(avgResidual) };
  };
  function updateRollupField(metrics){
    const field = document.getElementById('complexSectionWeight');
    if (!field) return;
    if (!metrics.rows.length) field.value = 'Calculated after components';
    else field.value = `${fmtMoney(metrics.avgResidual)} avg component residual risk | ${metrics.rows.length} component${metrics.rows.length===1?'':'s'} | ${metrics.status}`;
  }
  window.renderComplexProductSections = function(){
    const tbody = document.getElementById('complexProductSectionsBody');
    const status = document.getElementById('complexProductSectionStatus');
    if (!tbody) return;
    const metrics = window.calculateAverageComponentRisk47();
    updateRollupField(metrics);
    let sections = [];
    try { sections = Array.isArray(complexProductSections) ? complexProductSections : []; } catch(e) {}
    if (!sections.length) {
      tbody.innerHTML = '<tr><td colspan="9">No area/product family added yet. Define the parent area first, then add components underneath it.</td></tr>';
      if (status) { status.textContent = 'Define the Area / Product Family before assigning component risk items.'; status.classList.remove('hidden'); }
      try { refreshComplexProductSectionSelects(); } catch(e) {}
    ...
    if (!metrics.rows.length) {
      const sid = esc(sections[0]?.sectionId || '');
      tbody.innerHTML = `<tr><td colspan="9"><strong>${esc(metrics.areaName)}</strong><br><small>No saved components yet. Complete the Selected Component Workspace and click Save Area Component to build the rollup.</small><br><div class="rollup-actions"><button class="btn btn-secondary small-btn" type="button" data-delete-product-section="${sid}">Delete Area</button></div></td></tr>`;
      if (status) { status.textContent = 'Area is ready. Add the first component below.'; status.classList.remove('hidden'); }
      try { refreshComplexProductSectionSelects(); } catch(e) {}
      return;
    }
    tbody.innerHTML = metrics.rows.map(row => {
      const c=row.component, m=row.metrics, id=esc(c.componentId || '');
      return `<tr class="clickable-rollup-row" data-open-complex-component="${id}" title="Open component workspace">
        <td><strong>${esc(componentLabel(c))}</strong><br><small>${id}</small></td>
        <td>${m.riskItemCount}</td>
        <td>${fmtMoney(m.inherent)}</td>
        <td><strong>${fmtMoney(m.residual)}</strong></td>
        <td>${fmtMoney(m.deltaRisk)}</td>
        <td>${fmtMoney(m.insuranceImpact)}</td>
        <td>${esc(m.highestDriver)}</td>
        <td>${esc(c.scenarioStatus || m.status)}</td>
        <td><div class="rollup-actions"><button class="btn btn-secondary small-btn" type="button" data-open-complex-component-button="${id}">Open</button><button class="btn btn-danger small-btn" type="button" data-delete-complex-component="${id}">Delete</button></div></td>
      </tr>`;
    }).join('');
    tbody.querySelectorAll('tr[data-open-complex-component]').forEach(row => row.addEventListener('click', e => { if (e.target.closest('button,a,input,select,textarea')) return; openComplexScenarioComponent(row.dataset.openComplexComponent); }));
    tbody.querySelectorAll('[data-open-complex-component-button]').forEach(btn => btn.addEventListener('click', e => { e.stopPropagation(); openComplexScenarioComponent(btn.dataset.openComplexComponentButton); }));
    if (status) { status.textContent = `Average component residual risk for ${metrics.areaName}: ${fmtMoney(metrics.avgResidual)}. Highest component: ${metrics.highest ? componentLabel(metrics.highest.component) : 'None'}.`; status.classList.remove('hidden'); }
    try { refreshComplexProductSectionSelects(); } catch(e) {}
  };

  function scenarioMetrics(s){
    let base = deterministicRisk(s || {});
    let componentRows = [];
    if (s?.mode === 'complex' && Array.isArray(s.components) && s.components.length) {
      componentRows = s.components.map(c => ({ name:componentLabel(c), metrics:deterministicRisk(c) }));
      base = {
        inherent: Math.round(avg(componentRows.map(r=>r.metrics.inherent))),
        residual: Math.round(avg(componentRows.map(r=>r.metrics.residual))),
        expectedLoss: Math.round(avg(componentRows.map(r=>r.metrics.expectedLoss))),
        residualExpectedLoss: Math.round(avg(componentRows.map(r=>r.metrics.residualExpectedLoss))),
        deltaRisk: Math.round(avg(componentRows.map(r=>r.metrics.deltaRisk))),
        insuranceImpact: Math.round(avg(componentRows.map(r=>r.metrics.insuranceImpact))),
        mitigationEffect: avg(componentRows.map(r=>r.metrics.mitigationEffect)),
        riskItemCount: componentRows.reduce((sum,r)=>sum+r.metrics.riskItemCount,0),
        highestDriver: (componentRows.slice().sort((a,b)=>b.metrics.residual-a.metrics.residual)[0]?.metrics.highestDriver) || 'No risk items saved',
        status: riskStatus(Math.round(avg(componentRows.map(r=>r.metrics.residual))))
      };
    }
    const mc = simulateObject(s || {}, num(s?.randomScenarioCount, DEFAULT_ITERATIONS) || DEFAULT_ITERATIONS);
    return { ...base, mc, componentRows };
  }
  function getSaved(){ try { return typeof getSavedScenarios === 'function' ? (getSavedScenarios() || []) : []; } catch(e){ return []; } }
  function buildReview(){
    const rows = getSaved().filter(s => ['single','complex'].includes(String(s?.mode || ''))).map(s => ({ scenario:s, metrics:scenarioMetrics(s) }));
    rows.sort((a,b)=>b.metrics.residual-a.metrics.residual);
    return rows.map((r,i)=>({ rank:i+1, ...r }));
  }
  function drawBarChart(canvas, rows){
    if (!canvas || !canvas.getContext) return;
    const ctx = canvas.getContext('2d'); const w=canvas.width=canvas.clientWidth||900; const h=canvas.height=300;
    ctx.clearRect(0,0,w,h); ctx.font='12px Arial'; ctx.fillText('Scenario Risk Ranking - Average Residual Expected Loss', 12, 18);
    const max = Math.max(1, ...rows.map(r=>r.metrics.residual)); const barH = Math.max(18, Math.min(34, (h-50)/Math.max(1,rows.length)));
    rows.slice(0,10).forEach((r,i)=>{ const y=35+i*barH; const bw=(w-260)*(r.metrics.residual/max); ctx.fillRect(220,y,bw,barH-6); ctx.fillText((r.rank)+'. '+scenarioLabel(r.scenario).slice(0,28),12,y+12); ctx.fillText(fmtMoney(r.metrics.residual),225+bw,y+12); });
  }
  function renderScenarioReviewReport47(){
    const host = document.getElementById('scenarioReviewReport47Body'); if (!host) return;
    const rows = buildReview(); window.__rt47ScenarioReviewRows = rows;
    if (!rows.length) { host.innerHTML = '<p class="muted">No saved Single or Complex scenarios found. Save scenarios first, then generate the review.</p>'; return; }
    host.innerHTML = `<canvas id="scenarioRiskRankingChart47" style="width:100%;height:300px;border:1px solid #dbe4f2;border-radius:12px;background:#fff;margin-bottom:14px;"></canvas>
      <div class="table-wrap"><table><thead><tr><th>Rank</th><th>Scenario</th><th>Type</th><th>Average Residual Risk</th><th>P95</th><th>P99</th><th>Time Horizon Exposure</th><th>Top Driver</th><th>Management Concern</th><th>Actions</th></tr></thead><tbody>${rows.map(r=>{
        const m=r.metrics, s=r.scenario; const horiz = `${fmtMoney(m.residual)} / yr; ${fmtMoney(m.residual*3)} / 3 yr; ${fmtMoney(m.residual*5)} / 5 yr`;
        const concern = `${scenarioLabel(s)} ranks #${r.rank} based on average residual expected loss. Management should validate probability, impact range, mitigation effectiveness, and insurance-transfer assumptions for ${m.highestDriver}.`;
        return `<tr><td>${r.rank}</td><td><strong>${esc(scenarioLabel(s))}</strong><br><small>${esc(s.id||'')}</small></td><td>${esc(s.mode)}</td><td><strong>${fmtMoney(m.residual)}</strong></td><td>${fmtMoney(m.mc.p95)}</td><td>${fmtMoney(m.mc.p99)}</td><td>${esc(horiz)}</td><td>${esc(m.highestDriver)}</td><td>${esc(concern)}</td><td><button class="btn btn-secondary small-btn" type="button" data-rt47-outcomes="${esc(s.id||'')}">View Outcomes</button><button class="btn btn-secondary small-btn" type="button" data-rt47-csv="${esc(s.id||'')}">CSV</button></td></tr>`;
      }).join('')}</tbody></table></div><div id="scenarioOutcomePreview47" class="note-box" style="margin-top:12px;">Monte Carlo outcomes have been generated automatically. Select View Outcomes to preview rows for a scenario.</div>`;
    drawBarChart(document.getElementById('scenarioRiskRankingChart47'), rows);
  }
  function ensureReportUi(){
    const reportsView = document.getElementById('view-reports'); if (!reportsView || document.getElementById('scenarioReviewReport47')) return;
    const card = document.createElement('div'); card.className='card'; card.id='scenarioReviewReport47';
    card.innerHTML = `<div class="card-header"><h3>Scenario Review + Hot Product Ranking</h3><span>Phase ${PHASE}</span></div>
      <p class="muted">Reviews every saved Single and Complex scenario, ranks them by average residual expected loss, auto-runs Monte Carlo, and provides outcome tables for auditor/examiner verification.</p>
      <div class="builder-actions"><button class="btn btn-primary" type="button" id="generateScenarioReview47">Generate / Refresh Scenario Review</button><button class="btn btn-secondary" type="button" id="saveScenarioReview47">Save Report to Workspace</button></div>
      <div id="scenarioReviewReport47Body"><p class="muted">Generate the report after saving scenarios.</p></div>`;
    reportsView.insertBefore(card, reportsView.firstChild);
    document.getElementById('generateScenarioReview47')?.addEventListener('click', renderScenarioReviewReport47);
    document.getElementById('saveScenarioReview47')?.addEventListener('click', saveReport);
  }
  function findReportRow(id){ return (window.__rt47ScenarioReviewRows || buildReview()).find(r=>String(r.scenario?.id||'')===String(id||'')); }
  function outcomesCsv(row){
    const headers = ['iteration','inherentExpectedLoss','mitigationEffectiveness','insuranceOffset','residualExpectedLoss'];
    return [headers.join(',')].concat((row?.metrics?.mc?.outcomeRows || []).map(o => headers.map(h => JSON.stringify(o[h] ?? '')).join(','))).join('\n');
  }
  ...
  function previewOutcomes(id){
    const row = findReportRow(id); const box=document.getElementById('scenarioOutcomePreview47'); if(!box || !row) return;
    const rows = row.metrics.mc.outcomeRows.slice(0,25);
    box.innerHTML = `<strong>${esc(scenarioLabel(row.scenario))} Monte Carlo Outcome Preview</strong><br><small>Showing first 25 of ${row.metrics.mc.iterations} rows. Mean ${fmtMoney(row.metrics.mc.mean)}, P95 ${fmtMoney(row.metrics.mc.p95)}, P99 ${fmtMoney(row.metrics.mc.p99)}.</small><div class="table-wrap"><table><thead><tr><th>Iteration</th><th>Inherent</th><th>Mitigation</th><th>Insurance</th><th>Residual</th></tr></thead><tbody>${rows.map(o=>`<tr><td>${o.iteration}</td><td>${fmtMoney(o.inherentExpectedLoss)}</td><td>${esc(o.mitigationEffectiveness)}</td><td>${fmtMoney(o.insuranceOffset)}</td><td><strong>${fmtMoney(o.residualExpectedLoss)}</strong></td></tr>`).join('')}</tbody></table></div>`;
  }
  async function saveReport(){
    const rows = buildReview();
    const text = 'RiskTool Scenario Review + Hot Product Ranking\nGenerated: '+new Date().toISOString()+'\n\n'+rows.map(r=>`${r.rank}. ${scenarioLabel(r.scenario)} (${r.scenario.mode}) - Avg Residual Risk ${fmtMoney(r.metrics.residual)}, P95 ${fmtMoney(r.metrics.mc.p95)}, P99 ${fmtMoney(r.metrics.mc.p99)}. Driver: ${r.metrics.highestDriver}.`).join('\n');
    try {
      if (typeof workspaceFolderHandle !== 'undefined' && workspaceFolderHandle && typeof getOrCreateDirectory === 'function' && typeof writeJsonFile === 'function') {
        const reportsDir = await getOrCreateDirectory(workspaceFolderHandle, 'reports');
        await writeJsonFile(reportsDir, 'scenario-review-hot-products-'+Date.now()+'.json', { generatedAt:new Date().toISOString(), rows: rows.map(r=>({ rank:r.rank, id:r.scenario.id, name:scenarioLabel(r.scenario), mode:r.scenario.mode, averageResidualRisk:r.metrics.residual, p95:r.metrics.mc.p95, p99:r.metrics.mc.p99, topDriver:r.metrics.highestDriver })) });
        download('scenario-review-hot-products.txt', text);
        alert('Report JSON saved to workspace reports folder. A TXT copy was also downloaded.');
        return;
      }
  ...
  document.addEventListener('click', function(e){
    const out = e.target?.closest?.('[data-rt47-outcomes]'); if(out){ previewOutcomes(out.dataset.rt47Outcomes); return; }
    const csv = e.target?.closest?.('[data-rt47-csv]'); if(csv){ const row=findReportRow(csv.dataset.rt47Csv); if(row) download('monte_carlo_outcomes_'+(row.scenario.id||Date.now())+'.csv', outcomesCsv(row), 'text/csv;charset=utf-8'); }
  });
  function setVersions(){
    try { window.RISKTOOL_RUNTIME_VERSION = PHASE; } catch(e) {}
    ['appVersion','versionDisplay','footerVersion','phaseVersion','dashboardVersion'].forEach(id=>{ const el=document.getElementById(id); if(el) el.textContent=PHASE; });
    document.querySelectorAll('[data-app-version]').forEach(el=>el.textContent=PHASE);
    document.querySelectorAll('.sidebar-note h4').forEach(el=>{ if((el.textContent||'').includes('Phase')) el.textContent='Phase '+PHASE; });
  }
  document.addEventListener('DOMContentLoaded', function(){ setVersions(); ensureReportUi(); try { window.renderComplexProductSections(); } catch(e){} });
  setTimeout(function(){ setVersions(); ensureReportUi(); try { window.renderComplexProductSections(); } catch(e){} }, 700);
})();
/* ===== END PHASE 23.0.47 ===== */
