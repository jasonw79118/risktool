...
function renderComplexProductSections() {
  ...
  if (!complexProductSections.length) {
    tbody.innerHTML = '<tr><td colspan="5">No area/product family added yet.</td></tr>';
  ...
  tbody.innerHTML = complexProductSections.map(section =>     '<tr>' +
    ...
    '<td>' + escapeHtml(section.calculationNote || "Calculated from component risk items, evidence, mitigation, insurance, and exposure estimates.") + '</td>' +
  ...
  if (status) status.textContent = complexProductSections.length + " area/product family section(s) defined. Area rollup weights are calculated from linked risk items, hard facts/evidence, mitigation strength, insurance coverage, and scenario exposure estimates.";
  refreshComplexProductSectionSelects();
}
...
/* ===== END PHASE 23.0.39 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.39 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.39 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.39 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.39 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.39 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.39 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.39 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.39 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.39 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.39";
...
/* ===== PHASE 23.0.39 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.39";
...
/* ===== PHASE 23.0.39 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.39";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.39 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.39 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.39";
...
/* ===== PHASE 23.0.39 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.39";
