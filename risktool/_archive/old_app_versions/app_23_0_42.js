...
function renderComplexProductSections() {
  ...
  if (!complexProductSections.length) {
    tbody.innerHTML = '<tr><td colspan="8">No area/product family added yet. Define the parent area first, then add components underneath it.</td></tr>';
  ...
  tbody.innerHTML = rows.map(row => '<tr class="clickable-rollup-row" data-open-complex-component="' + escapeHtml(row.component.componentId || "") + '" title="Open component workspace">' +
    ...
    '<td>' + escapeHtml(row.statusText) + '<br><button class="scenario-link" type="button" data-open-complex-component-button="' + escapeHtml(row.component.componentId || "") + '">Open</button></td>' +
    '</tr>').join("");
...
function renderHardFactsTable(targetId, items) {
  ...
  if (!items.length) { tbody.innerHTML = '<tr><td colspan="8">No hard facts / evidence entries added yet.</td></tr>'; return; }
...
/* ===== END PHASE 23.0.42 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.42 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.42 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.42 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.42 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.42 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.42 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.42 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.42 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.42 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.42";
...
/* ===== PHASE 23.0.42 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.42";
...
/* ===== PHASE 23.0.42 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.42";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.42 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.42 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.42";
...
/* ===== PHASE 23.0.42 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.42";
