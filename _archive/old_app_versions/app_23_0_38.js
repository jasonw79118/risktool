...
function calculateProductSectionWeight(section) {
  ...
  const linkedItems = currentComplexItems.filter(item => String(item.product || "") === productName);
  if (!linkedItems.length) {
    return { weight: 1, rawScore: 0, note: "Calculated 1/5: no linked risk items yet. Add risk items to raise or validate this section's exposure." };
  }
  let weightedScore = 0;
  ...
  const evidenceCount = Array.isArray(complexHardFacts) ? complexHardFacts.length : 0;
  const evidenceSourced = (complexHardFacts || []).filter(f => String(f.source || f.attachment || f.notes || "").trim()).length;
  ...
  const mitigationCredits = (complexMitigations || []).map(m => statusCreditValue(m.status));
  ...
  const coverageAmount = totalCurrencyField(complexInsurance, "coverageAmount");
  const deductibleAmount = totalCurrencyField(complexInsurance, "deductible");
  ...
  const note = 'Calculated ' + calculatedWeight + '/5 from ' + linkedItems.length + ' linked risk item(s), base score ' + Math.round(baseScore) + ', evidence count ' + evidenceCount + ', mitigation credit ' + Math.round(mitigationCredit * 100) + '%, and insurance offset ' + Math.round(insuranceCredit * 100) + '%.';
...
function renderComplexProductSections() {
  ...
  tbody.innerHTML = complexProductSections.map(section =>     '<tr>' +
    '<td>' + escapeHtml(section.productServiceArea) + '</td>' +
    '<td><strong>' + Number(section.calculatedWeight || section.weight || 1) + ' / 5</strong><br><small>Risk score ' + Number(section.calculatedRiskScore || 0) + ' / 100</small></td>' +
    '<td>' + escapeHtml(section.calculationNote || "Calculated from linked risk items and scenario evidence.") + '</td>' +
  ...
  if (status) status.textContent = complexProductSections.length + " product section(s) defined. Product weights are calculated from linked risk items, hard facts/evidence, mitigation strength, insurance coverage, and scenario exposure estimates.";
  refreshComplexProductSectionSelects();
}
function addComplexProductSection() {
  ...
  complexProductSections.push({
    ...
    calculationNote: "Calculated after linked risk items are added.",
    notes
  });
...
function deleteComplexProductSection(sectionId) {
  ...
  if (linkedItems.length) {
    const confirmed = window.confirm(`This will delete the Product/Service/Area "${section.productServiceArea}" and ${linkedItems.length} associated risk item(s). Continue?`);
    if (!confirmed) return;
    currentComplexItems = currentComplexItems.filter(item => String(item.product || "") !== String(section.productServiceArea || ""));
  } else {
    const confirmed = window.confirm(`Delete Product/Service/Area "${section.productServiceArea}"?`);
    if (!confirmed) return;
  }
...
/* ===== END PHASE 23.0.38 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.38 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.38 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.38 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.38 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.38 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.38 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.38 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.38 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.38 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.38";
...
/* ===== PHASE 23.0.38 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.38";
...
/* ===== PHASE 23.0.38 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.38";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.38 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.38 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.38";
...
/* ===== PHASE 23.0.38 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.38";
