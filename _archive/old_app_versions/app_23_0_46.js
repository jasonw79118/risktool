...
/* ===== PHASE 23.0.46 SCENARIO ENGINE + MONTE CARLO PORTFOLIO REVIEW ===== */
(function(){
  const PHASE = "23.0.46";
  const SCENARIO_REVIEW_KEY = "risktool_last_scenario_review_23_0_46";
  const fmtMoney = (v) => { try { return new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(Number(v)||0); } catch(e){ return '$' + Math.round(Number(v)||0).toLocaleString(); } };
  const esc = (v) => (typeof escapeHtml === 'function') ? escapeHtml(v) : String(v ?? '').replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const num = (v) => { const n = Number(String(v ?? '').replace(/[^0-9.-]/g,'')); return Number.isFinite(n) ? n : 0; };
  const clamp = (v,min,max) => Math.max(min, Math.min(max, Number(v)||0));
  const pctile = (arr,p) => { if(!arr.length) return 0; const a=arr.slice().sort((x,y)=>x-y); return a[Math.min(a.length-1, Math.max(0, Math.floor((a.length-1)*p)))]; };
  function allowedIterations(v){ const allowed=[100,500,1000,10000,100000]; const n=Number(v||1000); return allowed.includes(n) ? n : 1000; }
  function tri(min, mode, max){ min=num(min); mode=num(mode); max=num(max); if(max < min) max=min; if(mode < min) mode=min; if(mode > max) mode=max; if(max===min) return min; const u=Math.random(); const c=(mode-min)/(max-min); return u<c ? min + Math.sqrt(u*(max-min)*(mode-min)) : max - Math.sqrt((1-u)*(max-min)*(max-mode)); }
  function normalizeCostRange(min, likely, max, fallback){ let lo=num(min), mid=num(likely), hi=num(max); if(!lo && !mid && !hi && fallback) { lo=fallback.min; mid=fallback.likely; hi=fallback.max; } if(mid<=0) mid=lo || hi || 50000; if(lo<=0) lo=Math.max(0, mid*0.5); if(hi<=0) hi=Math.max(mid, mid*1.75); if(mid<lo) mid=lo; if(hi<mid) hi=mid; return {min:lo, likely:mid, max:hi}; }
  function likelihoodToProbRange(likelihood){ const base = clamp(likelihood,0,10) / 10; const mid = base > 0 ? base : 0.25; return { min: clamp(mid*0.55,0.01,0.95), likely: clamp(mid,0.01,0.98), max: clamp(Math.max(mid*1.45, mid+0.08),0.02,0.99) }; }
  function riskItemDriver(component){ const items = Array.isArray(component?.items) ? component.items : Array.isArray(component?.riskItems) ? component.riskItems : [];
    if(!items.length) return component?.riskDomain || component?.primaryRegulation || component?.primaryProduct || "Scenario exposure";
    const top = items.slice().sort((a,b)=>(num(b.score)*Math.max(1,num(b.weight||1)))-(num(a.score)*Math.max(1,num(a.weight||1))))[0];
    return top?.name || top?.issue || top?.title || top?.riskTitle || "Highest weighted risk item";
  }
  function insuranceStats(list){ const arr=Array.isArray(list)?list:[]; const premium=arr.reduce((s,x)=>s+num(x.premium),0); const deductible=arr.reduce((s,x)=>s+num(x.deductible),0); const coverage=arr.reduce((s,x)=>s+num(x.coverageAmount || x.coverage || x.limit),0); return { premium, deductible, coverage, usable: Math.max(0, coverage - deductible - premium) }; }
  function hardFactBenchmark(list){ const arr=Array.isArray(list)?list:[]; const total=arr.reduce((s,x)=>s+num(x.amount || x.lossAmount || x.cost),0); return { total, count: arr.length };
  }
  function componentInputs(component, parent){
    const hard = normalizeCostRange(component?.hardCostMin ?? parent?.hardCostMin, component?.hardCostLikely ?? parent?.hardCostLikely, component?.hardCostMax ?? parent?.hardCostMax, null);
    const soft = normalizeCostRange(component?.softCostMin ?? parent?.softCostMin ?? 0, component?.softCostLikely ?? parent?.softCostLikely ?? 0, component?.softCostMax ?? parent?.softCostMax ?? 0, {min:0, likely:0.25, max:0.75});
    const evidence = hardFactBenchmark(component?.hardFacts);
    if(evidence.total > hard.likely){ hard.likely = evidence.total; hard.max = Math.max(hard.max, evidence.total*1.5); }
    const likelihood = num(component?.likelihood || parent?.likelihood || 0);
    const prob = likelihoodToProbRange(likelihood || Math.max(1, Math.min(10, num(component?.inherent || parent?.inherent || 25)/10)));
    const control = clamp(component?.control ?? parent?.control ?? 0,0,95)/100;
    const mitigationList = Array.isArray(component?.mitigations) ? component.mitigations : [];
    const statusCredit = mitigationList.length ? mitigationList.reduce((s,m)=>s + (String(m.status||'').toLowerCase().includes('complete') ? 0.12 : String(m.status||'').toLowerCase().includes('progress') ? 0.07 : 0.03),0)/mitigationList.length : 0;
    const mitigationEffect = clamp(control + statusCredit,0,0.90);
    return { hard, soft, prob, mitigationEffect, insurance: insuranceStats(component?.insurance), driver: riskItemDriver(component), itemCount: (Array.isArray(component?.items)?component.items.length:0) };
  }
  function defaultComponentFromScenario(s){ return { ...s, componentId: s.id || 'single', scenarioName: s.name, items: Array.isArray(s.items) ? s.items : [], insurance: Array.isArray(s.insurance) ? s.insurance : [], hardFacts: Array.isArray(s.hardFacts) ? s.hardFacts : [], mitigations: Array.isArray(s.mitigations) ? s.mitigations : [] }; }
  function scenarioComponents(s){ if(s?.mode === 'complex' && Array.isArray(s.components) && s.components.length) return s.components.map(c => ({...c, control: c.control ?? s.control, hardCostMin: c.hardCostMin ?? s.hardCostMin, hardCostLikely: c.hardCostLikely ?? s.hardCostLikely, hardCostMax: c.hardCostMax ?? s.hardCostMax, softCostMin: c.softCostMin ?? s.softCostMin, softCostLikely: c.softCostLikely ?? s.softCostLikely, softCostMax: c.softCostMax ?? s.softCostMax })); return [defaultComponentFromScenario(s||{})]; }
  function simulateComponent(component, parent, iterations){ const inp=componentInputs(component,parent); const rows=[]; let before=[], residual=[], net=[]; for(let i=0;i<iterations;i++){ const p=tri(inp.prob.min, inp.prob.likely, inp.prob.max); const hard=tri(inp.hard.min, inp.hard.likely, inp.hard.max); const softMult=tri(inp.soft.min, inp.soft.likely, inp.soft.max); const impact=hard + (hard*softMult); const expected=p*impact; const res=expected*(1-inp.mitigationEffect); const ins=Math.min(res, inp.insurance.usable); const final=Math.max(0,res-ins); before.push(expected); residual.push(res); net.push(final); rows.push({ probability:p, hardCost:hard, softCost:hard*softMult, expectedLoss:expected, residualBeforeInsurance:res, insuranceOffset:ins, netLoss:final }); }
    return { input:inp, rows, meanExpected: avg(before), meanResidualBeforeInsurance: avg(residual), meanNet: avg(net), p50:pctile(net,.50), p90:pctile(net,.90), p95:pctile(net,.95), p99:pctile(net,.99) };
  }
  function avg(a){ return a.reduce((s,x)=>s+x,0)/Math.max(1,a.length); }
  function runScenarioMonteCarlo46(s){ const iterations=allowedIterations(s?.randomScenarioCount || 1000); const comps=scenarioComponents(s); const compRuns=comps.map(c=>simulateComponent(c,s,iterations)); const outcomeRows=[]; const netTotals=[]; const grossTotals=[]; const residualTotals=[]; for(let i=0;i<iterations;i++){ let gross=0,res=0,netLoss=0,insurance=0; const componentLosses=[]; compRuns.forEach((cr,idx)=>{ const r=cr.rows[i]; gross+=r.expectedLoss; res+=r.residualBeforeInsurance; netLoss+=r.netLoss; insurance+=r.insuranceOffset; componentLosses.push({componentName: comps[idx]?.scenarioName || comps[idx]?.name || comps[idx]?.componentId || `Component ${idx+1}`, netLoss: Math.round(r.netLoss)}); }); grossTotals.push(gross); residualTotals.push(res); netTotals.push(netLoss); outcomeRows.push({ scenarioNumber:i+1, grossExpectedLoss:Math.round(gross), residualBeforeInsurance:Math.round(res), insuranceOffset:Math.round(insurance), netLoss:Math.round(netLoss), componentLosses }); }
    const highestRun=compRuns.map((cr,idx)=>({cr, comp: comps[idx], risk: cr.meanNet})).sort((a,b)=>b.risk-a.risk)[0]; const meanGross=avg(grossTotals), meanRes=avg(residualTotals), meanNet=avg(netTotals); const mitigationValue=Math.max(0, meanGross-meanRes); const insuranceValue=Math.max(0, meanRes-meanNet); return { iterations, expectedLoss:Math.round(meanGross), residualExpectedLoss:Math.round(meanNet), residualBeforeInsurance:Math.round(meanRes), p50:Math.round(pctile(netTotals,.50)), p90:Math.round(pctile(netTotals,.90)), p95:Math.round(pctile(netTotals,.95)), p99:Math.round(pctile(netTotals,.99)), rangeLow:Math.round(pctile(netTotals,.10)), rangeMedian:Math.round(pctile(netTotals,.50)), rangeHigh:Math.round(pctile(netTotals,.90)), riskReductionValue:Math.round(mitigationValue), insuranceImpact:Math.round(insuranceValue), componentRuns: compRuns.map((cr,idx)=>({ componentId: comps[idx]?.componentId||'', componentName: comps[idx]?.scenarioName || comps[idx]?.name || comps[idx]?.componentId || `Component ${idx+1}`, driver: cr.input.driver, itemCount: cr.input.itemCount, expectedLoss:Math.round(cr.meanExpected), residualBeforeInsurance:Math.round(cr.meanResidualBeforeInsurance), netLoss:Math.round(cr.meanNet), p95:Math.round(cr.p95), insuranceImpact:Math.round(cr.meanResidualBeforeInsurance-cr.meanNet), mitigationEffect:cr.input.mitigationEffect })), highestComponent: highestRun ? (highestRun.comp?.scenarioName || highestRun.comp?.name || highestRun.comp?.componentId || '') : '', topDriver: highestRun ? highestRun.cr.input.driver : (s?.riskDomain || 'Scenario exposure'), outcomeRows };
  }
  function horizonRowsFrom(mc){ return [1,3,5,10,15,20,25,30].map(y=>({horizonLabel:y===30?'30+ Years':`${y} Year${y>1?'s':''}`, years:y, withoutMitigation:Math.round(mc.expectedLoss*y), withMitigation:Math.round(mc.residualExpectedLoss*y), riskReduction:Math.round((mc.expectedLoss-mc.residualExpectedLoss)*y)})); }
  function tierFromDollars(v){ return v>=1000000?'Critical':v>=500000?'High':v>=100000?'Elevated':v>=25000?'Moderate':'Lower'; }
  function frequencyFromDollars(v){ return v>=500000?'Monthly':v>=100000?'Quarterly':v>=25000?'Semi-Annual':'Annual'; }
  function summarize46(payload){ const mc=runScenarioMonteCarlo46(payload); const tier=tierFromDollars(mc.residualExpectedLoss); const frequency=frequencyFromDollars(mc.residualExpectedLoss); const itemCount = payload?.mode === 'complex' ? scenarioComponents(payload).reduce((s,c)=>s+(Array.isArray(c.items)?c.items.length:0),0) : 1; const inherentIndex = Math.min(100, Math.round(Math.log10(Math.max(1,mc.expectedLoss))*14)); const residualIndex = Math.min(100, Math.round(Math.log10(Math.max(1,mc.residualExpectedLoss))*14)); const decisionText = mc.residualExpectedLoss >= 500000 ? 'Management should prioritize mitigation, evidence validation, and control owner accountability because residual expected loss remains high after modeled mitigation and insurance.' : mc.residualExpectedLoss >= 100000 ? 'Management should monitor this scenario closely and validate whether additional controls or risk transfer are justified by the modeled exposure.' : 'Current modeled exposure appears lower, but management should maintain evidence and control monitoring for trend changes.';
    return { ...payload, inherent: inherentIndex, residual: residualIndex, total: inherentIndex, tier, frequency, itemCount, expectedLoss:mc.expectedLoss, residualExpectedLoss:mc.residualExpectedLoss, residualBeforeInsurance:mc.residualBeforeInsurance, riskReductionValue:mc.riskReductionValue, insuranceImpact:mc.insuranceImpact, mitigationROI:mc.riskReductionValue - num(payload?.mitigationCost), rangeLow:mc.rangeLow, rangeMedian:mc.rangeMedian, rangeHigh:mc.rangeHigh, p50:mc.p50, p90:mc.p90, p95:mc.p95, p99:mc.p99, highestComponent:mc.highestComponent, topDriver:mc.topDriver, componentDecisionMetrics:mc.componentRuns, horizonRows:horizonRowsFrom(mc), randomScenarioCount:mc.iterations, randomOutcomeRows:mc.outcomeRows, monteCarloMethodRows:[["Method","Triangular Monte Carlo using probability and impact ranges"],["Iterations",mc.iterations],["Scenario Scope",payload?.mode==='complex'?"Complex scenario components summed by iteration":"Single scenario modeled directly"],["Probability Basis","Likelihood converted to min / most likely / max probability range; future phase will expose direct probability fields"],["Impact Basis","Hard-cost min / most likely / max plus soft-cost multiplier range"],["Insurance Treatment","Coverage offset is capped by simulated residual loss after deductible and premium"],["Audit Use","Outcome table preserves every simulated draw for examiner verification"]], monteCarloInputRows:[["Scenario",payload?.name||"Unnamed"],["Builder",payload?.mode||""],["Components Modeled",scenarioComponents(payload).length],["Time Horizons","1, 3, 5, 10, 15, 20, 25, 30+ years"],["Primary Driver",mc.topDriver]], monteCarloOutputRows:[["Expected Gross Annual Loss",fmtMoney(mc.expectedLoss)],["Residual Annual Loss After Mitigation / Insurance",fmtMoney(mc.residualExpectedLoss)],["P50 Net Loss",fmtMoney(mc.p50)],["P90 Net Loss",fmtMoney(mc.p90)],["P95 Net Loss",fmtMoney(mc.p95)],["P99 Net Loss",fmtMoney(mc.p99)],["Mitigation Value",fmtMoney(mc.riskReductionValue)],["Insurance Impact",fmtMoney(mc.insuranceImpact)]], decisionText, generatedSummary:`${payload?.name || 'This scenario'} is modeled at ${fmtMoney(mc.residualExpectedLoss)} residual annual expected loss, with P95 tail exposure of ${fmtMoney(mc.p95)} and P99 tail exposure of ${fmtMoney(mc.p99)}. Primary driver: ${mc.topDriver}. ${decisionText}` };
  }
  window.runScenarioMonteCarlo46 = runScenarioMonteCarlo46;
  window.summarizePayload = summarizePayload = summarize46;
  window.calculateComplexComponentRollup = function(component){ const fakeParent={mode:'complex', randomScenarioCount:500}; const mc=runScenarioMonteCarlo46({...fakeParent, components:[component]}); const inherentScore=Math.min(100,Math.round(Math.log10(Math.max(1,mc.expectedLoss))*14)); const residualScore=Math.min(100,Math.round(Math.log10(Math.max(1,mc.residualExpectedLoss))*14)); return { inherent: inherentScore, residual: residualScore, deltaRisk: Math.max(0,inherentScore-residualScore), insuranceImpact: fmtMoney(mc.insuranceImpact), highestDriver: mc.topDriver, expectedLoss:mc.expectedLoss, residualExpectedLoss:mc.residualExpectedLoss, p95:mc.p95, dollarStatus:tierFromDollars(mc.residualExpectedLoss) }; };
  function scenarioReviewRows(){ return (typeof getSavedScenarios==='function'?getSavedScenarios():[]).filter(s=>s && (s.mode==='single'||s.mode==='complex')).map(s=>{ const summary = (s.residualExpectedLoss && s.randomOutcomeRows?.length) ? s : summarize46(s); return { ...summary, scenarioType: summary.mode==='complex'?'Complex':'Single' }; }).sort((a,b)=>(num(b.residualExpectedLoss)-num(a.residualExpectedLoss)) || (num(b.p95)-num(a.p95))); }
  function groupHotProducts(rows){ const map=new Map(); rows.forEach(r=>{ const key=r.productGroup || r.primaryProduct || r.name || 'Unassigned'; if(!map.has(key)) map.set(key,{name:key, scenarios:[], total:0, p95:0}); const g=map.get(key); g.scenarios.push(r); g.total+=num(r.residualExpectedLoss); g.p95=Math.max(g.p95,num(r.p95)); }); return Array.from(map.values()).sort((a,b)=>b.total-a.total); }
  function ensureScenarioReviewUi(){ const reports=document.getElementById('view-reports'); if(!reports || document.getElementById('scenarioReview46Card')) return; const card=document.createElement('div'); card.className='card'; card.id='scenarioReview46Card'; card.innerHTML=`<div class="card-header"><h3>Scenario Review & Hot Product Ranking</h3><span>23.0.46 Monte Carlo portfolio review</span></div><p class="muted">Ranks every saved Single and Complex scenario by residual expected loss, P95 tail exposure, decision metrics, and time-horizon exposure. Product/area hot spots are aggregated from saved scenario storage.</p><div class="builder-actions"><button class="btn btn-primary" id="refreshScenarioReview46" type="button">Generate / Refresh Scenario Review</button><button class="btn btn-secondary" id="downloadScenarioReview46" type="button">Download Review TXT</button><button class="btn btn-secondary" id="saveScenarioReview46" type="button">Save Review to Workspace</button></div><div class="note-box" id="scenarioReview46Summary">Save Single or Complex scenarios, then generate the review.</div><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Scenario</th><th>Type</th><th>Area / Product Family</th><th>Residual Expected Loss</th><th>P95</th><th>P99</th><th>Time Horizon Concern</th><th>Top Driver</th><th>Actions</th></tr></thead><tbody id="scenarioReview46Body"><tr><td colspan="10">Generate review to rank saved scenarios.</td></tr></tbody></table></div><h4>Hot Products / Area Families</h4><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Area / Product Family</th><th>Scenarios</th><th>Total Residual Expected Loss</th><th>Highest P95</th><th>Management Focus</th></tr></thead><tbody id="hotProducts46Body"><tr><td colspan="6">Generate review to identify hot products.</td></tr></tbody></table></div><h4>Risk Ranking Graph</h4><canvas id="scenarioRankingChart46" width="860" height="320"></canvas><h4>Hot Product Graph</h4><canvas id="hotProductsChart46" width="860" height="260"></canvas><div id="scenarioOutcomePreview46" class="note-box">Select View Outcomes to preview Monte Carlo outcome rows for a scenario.</div>`; const anchor=document.getElementById('riskPriorityCard') || reports.querySelector('.card'); reports.insertBefore(card, anchor || null); document.getElementById('refreshScenarioReview46')?.addEventListener('click', renderScenarioReview46); document.getElementById('downloadScenarioReview46')?.addEventListener('click', () => downloadScenarioReview46(false)); document.getElementById('saveScenarioReview46')?.addEventListener('click', () => downloadScenarioReview46(true)); }
  function drawRankingChart(canvasId, rows, valueField, labelField){ const canvas=document.getElementById(canvasId); if(!canvas) return; const ctx=canvas.getContext('2d'); const w=canvas.width,h=canvas.height; ctx.clearRect(0,0,w,h); ctx.fillStyle='#172033'; ctx.font='12px Arial'; const top=rows.slice(0,10); const max=Math.max(1,...top.map(r=>num(r[valueField]))); const left=180, barH=18, gap=9; ctx.strokeStyle='#dfe6f1'; for(let i=0;i<5;i++){ const x=left+(w-left-30)*i/4; ctx.beginPath(); ctx.moveTo(x,20); ctx.lineTo(x,h-20); ctx.stroke(); } top.forEach((r,i)=>{ const y=32+i*(barH+gap); const bw=(num(r[valueField])/max)*(w-left-45); ctx.fillStyle='#173a8c'; ctx.fillRect(left,y,bw,barH); ctx.fillStyle='#172033'; const label=String(r[labelField]||r.name||'').slice(0,24); ctx.fillText(label,10,y+13); ctx.fillText(fmtMoney(r[valueField]),left+bw+6,y+13); }); }
  function renderScenarioReview46(){ ensureScenarioReviewUi(); const rows=scenarioReviewRows(); window.__rt46ScenarioReviewRows=rows; const tbody=document.getElementById('scenarioReview46Body'); const summary=document.getElementById('scenarioReview46Summary'); if(summary){ const top=rows[0]; summary.innerHTML=top ? `<strong>Highest-risk saved scenario:</strong> ${esc(top.name)} at ${fmtMoney(top.residualExpectedLoss)} residual expected loss and ${fmtMoney(top.p95)} P95 tail exposure. The ranking is based on Monte Carlo output for each saved Single and Complex scenario.` : 'No saved Single or Complex scenarios found.'; } if(tbody){ tbody.innerHTML = rows.length ? rows.map((r,i)=>`<tr><td>${i+1}</td><td><strong>${esc(r.name)}</strong><br><small>${esc(r.id)}</small></td><td>${esc(r.scenarioType)}</td><td>${esc(r.productGroup || r.primaryProduct || '')}</td><td><strong>${fmtMoney(r.residualExpectedLoss)}</strong></td><td>${fmtMoney(r.p95)}</td><td>${fmtMoney(r.p99)}</td><td>${fmtMoney((r.horizonRows||[]).find(x=>x.years===5)?.withMitigation || (num(r.residualExpectedLoss)*5))} / 5 years</td><td>${esc(r.topDriver || r.riskDomain || '')}</td><td><button class="btn btn-secondary small-btn" data-open-review46="${esc(r.id)}">Open</button> <button class="btn btn-secondary small-btn" data-outcomes46="${esc(r.id)}">View Outcomes</button> <button class="btn btn-secondary small-btn" data-download-outcomes46="${esc(r.id)}">CSV</button></td></tr>`).join('') : '<tr><td colspan="10">No saved Single or Complex scenarios found.</td></tr>'; }
    const hot=groupHotProducts(rows); const hotBody=document.getElementById('hotProducts46Body'); if(hotBody){ hotBody.innerHTML = hot.length ? hot.map((g,i)=>`<tr><td>${i+1}</td><td><strong>${esc(g.name)}</strong></td><td>${g.scenarios.length}</td><td>${fmtMoney(g.total)}</td><td>${fmtMoney(g.p95)}</td><td>${esc(g.name)} is hot because saved scenarios in this area total ${fmtMoney(g.total)} residual expected loss.</td></tr>`).join('') : '<tr><td colspan="6">No hot product data available.</td></tr>'; }
    drawRankingChart('scenarioRankingChart46', rows, 'residualExpectedLoss', 'name'); drawRankingChart('hotProductsChart46', hot, 'total', 'name'); try{ localStorage.setItem(SCENARIO_REVIEW_KEY, JSON.stringify({generatedAt:new Date().toISOString(), rows, hotProducts:hot})); }catch(e){} }
  function outcomeCsv(s){ const rows=(s?.randomOutcomeRows&&s.randomOutcomeRows.length)?s.randomOutcomeRows:runScenarioMonteCarlo46(s).outcomeRows; const header=['Iteration','Gross Expected Loss','Residual Before Insurance','Insurance Offset','Net Loss','Component Losses']; const body=rows.map(r=>[r.scenarioNumber,r.grossExpectedLoss,r.residualBeforeInsurance,r.insuranceOffset,r.netLoss,(r.componentLosses||[]).map(c=>`${c.componentName}:${c.netLoss}`).join('; ')]); const csv=[header].concat(body).map(row=>row.map(v=>{const x=String(v??''); return /[",\n]/.test(x)?`"${x.replaceAll('"','""')}"`:x;}).join(',')).join('\n'); return csv; }
  function findReviewScenario(id){ return scenarioReviewRows().find(r=>String(r.id)===String(id)); }
  document.addEventListener('click', function(e){ const open=e.target.closest('[data-open-review46]'); if(open){ e.preventDefault(); if(typeof openScenario==='function') openScenario(open.dataset.openReview46); return; } const view=e.target.closest('[data-outcomes46]'); if(view){ e.preventDefault(); const s=findReviewScenario(view.dataset.outcomes46); const host=document.getElementById('scenarioOutcomePreview46'); if(host&&s){ const rows=(s.randomOutcomeRows||[]).slice(0,10); host.innerHTML=`<strong>Outcome preview for ${esc(s.name)}</strong><br><small>Showing first 10 of ${s.randomOutcomeRows?.length || 0} simulated rows. Use CSV for full examiner table.</small><div class="table-wrap"><table><thead><tr><th>#</th><th>Gross</th><th>Residual Before Insurance</th><th>Insurance Offset</th><th>Net Loss</th></tr></thead><tbody>${rows.map(r=>`<tr><td>${r.scenarioNumber}</td><td>${fmtMoney(r.grossExpectedLoss)}</td><td>${fmtMoney(r.residualBeforeInsurance)}</td><td>${fmtMoney(r.insuranceOffset)}</td><td>${fmtMoney(r.netLoss)}</td></tr>`).join('')}</tbody></table></div>`; } return; } const dl=e.target.closest('[data-download-outcomes46]'); if(dl){ e.preventDefault(); const s=findReviewScenario(dl.dataset.downloadOutcomes46); if(s) triggerDownload(`scenario_outcomes_${(s.id||'scenario').replace(/[^a-zA-Z0-9_-]/g,'_')}.csv`, new Blob([outcomeCsv(s)],{type:'text/csv;charset=utf-8'})); } }, true);
  async function downloadScenarioReview46(saveWorkspace){ const rows=scenarioReviewRows(); const hot=groupHotProducts(rows); const text=['RiskTool Scenario Review and Hot Product Ranking','Generated: '+new Date().toISOString(),'','Management Summary', rows[0] ? `Highest risk scenario is ${rows[0].name} with residual expected loss ${fmtMoney(rows[0].residualExpectedLoss)} and P95 ${fmtMoney(rows[0].p95)}.` : 'No scenarios available.','','Scenario Ranking'].concat(rows.map((r,i)=>`${i+1}. ${r.name} (${r.scenarioType}) - ${fmtMoney(r.residualExpectedLoss)} residual expected loss; P95 ${fmtMoney(r.p95)}; Driver: ${r.topDriver}; Concern: ${r.decisionText}`)).concat(['','Hot Product / Area Ranking']).concat(hot.map((g,i)=>`${i+1}. ${g.name} - ${fmtMoney(g.total)} total residual expected loss across ${g.scenarios.length} scenario(s).`)).join('\n'); if(saveWorkspace){ try{ if(typeof workspaceFolderHandle!=='undefined' && workspaceFolderHandle){ const dir=await workspaceFolderHandle.getDirectoryHandle('reports',{create:true}); const stamp=new Date().toISOString().replace(/[:.]/g,'-'); const h=await dir.getFileHandle(`scenario-review-${stamp}.txt`,{create:true}); const w=await h.createWritable(); await w.write(text); await w.close(); const j=await dir.getFileHandle(`scenario-review-${stamp}.json`,{create:true}); const jw=await j.createWritable(); await jw.write(JSON.stringify({generatedAt:new Date().toISOString(), rows, hotProducts:hot},null,2)); await jw.close(); alert('Scenario review saved to workspace reports folder.'); return; } }catch(err){ console.error(err); alert('Workspace save failed. Downloading the review instead.'); } } triggerDownload(`scenario_review_hot_products_${Date.now()}.txt`, new Blob([text],{type:'text/plain'})); }
  const priorRenderSaved = typeof renderSavedScenarios === 'function' ? renderSavedScenarios : null; if(priorRenderSaved){ window.renderSavedScenarios = renderSavedScenarios = function(){ const result=priorRenderSaved.apply(this,arguments); try{ renderScenarioReview46(); }catch(e){} return result; }; }
  const priorOpenReport = typeof openScenarioReport === 'function' ? openScenarioReport : null; if(priorOpenReport){ window.openScenarioReport = openScenarioReport = function(id){ const result=priorOpenReport.apply(this,arguments); try{ renderScenarioReview46(); }catch(e){} return result; }; }
  document.addEventListener('DOMContentLoaded', function(){ ensureScenarioReviewUi(); renderScenarioReview46(); try{ document.querySelectorAll('.sidebar-note h4').forEach(el=>{ if((el.textContent||'').includes('Phase')) el.textContent='Phase '+PHASE; }); const v=document.getElementById('appVersion'); if(v) v.textContent=PHASE; const note=document.getElementById('workspacePhaseNote'); if(note) note.textContent=note.textContent.replace(/23\.0\.\d+/g, PHASE); }catch(e){} });
})();
/* ===== END PHASE 23.0.46 ===== */
