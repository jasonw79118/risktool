...
function calculateComplexComponentRollup(component) {
  ...
  const residual = Math.max(0, Math.min(100, Math.round(inherent * (1 - controlCredit) * (1 - mitigationCredit) * (1 - insuranceCredit))));
  ...
  return { inherent, residual, highestDriver };
}
function renderComplexProductSections() {
  ...
  if (!complexProductSections.length) {
    tbody.innerHTML = '<tr><td colspan="6">No area/product family added yet. Define the parent area first, then add components underneath it.</td></tr>';
  ...
  if (!complexScenarioComponents.length) {
    ...
    tbody.innerHTML = '<tr><td colspan="6"><strong>Area Rollup: ' + escapeHtml(areaName) + '</strong><br><small>No saved components yet. Add component/product-service rows such as Loan Statements, Loan Disclosures, or Loan Rates to build the rollup.</small><br><button class="btn btn-secondary" type="button" data-delete-product-section="' + escapeHtml(section.sectionId) + '">Delete Area</button></td></tr>';
  ...
  tbody.innerHTML = rows.map(row => '<tr>' +
    '<td><button class="scenario-link" data-open-complex-component="' + escapeHtml(row.component.componentId || "") + '">' + escapeHtml(row.componentName) + '</button><br><small>' + escapeHtml(row.component.componentId || "") + '</small></td>' +
...
function renderComplexScenarioComponents() {
  ...
  if (!tbody) return;
  if (!complexScenarioComponents.length) {
    tbody.innerHTML = '<tr><td colspan="7">No area components added yet.</td></tr>';
  ...
  tbody.innerHTML = complexScenarioComponents.map((component, idx) => `
    <tr data-component-id="${escapeHtml(component.componentId || "")}">
      ...
      <td>${escapeHtml(component.scenarioName || "Unnamed Area Component")}</td>
...
/* ===== END PHASE 23.0.40 SAVE OVERRIDE ===== */


/* ===== PHASE 23.0.40 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 23.0.40 WORKSPACE BRIDGE ===== */

/* ===== PHASE 23.0.40 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 23.0.40 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 23.0.40 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 23.0.40 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 23.0.40 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 23.0.40 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 23.0.40 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "23.0.40";
...
/* ===== PHASE 23.0.40 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "23.0.40";
...
/* ===== PHASE 23.0.40 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "23.0.40";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 23.0.40 session write failed', e); return null; }
  }
...
/* ===== PHASE 23.0.40 SIMPLE POST-LOGIN RESTORE SCREEN ===== */
(function(){
  const PHASE = "23.0.40";
...
/* ===== PHASE 23.0.40 SAVED SCENARIO DELETE HARDENING ===== */
(function(){
  const PHASE = "23.0.40";
