...
/* ===== PHASE 23.0.67 FINAL RUNTIME + EDITABILITY LOCK ===== */
(function(){
  const PHASE='23.0.67';
  const VALID=['dashboard','single','complex','beta','categories','users','saved','reports','information','portfolio-report','scenario-review'];
  const SESSION_KEY='risk_manager_session_user_v2101';
  const q=(s,r=document)=>r.querySelector(s);
  const qa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  function loggedIn(){try{return !!localStorage.getItem(SESSION_KEY);}catch(e){return false;}}
  function addStyles(){
    if(q('#rt67FinalStyles'))return;
    const st=document.createElement('style');st.id='rt67FinalStyles';
    st.textContent='#dashboardChartCard{display:none!important}#appVersion{z-index:7000!important}body.rt67-logged-in #loginGate{display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important}body.rt67-logged-in .modal-overlay:not(#loginGate){display:none!important;visibility:hidden!important;opacity:0!important;pointer-events:none!important}body.rt67-logged-in .main,body.rt67-logged-in .sidebar,body.rt67-logged-in .view.active,body.rt67-logged-in .card{pointer-events:auto!important;opacity:1!important;filter:none!important}body.rt67-logged-in .view.active{display:block!important}body.rt67-logged-in .view.active input:not([readonly]),body.rt67-logged-in .view.active select,body.rt67-logged-in .view.active textarea,body.rt67-logged-in .view.active button{pointer-events:auto!important;user-select:auto!important;opacity:1!important;filter:none!important}body.rt67-logged-in .view.active input:not([readonly]),body.rt67-logged-in .view.active select,body.rt67-logged-in .view.active textarea{background:#fff!important;color:#111827!important}.nav-item.active{background:#1177f2!important;color:#fff!important}.nav-item:not(.active){background:transparent!important}';
    document.head.appendChild(st);
  }
  function version(){try{window.RISKTOOL_RUNTIME_VERSION=PHASE;}catch(e){}qa('#appVersion,.app-version,[data-version-label]').forEach(el=>el.textContent=PHASE);qa('.sidebar-note h4').forEach(el=>{if(/^Phase\s+/i.test(el.textContent||''))el.textContent='Phase '+PHASE;});const note=q('#workspacePhaseNote');if(note)note.textContent=(note.textContent||'').replace(/23\.0\.\d+/g,PHASE);}
  function normalize(v){v=String(v||location.hash.replace(/^#/,'')||'dashboard').trim();if(!v||v==='manual-faq'||v.indexOf('manual-')===0||!VALID.includes(v))v='dashboard';return v;}
  function forceView(v){v=normalize(v);qa('.view').forEach(x=>{const on=x.id==='view-'+v;x.classList.toggle('active',on);x.style.display=on?'block':'none';});qa('.nav-item[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===v));if(location.hash!=='#'+v){try{history.replaceState(null,'',location.pathname+location.search+'#'+v);}catch(e){}}try{window.activeMode=v;}catch(e){}}
  function hideBlocks(){if(!loggedIn())return;document.body.classList.add('rt67-logged-in');const gate=q('#loginGate');if(gate){gate.classList.remove('rt66-login-open','rt65-login-open','rt64-login-open');gate.style.display='none';gate.style.visibility='hidden';gate.style.opacity='0';gate.style.pointerEvents='none';}qa('.modal-overlay').forEach(m=>{if(m.id==='loginGate')return;m.style.display='none';m.style.visibility='hidden';m.style.opacity='0';m.style.pointerEvents='none';});}
  function unlock(){if(!loggedIn())return;hideBlocks();qa('.main,.sidebar,.view.active,.card,.form-grid,.table-wrap').forEach(el=>{el.style.pointerEvents='auto';el.style.opacity='1';el.style.filter='none';});qa('#view-single input,#view-single select,#view-single textarea,#view-single button,#view-complex input,#view-complex select,#view-complex textarea,#view-complex button,#view-beta input,#view-beta select,#view-beta textarea,#view-beta button').forEach(el=>{if(el.hasAttribute('readonly'))return;el.disabled=false;el.removeAttribute('disabled');el.style.pointerEvents='auto';el.style.opacity='1';el.style.filter='none';if(/^(INPUT|SELECT|TEXTAREA)$/.test(el.tagName)){el.style.backgroundColor='#fff';el.style.color='#111827';}});}
  function loginGate(){const gate=q('#loginGate');if(!gate)return;if(loggedIn()){hideBlocks();return;}document.body.classList.remove('rt67-logged-in');gate.style.display='flex';gate.style.visibility='visible';gate.style.opacity='1';gate.style.pointerEvents='auto';}
  function nav(){qa('.nav-item[data-view]').forEach(btn=>{if(btn.dataset.rt67Bound)return;btn.dataset.rt67Bound='1';btn.addEventListener('click',ev=>{ev.preventDefault();ev.stopImmediatePropagation();forceView(btn.dataset.view);unlock();},true);});window.activateView=function(v){forceView(v);unlock();};try{activateView=window.activateView;}catch(e){}}
  const oldStart=window.startUserSession;window.startUserSession=function(){let r;try{r=typeof oldStart==='function'?oldStart.apply(this,arguments):undefined;}catch(e){console.error(e);}setTimeout(()=>{version();loginGate();forceView(normalize());unlock();},80);setTimeout(()=>{hideBlocks();unlock();},350);return r;};
  function boot(){addStyles();version();nav();loginGate();forceView(normalize());unlock();try{if(typeof refreshLibraries==='function')refreshLibraries();}catch(e){}try{if(typeof renderDashboardOpenTable==='function')renderDashboardOpenTable();}catch(e){}try{if(typeof renderSavedScenarios==='function')renderSavedScenarios();}catch(e){}}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',()=>{setTimeout(boot,50);setTimeout(boot,500);setTimeout(boot,1500);});else{setTimeout(boot,50);setTimeout(boot,500);setTimeout(boot,1500);}
  window.addEventListener('hashchange',()=>setTimeout(()=>{forceView(normalize());unlock();},20));
  document.addEventListener('click',()=>setTimeout(()=>{version();loginGate();unlock();},30),true);
  document.addEventListener('focusin',()=>setTimeout(unlock,5),true);
  setInterval(()=>{version();loginGate();unlock();},750);
})();
/* ===== END PHASE 23.0.67 FINAL RUNTIME + EDITABILITY LOCK ===== */
