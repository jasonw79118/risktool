...
(function(){
'use strict';
const VERSION='24.0.00', $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
/* ===== PHASE 24.0.00 PORTFOLIO TOTALS INTEGRITY OVERRIDE ===== */
(function(){
  'use strict';
  const VERSION='24.0.00';
  const $=(s,r=document)=>r.querySelector(s);
  const $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
  const num=v=>{ if(v==null) return 0; if(typeof v==='number') return isFinite(v)?v:0; const n=Number(String(v).replace(/[$,%\s,]/g,'')); return isFinite(n)?n:0; };
  const money=v=>num(v).toLocaleString(undefined,{style:'currency',currency:'USD',maximumFractionDigits:0});
  const cell=v=>{const s=String(v??''); return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s;};
  function setVersion(){
    try{const e=$('#appVersion'); if(e)e.textContent=VERSION;}catch(e){}
    try{document.querySelectorAll('[id*=Version],[class*=version]').forEach(n=>{ if(/23\.0\.\d+|24\.0\.00/.test(n.textContent||'')) n.textContent=(n.textContent||'').replace(/23\.0\.\d+/g,VERSION); });}catch(e){}
  }
  function saved(){
    try{ if(typeof getSavedScenarios==='function') return getSavedScenarios()||[]; }catch(e){}
    try{ const raw=localStorage.getItem('risktool.savedScenarios')||localStorage.getItem('savedScenarios')||localStorage.getItem('risktool.scenarios'); return raw?JSON.parse(raw):[]; }catch(e){ return []; }
  }
  function summary(raw){ try{ if(typeof summarizePayload==='function') return summarizePayload({...raw}); }catch(e){} return raw||{}; }
  function typeOf(s){ return String(s?.mode||s?.scenarioType||s?.type||s?.builder||'single').toLowerCase(); }
  function insurance(raw,sm){
    const lists=[raw?.insurance, raw?.insurancePolicies, raw?.riskTransfer, sm?.insurance, sm?.insurancePolicies].filter(Array.isArray);
    let total=0; lists.forEach(list=>list.forEach(x=>{ total+=num(x.coverageAmount||x.coverage||x.limit||x.amount||x.policyLimit||x.adjustedImpact||x.transferAmount); }));
    return total || num(sm?.insuranceImpact||raw?.insuranceImpact||raw?.coverageAmount||raw?.insuranceCoverage);
  }
  function components(raw){
    const candidates=[raw?.components, raw?.componentRows, raw?.productComponents, raw?.complexComponents, raw?.componentWorkspaces].filter(Array.isArray);
    return candidates[0]||[];
  }
  function costs(raw){
    const candidates=[raw?.costLosses, raw?.costsLosses, raw?.costs, raw?.losses, raw?.hardFacts, raw?.evidence].filter(Array.isArray);
    return candidates.flat();
  }
  function items(raw){
    const direct=[raw?.items, raw?.riskItems, raw?.risks].filter(Array.isArray).flat();
    const comp=components(raw).flatMap(c=>[c.items,c.riskItems,c.risks].filter(Array.isArray).flat());
    return direct.concat(comp);
  }
  function expectedFromItems(list){
    if(!Array.isArray(list)||!list.length) return 0;
    return list.reduce((a,x)=>{
      const residual=num(x.residualExpectedLoss||x.residualLoss||x.residualRisk||x.residual||x.postMitigationLoss);
      if(residual) return a+residual;
      const prob=num(x.probLikely||x.probability||x.likelihood||x.frequencyPercent||x.frequency);
      const impact=num(x.impactLikely||x.impact||x.amount||x.lossAmount||x.expectedLoss);
      if(prob&&impact) return a + (prob>1?prob/100:prob)*impact;
      return a + num(x.expectedLoss||x.totalExposure||x.amountLikely||x.amount);
    },0);
  }
  function totalExposure(raw,sm){
    const componentTotal=components(raw).reduce((a,c)=>a+num(c.totalExposure||c.exposure||c.expectedLoss||c.amount||c.costLikely),0);
    const costTotal=costs(raw).reduce((a,c)=>a+num(c.amountLikely||c.amount||c.lossAmount||c.cost||c.expectedLoss),0);
    return num(sm?.totalExposure||raw?.totalExposure||sm?.expectedLoss||raw?.expectedLoss||sm?.inherentExpectedLoss||raw?.inherentExpectedLoss) || componentTotal || costTotal || expectedFromItems(items(raw));
  }
  function residualRisk(raw,sm,exposure){
    const itemResidual=expectedFromItems(items(raw));
    return num(sm?.residualExpectedLoss||raw?.residualExpectedLoss||sm?.residualRisk||raw?.residualRisk||sm?.residual||raw?.residual||sm?.betaExpectedValue||raw?.betaExpectedValue) || itemResidual || exposure;
  }
  function topDriver(raw,sm){
    const list=items(raw);
    if(list.length){
      const t=list.slice().sort((a,b)=>num(b.residualExpectedLoss||b.residualRisk||b.residual||b.impactLikely||b.amount)-num(a.residualExpectedLoss||a.residualRisk||a.residual||a.impactLikely||a.amount))[0];
      return t?.name||t?.riskItemName||t?.description||t?.riskDomain||'Risk item';
    }
    return sm?.highestDriver||raw?.highestDriver||raw?.riskDomain||raw?.primaryRegulation||'Review scenario detail';
  }
  function p90(raw,sm,residual){ return num(sm?.rangeHigh||sm?.p90||sm?.betaP90||raw?.p90||raw?.rangeHigh)||residual; }
  function rows(){
    return saved().map(raw=>{const sm=summary(raw); const exposure=totalExposure(raw,sm); const residual=residualRisk(raw,sm,exposure); const insuranceImpact=insurance(raw,sm); return {raw,sm,type:typeOf(raw),id:String(raw?.id||sm?.id||raw?.scenarioId||''),name:raw?.name||sm?.name||raw?.scenarioName||'Unnamed Scenario',area:raw?.productGroup||raw?.primaryProduct||sm?.productGroup||raw?.reportingLine||'',status:raw?.status||sm?.status||'Draft',exposure,residual,insurance:insuranceImpact,p90:p90(raw,sm,residual),driver:topDriver(raw,sm)};}).sort((a,b)=>b.residual-a.residual);
  }
  function outcomeRows(r,limit=100){
    const pools=[r.sm?.randomOutcomeRows,r.sm?.randomOutcomes,r.sm?.monteCarloRows,r.sm?.monteCarloOutcomes,r.raw?.randomOutcomeRows,r.raw?.randomOutcomes,r.raw?.monteCarloRows,r.raw?.monteCarloOutcomes];
    for(const p of pools){ if(Array.isArray(p)&&p.length) return p.slice(0,limit); }
    const low=num(r.sm?.rangeLow||r.sm?.p10||r.residual*.65), med=num(r.sm?.rangeMedian||r.sm?.p50||r.exposure||r.residual), high=num(r.sm?.rangeHigh||r.sm?.p90||r.p90||med);
    return Array.from({length:limit},(_,i)=>{const u=((Math.sin((i+1)*12.9898)*43758.5453)%1+1)%1; const v=u<.5?low+(med-low)*(u/.5):med+(high-med)*((u-.5)/.5); return {iteration:i+1,annualLoss:v,residualLoss:v,hardCost:0,softCost:0,totalLoss:v};});
  }
  ...
  function exportScenarioCsv(r){
    const lines=[['Scenario ID','Scenario Name','Type','Run','Annual Loss','Residual Loss','Hard Cost','Soft Cost','Total Loss','Outcome'].join(',')];
    outcomeRows(r,100000).forEach((x,i)=>lines.push([r.id,r.name,r.type,x.iteration||x.run||i+1,x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss||0,x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss||0,x.hardCost||x.hardCostSample||0,x.softCost||x.softCostSample||0,x.totalLoss||x.outcome||x.annualLoss||x.residualLoss||0,x.outcome||x.totalLoss||x.annualLoss||x.residualLoss||0].map(cell).join(',')));
    download(`risktool_${String(r.id||'scenario').replace(/[^a-zA-Z0-9_-]/g,'_')}_monte_carlo.csv`,lines.join('\n'));
  }
  function renderOutcomes(r){
    const host=$('#rt24Outcomes'); if(!host) return;
    host.innerHTML='<div class="card-header"><h3>Selected Scenario Outcomes</h3><span>100 Monte Carlo Rows</span></div><p class="muted">'+esc(r.name)+' — '+esc(r.id)+'</p><div class="table-wrap"><table><thead><tr><th>Run</th><th>Annual Loss</th><th>Residual Loss</th><th>Hard Cost</th><th>Soft Cost</th></tr></thead><tbody>'+outcomeRows(r,100).map((x,i)=>'<tr><td>'+esc(x.iteration||x.run||i+1)+'</td><td>'+money(x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss)+'</td><td>'+money(x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss)+'</td><td>'+money(x.hardCost||x.hardCostSample||0)+'</td><td>'+money(x.softCost||x.softCostSample||0)+'</td></tr>').join('')+'</tbody></table></div>';
  }
  function reviewScenario(r){
    try{ if(typeof openScenario==='function') return openScenario(r.id); }catch(e){}
    try{ if(typeof loadSavedScenario==='function') return loadSavedScenario(r.id); }catch(e){}
    alert('Scenario review link could not find the existing openScenario/loadSavedScenario function. Open it from Saved Scenarios.');
  }
  function heatClass(v,max){const p=max?num(v)/max:0; return p>=.8?'rt24-h5':p>=.6?'rt24-h4':p>=.4?'rt24-h3':p>=.2?'rt24-h2':'rt24-h1';}
  function renderPortfolio24(){
    const v=$('#view-reports'); if(!v) return;
    setVersion();
    const rs=rows();
    const totals=rs.reduce((t,r)=>{t.exposure+=r.exposure;t.residual+=r.residual;t.insurance+=r.insurance;return t;},{exposure:0,residual:0,insurance:0});
    const max=Math.max(1,...rs.map(r=>r.residual));
    v.innerHTML='<div class="section-header"><h2>Portfolio Report</h2><p>Runs all saved Single, Complex, and Beta scenarios, ranks by residual risk, and supports review, outcomes, and CSV export.</p></div>'+
      '<div class="builder-actions"><button class="btn btn-primary" id="rt24RunPortfolio" type="button">Run Portfolio Report</button><button class="btn btn-secondary" id="rt24SaveSnapshot" type="button">Save Report Snapshot</button></div>'+
      '<div class="grid three" id="rt24PortfolioTotals"><div class="card metric-card"><h3>Total Exposure</h3><div class="metric-value">'+money(totals.exposure)+'</div><p class="muted">SUM of Total Exposure for the scenarios listed below.</p></div><div class="card metric-card"><h3>Residual Risk</h3><div class="metric-value">'+money(totals.residual)+'</div><p class="muted">SUM of Residual Risk for the scenarios listed below.</p></div><div class="card metric-card"><h3>Insurance Impact</h3><div class="metric-value">'+money(totals.insurance)+'</div><p class="muted">SUM of Insurance Impact for the scenarios listed below.</p></div></div>'+
      '<div class="card"><div class="card-header"><h3>Heat Map Visualization</h3><span>Residual Risk</span></div><div class="rt24-heatmap">'+(rs.length?rs.map((r,i)=>'<button class="rt24-heat '+heatClass(r.residual,max)+'" data-id="'+esc(r.id)+'"><strong>'+(i+1)+'</strong><span>'+esc(r.name.slice(0,28))+'</span><em>'+money(r.residual)+'</em></button>').join(''):'<div class="note-box">No saved scenarios yet.</div>')+'</div></div>'+
      '<div class="card"><div class="card-header"><h3>Portfolio Ranking & Recommendations</h3><span>All saved scenarios; totals above are calculated from this table only.</span></div><div class="table-wrap"><table><thead><tr><th>Rank</th><th>ID</th><th>Type</th><th>Name</th><th>Status</th><th>Total Exposure</th><th>Residual Risk</th><th>Insurance Impact</th><th>Highest Driver</th><th>Actions</th></tr></thead><tbody>'+(rs.length?rs.map((r,i)=>'<tr class="rt24-row" data-id="'+esc(r.id)+'"><td>'+(i+1)+'</td><td>'+esc(r.id)+'</td><td>'+esc(r.type)+'</td><td><strong>'+esc(r.name)+'</strong><br><span class="muted">'+esc(r.area)+'</span></td><td>'+esc(r.status)+'</td><td>'+money(r.exposure)+'</td><td>'+money(r.residual)+'</td><td>'+money(r.insurance)+'</td><td>'+esc(r.driver)+'</td><td class="rt24-actions"><button class="btn btn-secondary small-btn" type="button" data-review="'+esc(r.id)+'">Review</button><button class="btn btn-secondary small-btn" type="button" data-outcomes="'+esc(r.id)+'">View Outcomes</button><button class="btn btn-secondary small-btn" type="button" data-csv="'+esc(r.id)+'">Export CSV</button></td></tr>').join(''):'<tr><td colspan="10">No saved scenarios yet. Save scenarios, then run the portfolio report.</td></tr>')+'</tbody></table></div></div>'+
      '<div class="card" id="rt24Drilldown"><div class="card-header"><h3>Scenario Drilldown</h3><span>Auditor / Examiner Support</span></div><div class="note-box">Click a scenario row to review summary details. Use View Outcomes to populate 100 Monte Carlo rows below.</div></div>'+
      '<div class="card" id="rt24Outcomes"><div class="card-header"><h3>Selected Scenario Outcomes</h3><span>Monte Carlo</span></div><div class="note-box">Select View Outcomes to preview 100 Monte Carlo rows for a scenario.</div></div>';
    const find=id=>rs.find(r=>String(r.id)===String(id));
    function drill(r,show){ const d=$('#rt24Drilldown'); if(d)d.innerHTML='<div class="card-header"><h3>Scenario Drilldown</h3><span>'+esc(r.type)+'</span></div><div class="form-grid" style="grid-template-columns:repeat(4,minmax(160px,1fr));"><label>Scenario<input readonly value="'+esc(r.name)+'"></label><label>Total Exposure<input readonly value="'+money(r.exposure)+'"></label><label>Residual Risk<input readonly value="'+money(r.residual)+'"></label><label>Insurance Impact<input readonly value="'+money(r.insurance)+'"></label></div><div class="note-box"><strong>Top driver:</strong> '+esc(r.driver)+'</div>'; if(show)renderOutcomes(r); }
    $$('.rt24-row').forEach(tr=>tr.addEventListener('click',e=>{ if(e.target.closest('button'))return; const r=find(tr.dataset.id); if(r)drill(r,false); },true));
    $$('[data-review]').forEach(b=>b.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); const r=find(b.dataset.review); if(r)reviewScenario(r); },true));
    $$('[data-outcomes]').forEach(b=>b.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); const r=find(b.dataset.outcomes); if(r)drill(r,true); },true));
    $$('[data-csv]').forEach(b=>b.addEventListener('click',e=>{ e.preventDefault(); e.stopPropagation(); const r=find(b.dataset.csv); if(r)exportScenarioCsv(r); },true));
    $$('.rt24-heat').forEach(b=>b.addEventListener('click',e=>{ e.preventDefault(); const r=find(b.dataset.id); if(r)drill(r,false); },true));
    $('#rt24RunPortfolio')?.addEventListener('click',e=>{ e.preventDefault(); renderPortfolio24(); },true);
    $('#rt24SaveSnapshot')?.addEventListener('click',e=>{ e.preventDefault(); const csv=['Rank,Scenario ID,Scenario Name,Type,Status,Total Exposure,Residual Risk,Insurance Impact,Highest Driver'].concat(rs.map((r,i)=>[i+1,r.id,r.name,r.type,r.status,r.exposure,r.residual,r.insurance,r.driver].map(cell).join(','))).join('\n'); download('portfolio_report_snapshot.csv',csv); },true);
  }
  function styles(){ if($('#rt24Styles')) return; const s=document.createElement('style'); s.id='rt24Styles'; s.textContent='.metric-card .metric-value{font-size:28px;font-weight:800;margin:8px 0}.rt24-heatmap{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px}.rt24-heat{border:1px solid #cbd5e1;border-radius:12px;padding:10px;text-align:left;cursor:pointer;background:#eef4fb;color:#10223f}.rt24-heat strong{display:block;font-size:16px}.rt24-heat span{display:block;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rt24-heat em{display:block;font-style:normal;margin-top:4px}.rt24-h1{opacity:.55}.rt24-h2{opacity:.7}.rt24-h3{opacity:.85}.rt24-h4{box-shadow:inset 0 0 0 2px rgba(0,0,0,.12)}.rt24-h5{box-shadow:inset 0 0 0 3px rgba(0,0,0,.2);font-weight:800}.rt24-row{cursor:pointer}.rt24-row:hover{background:#f3f7ff}.rt24-actions{display:flex;gap:6px;flex-wrap:wrap}.rt24-actions .small-btn{white-space:nowrap}'; document.head.appendChild(s); }
  function schedulePortfolio(){ [160,400,900,1600].forEach(t=>setTimeout(()=>{ if($('#view-reports')?.classList.contains('active')) renderPortfolio24(); },t)); }
  const oldAct=window.activateView || (typeof activateView==='function'?activateView:null);
  if(oldAct){ window.activateView=function(v){ const r=oldAct.apply(this,arguments); setVersion(); styles(); if(v==='reports'||v==='portfolio-report') schedulePortfolio(); return r; }; try{ activateView=window.activateView; }catch(e){} }
  document.addEventListener('click',e=>{ if(e.target.closest('.nav-item[data-view="reports"],.nav-item[data-view="portfolio-report"],#rt24RunPortfolio')) schedulePortfolio(); },true);
  window.addEventListener('hashchange',()=>schedulePortfolio(),true);
  function boot(){ setVersion(); styles(); if($('#view-reports')?.classList.contains('active')) schedulePortfolio(); }
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  window.renderPortfolio24=renderPortfolio24;
})();
/* ===== END PHASE 24.0.00 PORTFOLIO TOTALS INTEGRITY OVERRIDE ===== */
