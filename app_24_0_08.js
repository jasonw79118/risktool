...
function runFinancialMonteCarlo(payload) {
  const allowedIterations = [100, 1000, 10000, 100000];
...
/* ===== PHASE 24.0.08 COMPLEX COST/LOSS TABLE + EVIDENCE-DRIVEN RISK ITEMS ===== */
...
/* ===== END PHASE 24.0.08 SAVE OVERRIDE ===== */


/* ===== PHASE 24.0.08 WORKSPACE BRIDGE ===== */
document.addEventListener("DOMContentLoaded", () => {
  try { refreshWorkspaceDiagnostics(); } catch (e) { console.error(e); }
});
/* ===== END PHASE 24.0.08 WORKSPACE BRIDGE ===== */

/* ===== PHASE 24.0.08 CHROME EDGE WORKSPACE MODE ===== */
...
/* ===== END PHASE 24.0.08 CHROME EDGE WORKSPACE MODE ===== */


/* ===== PHASE 24.0.08 SESSION RESTORE AFTER LOGIN ===== */
...
/* ===== END PHASE 24.0.08 SESSION RESTORE AFTER LOGIN ===== */

/* ===== PHASE 24.0.08 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */
(function(){
...
/* ===== END PHASE 24.0.08 LOGIN RECONNECT + RESTORE PROMPT FIX ===== */


/* ===== PHASE 24.0.08 WORKSPACE-GATED LISTS + POST-LOGIN RESTORE ===== */
(function(){
  const PHASE = "24.0.08";
...
/* ===== PHASE 24.0.08 DIRECT LOGIN RECONNECT HOOK ===== */
(function(){
  const PHASE = "24.0.08";
...
/* ===== PHASE 24.0.08 DIRECT POST-LOGIN RECONNECT FLOW ===== */
(function(){
  const PHASE = "24.0.08";
  ...
  async function writeSessionState29(reason, extra){
    try {
      ...
      await writable.close();
      return payload;
    } catch(e) { console.error('Phase 24.0.08 session write failed', e); return null; }
  }
...
/* ===== PHASE 24.0.08 RUNTIME REBASE: LOGIN + EDITABILITY + VIEW GUARD ===== */
(function(){
  'use strict';
  var PHASE='24.0.08';
...
/* ===== END PHASE 24.0.08 RUNTIME REBASE ===== */
/* ===== PHASE 24.0.08 COMPLEX WORKSPACE UI ===== */
(function(){
  const PHASE = "24.0.08";
  try { window.RISKTOOL_RUNTIME_VERSION = PHASE; } catch(e) {}
...
/* ===== END PHASE 24.0.08 COMPLEX WORKSPACE UI ===== */

/* ===== PHASE 24.0.08 COMPLEX WORKSPACE VISUAL MATCH PATCH ===== */
(function(){
  const PHASE = "24.0.08";
...
/* ===== END PHASE 24.0.08 COMPLEX WORKSPACE VISUAL MATCH PATCH ===== */

/* ===== PHASE 24.0.08 COMPLEX WORKSPACE LAYOUT CORRECTION ===== */
(function(){
  const PHASE = "24.0.08";
  try { window.RISKTOOL_RUNTIME_VERSION = PHASE; } catch(e) {}
  const qs=(s,r=document)=>r.querySelector(s);
...
/* ===== END PHASE 24.0.08 COMPLEX WORKSPACE LAYOUT CORRECTION ===== */


/* ===== PHASE 24.0.08 SINGLE + BETA WORKSPACE UI RESTORE ===== */
(function(){
  const PHASE='24.0.08';
  const qs=(s,r=document)=>r.querySelector(s); const qsa=(s,r=document)=>Array.from(r.querySelectorAll(s));
  const esc=v=>String(v??'').replace(/[&<>\"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
...
/* ===== END PHASE 24.0.08 SINGLE + BETA WORKSPACE UI RESTORE ===== */


/* ===== PHASE 24.0.08 COMPLEX TEST + NEW COMPONENT ACTIONS ===== */
(function(){
  'use strict';
  const PHASE='24.0.08';
  const qs=(s,r=document)=>r.querySelector(s);
...
/* ===== END PHASE 24.0.08 COMPLEX TEST + NEW COMPONENT ACTIONS ===== */


/* ===== PHASE 24.0.08 BUTTON PLACEMENT + POST-LOGIN WORKSPACE RESTORE ===== */
(function(){
  const VERSION='24.0.08';
  const qs=(s,r=document)=>r.querySelector(s);
  function setVersion(){
...
/* ===== END PHASE 24.0.08 BUTTON PLACEMENT + POST-LOGIN WORKSPACE RESTORE ===== */


/* ===== PHASE 24.0.08 STABILIZATION PATCH ===== */
(function(){
  'use strict';
  const VERSION='24.0.08';
  const qs=(s,r=document)=>r.querySelector(s);
  ...
  function call(name,...args){try{if(typeof window[name]==='function')return window[name](...args)}catch(e){console.warn('RT 24.0.08',name,e)}}
...
/* ===== END PHASE 24.0.08 STABILIZATION PATCH ===== */


/* ===== PHASE 24.0.08 TARGETED FIX: POST-LOGIN LOAD + PORTFOLIO RUNNER ===== */
(function(){
  'use strict';
  const VERSION='24.0.08';
...
/* ===== END PHASE 24.0.08 TARGETED FIX ===== */

/* ===== PHASE 24.0.08 TARGETED LOAD + PORTFOLIO REPORT REBUILD ===== */
(function(){
  'use strict';
  const VERSION='24.0.08';
...
/* ===== END PHASE 24.0.08 TARGETED LOAD + PORTFOLIO REPORT REBUILD ===== */


/* ===== PHASE 24.0.08 TARGETED POST-LOGIN LOAD + PORTFOLIO ACTION BUTTONS ===== */
(function(){
'use strict';
const VERSION='24.0.08', $=(s,r=document)=>r.querySelector(s), $$=(s,r=document)=>Array.from(r.querySelectorAll(s));
const esc=v=>String(v??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
...
function closeM(){ $$('#rt78PostLoginLoadModal,#rt77PostLoginLoadModal,#rt76PostLoginLoadModal,#restoreSessionModal,#postLoginWorkspaceReconnectModal,#rtReconnectWorkspace25,#rtDirectReconnect28,#rtWorkspaceReconnect29,.rt-modal').forEach(m=>{try{m.style.display='none';m.classList.add('hidden')}catch(e){}}); }
function dash(){closeM(); try{unlockAppShell()}catch(e){} try{activateView('dashboard')}catch(e){} try{refreshLibraries()}catch(e){}}
async function connectFolder(status){
 if(status)status.textContent='Opening Chrome/Edge workspace folder picker...';
 try{
  if(typeof window.showDirectoryPicker!=='function'){ if(status)status.textContent='Use current Chrome or Edge for workspace folder access.'; return false; }
  const h=await window.showDirectoryPicker({mode:'readwrite'});
  try{workspaceFolderHandle=h}catch(e){window.workspaceFolderHandle=h}
  try{workspaceFolderName=h?.name||'Selected folder'}catch(e){window.workspaceFolderName=h?.name||'Selected folder'}
  try{ if(typeof setWorkspaceLocalPath==='function' && typeof getWorkspaceLocalPath==='function' && !getWorkspaceLocalPath()) setWorkspaceLocalPath(h?.name||'Selected folder') }catch(e){}
  let loaded=[]; try{ if(typeof loadWorkspaceScenarioCache==='function') loaded=await loadWorkspaceScenarioCache(); }catch(e){console.warn(e)}
  try{refreshWorkspaceFolderModeUi()}catch(e){} try{refreshWorkspaceDiagnostics()}catch(e){} try{renderSavedScenarios()}catch(e){} try{renderDashboardOpenTable()}catch(e){}
  if(status)status.textContent=`Workspace connected: ${h?.name||'Selected folder'}. Loaded ${Array.isArray(loaded)?loaded.length:0} scenario record(s). Continuing...`;
  setTimeout(dash,350); return true;
 }catch(e){console.error(e); if(status)status.textContent='Workspace folder was not connected. Select the approved workspace folder again, or choose Not Now to continue.'; return false;}
}
function modal(){let m=$('#rt78PostLoginLoadModal'); if(m)return m; m=document.createElement('div'); m.id='rt78PostLoginLoadModal'; m.style.cssText='display:none;position:fixed;inset:0;z-index:30000;background:rgba(12,18,34,.72);align-items:center;justify-content:center;padding:24px;'; m.innerHTML='<div class="card" style="width:min(720px,96vw);max-height:90vh;overflow:auto;"><div class="card-header"><h3>Load Scenario Workspace</h3><span>Post-login workspace restore</span></div><div class="card-body"><p style="margin-top:0;">Login is complete. Select the approved RiskTool workspace folder to load saved scenario files. Choose Not Now to continue to the dashboard without loading files.</p><div class="note-box" id="rt78LoadStatus">Waiting for workspace folder selection.</div><div class="builder-actions"><button class="btn btn-primary" type="button" id="rt78ChooseWorkspace">Load Scenario Workspace</button><button class="btn btn-secondary" type="button" id="rt78NotNow">Not Now</button></div></div></div>'; document.body.appendChild(m); $('#rt78ChooseWorkspace',m)?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();connectFolder($('#rt78LoadStatus'))},true); $('#rt78NotNow',m)?.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();dash()},true); return m;}
function showLoad(){try{if(typeof isUserLoggedIn==='function'&&!isUserLoggedIn())return}catch(e){} closeM(); const m=modal(), st=$('#rt78LoadStatus',m); if(st)st.textContent='Waiting for workspace folder selection.'; m.style.display='flex'; m.classList.remove('hidden')}
window.showPostLoginRestoreScreen=showLoad; window.rt78ShowPostLoginLoad=showLoad;
function saved(){try{return (typeof getSavedScenarios==='function'?getSavedScenarios():[])||[]}catch(e){return[]}}
function typeOf(s){return String(s?.mode||s?.scenarioType||'single').toLowerCase()}
function sum(s){try{if(typeof summarizePayload==='function')return summarizePayload({...s})}catch(e){console.warn(e)} return {...s}}
function ins(s){return (Array.isArray(s.insurance)?s.insurance:[]).reduce((a,x)=>a+num(x.coverageAmount||x.coverage||x.limit),0)}
function drv(raw,sm){const items=Array.isArray(raw.items)?raw.items:[]; if(items.length){const t=items.slice().sort((a,b)=>num(b.residual||b.total||b.impactMax||b.impactLikely)-num(a.residual||a.total||a.impactMax||a.impactLikely))[0]; return t?.name||t?.riskItemName||t?.description||t?.riskDomain||'Component risk item'} return sm.highestDriver||raw.highestDriver||raw.riskDomain||raw.primaryRegulation||'Scenario level exposure'}
function rows(){return saved().map(raw=>{const sm=sum(raw), typ=typeOf(raw), exposure=num(sm.expectedLoss||sm.totalExposure||sm.betaExpectedValue||sm.inherent||raw.expectedLoss||raw.totalExposure), residual=num(sm.residualExpectedLoss||sm.residual||sm.betaP90||raw.residualExpectedLoss||raw.residual), p90=num(sm.rangeHigh||sm.p90||sm.betaP90||residual); return {raw,sum:sm,type:typ,exposure,residual,p90,insurance:ins(raw),driver:drv(raw,sm)}}).sort((a,b)=>b.residual-a.residual)}
function outcomeRows(r,limit=100){const s=r.sum||{}, raw=r.raw||{}; for(const c of [s.randomOutcomeRows,s.randomOutcomes,s.monteCarloRows,s.monteCarloOutcomes,raw.randomOutcomeRows,raw.randomOutcomes,raw.monteCarloRows,raw.monteCarloOutcomes]) if(Array.isArray(c)&&c.length)return c.slice(0,limit); const low=num(s.rangeLow||s.p10||s.betaP10||r.residual*.65||r.residual), med=num(s.rangeMedian||s.p50||s.betaP50||s.expectedLoss||r.residual||r.exposure), high=num(s.rangeHigh||s.p90||s.betaP90||r.p90||med||low), out=[]; for(let i=1;i<=limit;i++){const w=Math.sin(i*12.9898)*43758.5453, u=w-Math.floor(w), v=u<.5?low+(med-low)*(u/.5):med+(high-med)*((u-.5)/.5); out.push({iteration:i,annualLoss:v,residualLoss:v,hardCost:0,softCost:0})} return out}
function cell(v){const s=String(v??''); return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s}
function dl(name,text,type='text/csv;charset=utf-8'){const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type})); a.download=name; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},500)}
function exportCsv(r){const out=outcomeRows(r,100000), lines=[['Scenario ID','Scenario Name','Type','Run','Annual Loss','Residual Loss','Hard Cost','Soft Cost','Total Loss','Outcome'].join(',')]; out.forEach((x,i)=>lines.push([r.raw.id||r.sum.id||'',r.raw.name||r.sum.name||'',r.type,x.iteration||x.run||i+1,x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss||0,x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss||0,x.hardCost||x.hardCostSample||0,x.softCost||x.softCostSample||0,x.totalLoss||x.outcome||x.annualLoss||x.residualLoss||0,x.outcome||x.totalLoss||x.annualLoss||x.residualLoss||0].map(cell).join(','))); dl(`risktool_${String(r.raw.id||r.sum.id||'scenario').replace(/[^a-zA-Z0-9_-]/g,'_')}_monte_carlo.csv`,lines.join('\n'))}
function heat(v,max){const p=max?v/max:0; return p>=.8?'rt78-h5':p>=.6?'rt78-h4':p>=.4?'rt78-h3':p>=.2?'rt78-h2':'rt78-h1'}
function renderOut(r){const h=$('#rt78Outcomes'); if(!h)return; const rs=outcomeRows(r,100); h.innerHTML='<div class="card-header"><h3>Selected Scenario Outcomes</h3><span>100 Monte Carlo Rows</span></div><p class="muted">'+esc(r.raw.name||r.sum.name||'Selected scenario')+' — '+esc(r.raw.id||r.sum.id||'')+'</p><div class="table-wrap"><table><thead><tr><th>Run</th><th>Annual Loss</th><th>Residual Loss</th><th>Hard Cost</th><th>Soft Cost</th></tr></thead><tbody>'+rs.map((x,i)=>'<tr><td>'+esc(x.iteration||x.run||i+1)+'</td><td>'+money(x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss)+'</td><td>'+money(x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss)+'</td><td>'+money(x.hardCost||x.hardCostSample||0)+'</td><td>'+money(x.softCost||x.softCostSample||0)+'</td></tr>').join('')+'</tbody></table></div>'}
function review(id){try{if(typeof openScenario==='function')openScenario(id)}catch(e){console.error(e)}}
function renderPortfolio(){const v=$('#view-reports'); if(!v)return; const rs=rows(), totals=rs.reduce((t,r)=>(t.exposure+=r.exposure,t.residual+=r.residual,t.insurance+=r.insurance,t),{exposure:0,residual:0,insurance:0}), max=Math.max(1,...rs.map(r=>r.residual)); v.innerHTML='<div class="section-header"><h2>Portfolio Report</h2><p>Runs all saved Single, Complex, and Beta scenarios, ranks by residual risk, and supports review, outcomes, and CSV export.</p></div><div class="builder-actions"><button class="btn btn-primary" id="rt78RunPortfolio" type="button">Run Portfolio Report</button><button class="btn btn-secondary" id="rt78SaveSnapshot" type="button">Save Report Snapshot</button></div><div class="grid three"><div class="card metric-card"><h3>Total Exposure</h3><div class="metric-value">'+money(totals.exposure)+'</div><p class="muted">Modeled exposure across all saved scenarios.</p></div><div class="card metric-card"><h3>Residual Risk</h3><div class="metric-value">'+money(totals.residual)+'</div><p class="muted">Remaining modeled risk after controls.</p></div><div class="card metric-card"><h3>Insurance Impact</h3><div class="metric-value">'+money(totals.insurance)+'</div><p class="muted">Total recorded coverage available for transfer.</p></div></div><div class="card"><div class="card-header"><h3>Heat Map Visualization</h3><span>Residual Risk</span></div><div class="rt78-heatmap">'+(rs.length?rs.map((r,i)=>'<button class="rt78-heat '+heat(r.residual,max)+'" data-id="'+esc(r.raw.id||r.sum.id||'')+'"><strong>'+(i+1)+'</strong><span>'+esc((r.raw.name||r.sum.name||'Scenario').slice(0,28))+'</span><em>'+money(r.residual)+'</em></button>').join(''):'<div class="note-box">No saved scenarios yet.</div>')+'</div></div><div class="card"><div class="card-header"><h3>Portfolio Ranking & Recommendations</h3><span>All saved scenarios</span></div><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Scenario</th><th>Type</th><th>Area / Product Family</th><th>Total Exposure</th><th>Residual Risk</th><th>Insurance Impact</th><th>P90</th><th>Top Driver</th><th>Actions</th></tr></thead><tbody>'+(rs.length?rs.map((r,i)=>{const id=String(r.raw.id||r.sum.id||'');return '<tr class="rt78-row" data-id="'+esc(id)+'"><td>'+(i+1)+'</td><td><strong>'+esc(r.raw.name||r.sum.name||'Unnamed')+'</strong><br><span class="muted">'+esc(id)+'</span></td><td>'+esc(r.type)+'</td><td>'+esc(r.raw.productGroup||r.raw.primaryProduct||r.sum.productGroup||'')+'</td><td>'+money(r.exposure)+'</td><td>'+money(r.residual)+'</td><td>'+money(r.insurance)+'</td><td>'+money(r.p90)+'</td><td>'+esc(r.driver)+'</td><td class="rt78-actions"><button class="btn btn-secondary small-btn" type="button" data-review-scenario="'+esc(id)+'">Review</button><button class="btn btn-secondary small-btn" type="button" data-view-outcomes="'+esc(id)+'">View Outcomes</button><button class="btn btn-secondary small-btn" type="button" data-export-csv="'+esc(id)+'">Export CSV</button></td></tr>'}).join(''):'<tr><td colspan="10">No saved scenarios yet. Save scenarios, then run the portfolio report.</td></tr>')+'</tbody></table></div></div><div class="card" id="rt78Drilldown"><div class="card-header"><h3>Scenario Drilldown</h3><span>Auditor / Examiner Support</span></div><div class="note-box">Click a scenario row to review summary details. Use View Outcomes to populate 100 Monte Carlo rows below.</div></div><div class="card" id="rt78Outcomes"><div class="card-header"><h3>Selected Scenario Outcomes</h3><span>Monte Carlo</span></div><div class="note-box">Select View Outcomes to preview 100 Monte Carlo rows for a scenario.</div></div>'; function pick(id,show){const r=rs.find(x=>String(x.raw.id||x.sum.id||'')===String(id)); if(!r)return; const d=$('#rt78Drilldown'); if(d)d.innerHTML='<div class="card-header"><h3>Scenario Drilldown</h3><span>'+esc(r.type)+'</span></div><div class="form-grid" style="grid-template-columns:repeat(4,minmax(160px,1fr));"><label>Scenario<input readonly value="'+esc(r.raw.name||r.sum.name||'Unnamed')+'"></label><label>Residual Risk<input readonly value="'+money(r.residual)+'"></label><label>Total Exposure<input readonly value="'+money(r.exposure)+'"></label><label>P90<input readonly value="'+money(r.p90)+'"></label></div><div class="note-box"><strong>Top driver:</strong> '+esc(r.driver)+'</div><p class="muted">'+esc(r.sum.generatedSummary||r.raw.generatedSummary||'No generated summary available.')+'</p>'; if(show)renderOut(r)} $$('.rt78-row').forEach(tr=>tr.addEventListener('click',e=>{if(e.target.closest('button'))return;pick(tr.dataset.id,false)},true)); $$('[data-review-scenario]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();review(b.dataset.reviewScenario)},true)); $$('[data-view-outcomes]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();pick(b.dataset.viewOutcomes,true)},true)); $$('[data-export-csv]').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const r=rs.find(x=>String(x.raw.id||x.sum.id||'')===String(b.dataset.exportCsv)); if(r)exportCsv(r)},true)); $$('.rt78-heat').forEach(b=>b.addEventListener('click',e=>{e.preventDefault();pick(b.dataset.id,false)},true)); $('#rt78RunPortfolio')?.addEventListener('click',e=>{e.preventDefault();renderPortfolio()},true); $('#rt78SaveSnapshot')?.addEventListener('click',e=>{e.preventDefault(); const csv=['Rank,Scenario,Type,Total Exposure,Residual Risk,Insurance Impact,P90,Top Driver'].concat(rs.map((r,i)=>[i+1,r.raw.name||r.sum.name||'',r.type,r.exposure,r.residual,r.insurance,r.p90,r.driver].map(cell).join(','))).join('\n'); dl('portfolio_report_snapshot.csv',csv)},true)}
function styles(){if($('#rt78Styles'))return; const s=document.createElement('style'); s.id='rt78Styles'; s.textContent='.metric-card .metric-value{font-size:28px;font-weight:800;margin:8px 0}.rt78-heatmap{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px}.rt78-heat{border:1px solid #cbd5e1;border-radius:12px;padding:10px;text-align:left;cursor:pointer;background:#eef4fb;color:#10223f}.rt78-heat strong{display:block;font-size:16px}.rt78-heat span{display:block;font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rt78-heat em{display:block;font-style:normal;margin-top:4px}.rt78-h1{opacity:.55}.rt78-h2{opacity:.7}.rt78-h3{opacity:.85}.rt78-h4{box-shadow:inset 0 0 0 2px rgba(0,0,0,.12)}.rt78-h5{box-shadow:inset 0 0 0 3px rgba(0,0,0,.2);font-weight:800}.rt78-row{cursor:pointer}.rt78-row:hover{background:#f3f7ff}.rt78-actions{display:flex;gap:6px;flex-wrap:wrap}.rt78-actions .small-btn{white-space:nowrap}'; document.head.appendChild(s)}
const oldAct=window.activateView||(typeof activateView==='function'?activateView:null); if(oldAct){window.activateView=function(v){const r=oldAct.apply(this,arguments); setTimeout(()=>{setV();styles(); if(v==='reports'||v==='portfolio-report')renderPortfolio()},80); return r}; try{activateView=window.activateView}catch(e){}}
...
/* ===== PHASE 24.0.08 PORTFOLIO TOTALS + VERSION LOCK ===== */
(function(){
  'use strict';
  const VERSION = '24.0.08';
  const SCENARIO_KEY = 'risk_manager_saved_evaluations_v2';
  ...
  const money = n => {
    try { return new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(num(n)); }
    catch(e){ return '$' + Math.round(num(n)).toLocaleString(); }
  };
  function lockVersion(){
    try { window.RISKTOOL_RUNTIME_VERSION = VERSION; } catch(e) {}
    try { if (typeof APP_VERSION !== 'undefined') window.APP_VERSION = VERSION; } catch(e) {}
    document.title = 'RiskTool ' + VERSION;
    const selectors = ['#appVersion','.app-version','[data-version-label]','[data-app-version]','[data-version]','.version'];
    selectors.forEach(sel => $$(sel).forEach(el => {
      const txt = String(el.textContent || '').trim();
      if (!txt || /\d+\.\d+\.\d+/.test(txt) || el.id === 'appVersion') el.textContent = txt.startsWith('Phase') ? 'Phase ' + VERSION : VERSION;
    }));
    $$('.sidebar-note h4').forEach(el => el.textContent = 'Phase ' + VERSION);
    $$('*').forEach(el => {
      if (el.childNodes && el.childNodes.length === 1 && el.childNodes[0].nodeType === 3) {
        const t = el.textContent || '';
        if (/\b(?:23|24)\.0\.\d+\b/.test(t)) el.textContent = t.replace(/\b(?:23|24)\.0\.\d+\b/g, VERSION);
      }
    });
  }
  function savedScenarios(){
    try { if (typeof window.getSavedScenarios === 'function') return window.getSavedScenarios() || []; } catch(e) {}
    try { return JSON.parse(localStorage.getItem(SCENARIO_KEY) || '[]') || []; } catch(e) { return []; }
  }
  function summarize(raw){
    try { if (typeof window.summarizePayload === 'function') return window.summarizePayload(raw) || raw || {}; } catch(e) {}
    return raw || {};
  }
  function scenarioType(raw){
    return String(raw?.mode || raw?.scenarioType || raw?.type || 'single').replace(/scenario/ig,'').trim() || 'single';
  }
  function scenarioId(raw,sum){ return String(raw?.id || sum?.id || raw?.scenarioId || 'SCN-' + Math.random().toString(36).slice(2)); }
  function scenarioName(raw,sum){ return raw?.name || raw?.scenarioName || sum?.name || 'Unnamed Scenario'; }
  function insuranceAmount(raw,sum, exposure, residual){
    const arrays = [raw?.insurance, raw?.insurances, raw?.insurancePolicies, raw?.riskTransfer, sum?.insurance].filter(Array.isArray);
    let total = 0;
    arrays.forEach(arr => arr.forEach(x => { total += num(x.coverageAmount ?? x.limit ?? x.amount ?? x.value ?? x.expectedTransfer ?? x.insuranceImpact); }));
    const direct = num(raw?.insuranceImpact ?? raw?.insuranceBenefit ?? sum?.insuranceImpact ?? sum?.insuranceBenefit);
    if (direct) total += direct;
    if (!total && exposure >= residual) total = exposure - residual;
    return Math.max(0,total);
  }
  function exposureAmount(raw,sum){
    const direct = num(sum?.totalExposure ?? raw?.totalExposure ?? sum?.expectedLoss ?? raw?.expectedLoss ?? raw?.betaExpectedValue ?? raw?.p90 ?? sum?.p90);
    if (direct) return direct;
    const arrays = [raw?.costs, raw?.costLosses, raw?.costLoss, raw?.losses, raw?.components, raw?.riskItems].filter(Array.isArray);
    let total = 0;
    arrays.forEach(arr => arr.forEach(x => {
      total += num(x.amountLikely ?? x.amount ?? x.expectedLoss ?? x.totalExposure ?? x.cost ?? x.loss ?? x.impactLikely);
    }));
    return total;
  }
  function residualAmount(raw,sum, exposure){
    const direct = num(sum?.residualRisk ?? sum?.residualExpectedLoss ?? raw?.residualRisk ?? raw?.residualExpectedLoss ?? raw?.residual ?? sum?.residual ?? raw?.betaP90);
    if (direct) return direct;
    const mitigation = num(raw?.mitigationImpact ?? raw?.controlEffectiveness ?? sum?.mitigationImpact ?? sum?.controlEffectiveness);
    if (mitigation > 0 && mitigation <= 100) return exposure * (1 - mitigation / 100);
    return exposure;
  }
  function p90Amount(raw,sum,residual){ return num(sum?.p90 ?? raw?.p90 ?? raw?.betaP90 ?? raw?.percentile90 ?? residual); }
  function topDriver(raw,sum){
    return sum?.highestDriver || raw?.highestDriver || raw?.topDriver || raw?.riskDriver || (Array.isArray(raw?.riskItems) && raw.riskItems[0]?.name) || 'Review scenario detail';
  }
  function portfolioRows(){
    return savedScenarios().map(raw => {
      const sum = summarize(raw);
      const exposure = exposureAmount(raw,sum);
      const residual = residualAmount(raw,sum,exposure);
      const insurance = insuranceAmount(raw,sum,exposure,residual);
      return { raw, sum, id: scenarioId(raw,sum), name: scenarioName(raw,sum), type: scenarioType(raw), exposure, residual, insurance, p90: p90Amount(raw,sum,residual), driver: topDriver(raw,sum) };
    }).sort((a,b) => b.residual - a.residual);
  }
  function portfolioTotals(rows){
    return rows.reduce((t,r) => { t.exposure += num(r.exposure); t.residual += num(r.residual); t.insurance += num(r.insurance); return t; }, { exposure:0, residual:0, insurance:0 });
  }
  function outcomeRows(r, limit){
    const raw = r.raw || {}, sum = r.sum || {};
    const candidates = [raw.monteCarloOutcomes, raw.simulationResults, raw.outcomes, raw.iterations, sum.outcomes].filter(Array.isArray);
    const arr = candidates.find(a => a.length) || [];
    if (arr.length) return arr.slice(0,limit);
    const low = Math.max(0, r.residual * 0.45), mid = r.residual || r.exposure || 1, high = Math.max(mid, r.p90 || mid * 1.6);
    const out=[];
    for(let i=1;i<=limit;i++){
      const seed = Math.sin((i + r.id.length) * 12.9898) * 43758.5453;
      const u = seed - Math.floor(seed);
      const loss = u < .5 ? low + (mid-low)*(u/.5) : mid + (high-mid)*((u-.5)/.5);
      out.push({ iteration:i, annualLoss:loss, residualLoss:loss, hardCost:0, softCost:0, totalLoss:loss });
    }
    return out;
  }
  ...
  function renderOutcomes(row){
    const host = $('#rt2401Outcomes'); if(!host) return;
    const rows = outcomeRows(row,100);
    host.innerHTML = '<div class="card-header"><h3>Selected Scenario Outcomes</h3><span>100 Monte Carlo Rows</span></div>'+
      '<p class="muted">'+esc(row.name)+' — '+esc(row.id)+'</p>'+
      '<div class="table-wrap"><table><thead><tr><th>Run</th><th>Annual Loss</th><th>Residual Loss</th><th>Hard Cost</th><th>Soft Cost</th><th>Total Loss</th></tr></thead><tbody>'+
      rows.map((x,i)=>'<tr><td>'+esc(x.iteration||x.run||i+1)+'</td><td>'+money(x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss)+'</td><td>'+money(x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss)+'</td><td>'+money(x.hardCost||x.hardCostSample||0)+'</td><td>'+money(x.softCost||x.softCostSample||0)+'</td><td>'+money(x.totalLoss||x.outcome||x.annualLoss||x.residualLoss||0)+'</td></tr>').join('')+
      '</tbody></table></div>';
    removeLegacyPortfolioTotals();
    setTimeout(removeLegacyPortfolioTotals, 50);
    setTimeout(removeLegacyPortfolioTotals, 250);
  }
  function exportScenarioCsv(row){
    const lines = [['Scenario ID','Scenario Name','Type','Run','Annual Loss','Residual Loss','Hard Cost','Soft Cost','Total Loss','Outcome'].join(',')];
    outcomeRows(row,100000).forEach((x,i)=>lines.push([row.id,row.name,row.type,x.iteration||x.run||i+1,x.annualLoss||x.outcome||x.totalLoss||x.loss||x.residualLoss||0,x.residualLoss||x.outcome||x.totalLoss||x.loss||x.annualLoss||0,x.hardCost||x.hardCostSample||0,x.softCost||x.softCostSample||0,x.totalLoss||x.outcome||x.annualLoss||x.residualLoss||0,x.outcome||x.totalLoss||x.annualLoss||x.residualLoss||0].map(csvCell).join(',')));
    download('risktool_'+row.id.replace(/[^a-zA-Z0-9_-]/g,'_')+'_monte_carlo.csv', lines.join('\n'));
  }
  function reviewScenario(row){
    try { if (typeof window.openScenario === 'function') return window.openScenario(row.id); } catch(e) {}
    try { if (typeof window.activateView === 'function') window.activateView(row.type.toLowerCase().includes('complex') ? 'complex' : row.type.toLowerCase().includes('beta') ? 'beta' : 'single'); } catch(e) {}
  }
  function addStyles(){
    if ($('#rt2401Styles')) return;
    const s=document.createElement('style'); s.id='rt2401Styles';
    s.textContent = '.rt2401-metrics{display:grid;grid-template-columns:repeat(3,minmax(180px,1fr));gap:14px;margin:14px 0}.rt2401-metric .metric-value{font-size:28px;font-weight:800;margin:8px 0}.rt2401-heatmap{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:10px}.rt2401-heat{border:1px solid #cbd5e1;border-radius:12px;padding:10px;text-align:left;cursor:pointer;background:#eef4fb;color:#10223f}.rt2401-heat strong,.rt2401-heat span,.rt2401-heat em{display:block}.rt2401-heat span{font-weight:700;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.rt2401-heat em{font-style:normal;margin-top:4px}.rt2401-h1{opacity:.55}.rt2401-h2{opacity:.7}.rt2401-h3{opacity:.85}.rt2401-h4{box-shadow:inset 0 0 0 2px rgba(0,0,0,.12)}.rt2401-h5{box-shadow:inset 0 0 0 3px rgba(0,0,0,.2);font-weight:800}.rt2401-row{cursor:pointer}.rt2401-row:hover{background:#f3f7ff}.rt2401-actions{display:flex;gap:6px;flex-wrap:wrap}.rt2401-actions .small-btn{white-space:nowrap}';
    document.head.appendChild(s);
  }
  function removeLegacyPortfolioTotals(){
    const view = $('#view-reports'); if(!view) return;
    // Legacy report block reappears after View Outcomes in older runtime handlers.
    // It contains the duplicated Total Scenarios / Total Exposure / Residual Risk / Insurance Impact fields.
    Array.from(view.children).forEach(child => {
      const text = (child.textContent || '').replace(/\s+/g,' ').trim();
      const isLegacyTotals = text.includes('Total Scenarios') && text.includes('Total Exposure') && text.includes('Residual Risk') && text.includes('Insurance Impact');
      const isOurMetrics = child.classList && child.classList.contains('rt2401-metrics');
      const isOurHeader = child.classList && child.classList.contains('section-header');
      if (isLegacyTotals && !isOurMetrics && !isOurHeader) child.remove();
    });
    // Also remove any nested legacy cards that get injected by prior report code.
    $$('div.card', view).forEach(card => {
      const text = (card.textContent || '').replace(/\s+/g,' ').trim();
      if (text.includes('Total Scenarios') && text.includes('Highest Driver') && !card.querySelector('#rt2401Outcomes')) card.remove();
    });
  }
  function heatClass(value,max){ const p=max ? value/max : 0; return p>=.8?'rt2401-h5':p>=.6?'rt2401-h4':p>=.4?'rt2401-h3':p>=.2?'rt2401-h2':'rt2401-h1'; }
  function renderPortfolioReport(){
    const view = $('#view-reports'); if (!view) return;
    addStyles(); lockVersion();
    const rows = portfolioRows();
    const totals = portfolioTotals(rows);
    const maxResidual = Math.max(1, ...rows.map(r=>r.residual));
    view.dataset.rt2401Portfolio = 'true';
    view.innerHTML = '<div class="section-header"><h2>Portfolio Report</h2><p>Runs all saved Single, Complex, and Beta scenarios, ranks by residual risk, and supports review, outcomes, and CSV export.</p></div>'+
      '<div class="builder-actions"><button class="btn btn-primary" id="rt2401RunPortfolio" type="button">Run Portfolio Report</button><button class="btn btn-secondary" id="rt2401SaveSnapshot" type="button">Save Report Snapshot</button></div>'+
      '<div class="rt2401-metrics"><div class="card metric-card rt2401-metric"><h3>Total Exposure</h3><div class="metric-value">'+money(totals.exposure)+'</div><p class="muted">SUM of the scenario rows below.</p></div><div class="card metric-card rt2401-metric"><h3>Residual Risk</h3><div class="metric-value">'+money(totals.residual)+'</div><p class="muted">SUM of post-mitigation scenario rows below.</p></div><div class="card metric-card rt2401-metric"><h3>Insurance Impact</h3><div class="metric-value">'+money(totals.insurance)+'</div><p class="muted">SUM of scenario insurance impact below.</p></div></div>'+
      '<div class="card"><div class="card-header"><h3>Heat Map Visualization</h3><span>Residual Risk</span></div><div class="rt2401-heatmap">'+(rows.length?rows.map((r,i)=>'<button class="rt2401-heat '+heatClass(r.residual,maxResidual)+'" data-rt2401-id="'+esc(r.id)+'"><strong>'+(i+1)+'</strong><span>'+esc(r.name.slice(0,28))+'</span><em>'+money(r.residual)+'</em></button>').join(''):'<div class="note-box">No saved scenarios yet.</div>')+'</div></div>'+
      '<div class="card"><div class="card-header"><h3>Portfolio Ranking & Recommendations</h3><span>All saved scenarios</span></div><div class="table-wrap"><table><thead><tr><th>Rank</th><th>Scenario</th><th>Type</th><th>Area / Product Family</th><th>Total Exposure</th><th>Residual Risk</th><th>Insurance Impact</th><th>P90</th><th>Top Driver</th><th>Actions</th></tr></thead><tbody>'+
      (rows.length?rows.map((r,i)=>'<tr class="rt2401-row" data-rt2401-id="'+esc(r.id)+'"><td>'+(i+1)+'</td><td><strong>'+esc(r.name)+'</strong><br><span class="muted">'+esc(r.id)+'</span></td><td>'+esc(r.type)+'</td><td>'+esc(r.raw.productGroup||r.raw.primaryProduct||r.sum.productGroup||'')+'</td><td>'+money(r.exposure)+'</td><td>'+money(r.residual)+'</td><td>'+money(r.insurance)+'</td><td>'+money(r.p90)+'</td><td>'+esc(r.driver)+'</td><td class="rt2401-actions"><button class="btn btn-secondary small-btn" type="button" data-rt2401-review="'+esc(r.id)+'">Review</button><button class="btn btn-secondary small-btn" type="button" data-rt2401-outcomes="'+esc(r.id)+'">View Outcomes</button><button class="btn btn-secondary small-btn" type="button" data-rt2401-export="'+esc(r.id)+'">Export CSV</button></td></tr>').join(''):'<tr><td colspan="10">No saved scenarios yet. Save scenarios, then run the portfolio report.</td></tr>')+
      '</tbody></table></div></div><div class="card" id="rt2401Drilldown"><div class="card-header"><h3>Scenario Drilldown</h3><span>Auditor / Examiner Support</span></div><div class="note-box">Click a scenario row to review summary details. Use View Outcomes to populate 100 Monte Carlo rows below.</div></div><div class="card" id="rt2401Outcomes"><div class="card-header"><h3>Selected Scenario Outcomes</h3><span>Monte Carlo</span></div><div class="note-box">Select View Outcomes to preview 100 Monte Carlo rows for a scenario.</div></div>';
    removeLegacyPortfolioTotals();
    function getRow(id){ return rows.find(r => String(r.id) === String(id)); }
    function selectRow(id, showOutcomes){
      const r = getRow(id); if(!r) return;
      const d = $('#rt2401Drilldown');
      if (d) d.innerHTML = '<div class="card-header"><h3>Scenario Drilldown</h3><span>'+esc(r.type)+'</span></div><div class="form-grid" style="grid-template-columns:repeat(4,minmax(160px,1fr));"><label>Scenario<input readonly value="'+esc(r.name)+'"></label><label>Total Exposure<input readonly value="'+money(r.exposure)+'"></label><label>Residual Risk<input readonly value="'+money(r.residual)+'"></label><label>Insurance Impact<input readonly value="'+money(r.insurance)+'"></label></div><div class="note-box"><strong>Top driver:</strong> '+esc(r.driver)+'</div><p class="muted">The top summary cards are calculated from this same row dataset.</p>';
      if (showOutcomes) renderOutcomes(r);
    }
    $$('.rt2401-row', view).forEach(tr => tr.addEventListener('click', e => { if(e.target.closest('button')) return; selectRow(tr.dataset.rt2401Id,false); }, true));
    $$('[data-rt2401-id].rt2401-heat', view).forEach(b => b.addEventListener('click', e => { e.preventDefault(); selectRow(b.dataset.rt2401Id,false); }, true));
    $$('[data-rt2401-review]', view).forEach(b => b.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); const r=getRow(b.dataset.rt2401Review); if(r) reviewScenario(r); }, true));
    $$('[data-rt2401-outcomes]', view).forEach(b => b.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); selectRow(b.dataset.rt2401Outcomes,true); }, true));
    $$('[data-rt2401-export]', view).forEach(b => b.addEventListener('click', e => { e.preventDefault(); e.stopPropagation(); const r=getRow(b.dataset.rt2401Export); if(r) exportScenarioCsv(r); }, true));
    $('#rt2401RunPortfolio')?.addEventListener('click', e => { e.preventDefault(); renderPortfolioReport(); }, true);
    $('#rt2401SaveSnapshot')?.addEventListener('click', e => {
      e.preventDefault();
      const lines = ['Rank,Scenario,Type,Total Exposure,Residual Risk,Insurance Impact,P90,Top Driver'];
      rows.forEach((r,i)=>lines.push([i+1,r.name,r.type,r.exposure,r.residual,r.insurance,r.p90,r.driver].map(csvCell).join(',')));
      download('portfolio_report_snapshot.csv', lines.join('\n'));
    }, true);
    try { console.log('RiskTool 24.0.08 Portfolio Totals', { totalExposure: totals.exposure, residualRisk: totals.residual, insuranceImpact: totals.insurance, scenarioCount: rows.length }); } catch(e) {}
  }
  const originalActivate = window.activateView || (typeof activateView === 'function' ? activateView : null);
  if (originalActivate) {
    window.activateView = function(viewName){
      const result = originalActivate.apply(this, arguments);
      setTimeout(() => { lockVersion(); if (viewName === 'reports' || viewName === 'portfolio-report') renderPortfolioReport(); }, 120);
      return result;
    };
    try { activateView = window.activateView; } catch(e) {}
  }
  document.addEventListener('click', function(e){
    setTimeout(lockVersion, 30);
    setTimeout(removeLegacyPortfolioTotals, 80);
    setTimeout(removeLegacyPortfolioTotals, 350);
    if (e.target.closest('.nav-item[data-view="reports"],.nav-item[data-view="portfolio-report"],#rt78RunPortfolio,#rt76RunPortfolio,#rt2401RunPortfolio')) {
      setTimeout(renderPortfolioReport, 180);
    }
  }, true);
  function boot(){ lockVersion(); addStyles(); const v=$('#view-reports'); if(v && (!v.classList.contains('hidden')) && (v.classList.contains('active') || location.hash.includes('report') || location.hash.includes('portfolio'))) renderPortfolioReport(); }
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => [50,250,800,1600,3000].forEach(t=>setTimeout(boot,t)));
  else [50,250,800,1600,3000].forEach(t=>setTimeout(boot,t));
  let ticks=0; const timer=setInterval(()=>{ lockVersion(); ticks++; const v=$('#view-reports'); if(v && !v.classList.contains('hidden') && v.dataset.rt2401Portfolio !== 'true' && (v.textContent||'').includes('Total Exposure')) renderPortfolioReport(); if(ticks>20) clearInterval(timer); }, 500);
  try {
    const obs = new MutationObserver(() => {
      const view = $('#view-reports');
      if (view && !view.classList.contains('hidden')) removeLegacyPortfolioTotals();
    });
    const startObs = () => { const view = $('#view-reports'); if(view) obs.observe(view,{childList:true,subtree:true}); };
    if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', startObs); else startObs();
  } catch(e) {}
  window.renderPortfolioReport2401 = renderPortfolioReport;
})();
/* ===== END PHASE 24.0.08 PORTFOLIO TOTALS + VERSION LOCK ===== */


/* ===== PHASE 24.0.08 TEST MODE + MODEL VALIDATION SHELL ===== */
(function(){
  const VERSION = '24.0.08';
  const TEST_MODE_KEY = 'risktool_test_mode_enabled';
  const SCENARIO_KEY = 'risk_manager_scenarios_v431';
  const $ = (s,root=document)=>root.querySelector(s);
  const $$ = (s,root=document)=>Array.from(root.querySelectorAll(s));
  const esc = v => String(v ?? '').replace(/[&<>"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
  const num = v => { const n = Number(String(v ?? 0).replace(/[$,%\s,]/g,'')); return Number.isFinite(n)?n:0; };
  const money = n => new Intl.NumberFormat('en-US',{style:'currency',currency:'USD',maximumFractionDigits:0}).format(num(n));
  function lockVersion(){
    document.title = 'RiskTool ' + VERSION;
    const av=$('#appVersion'); if(av) av.textContent=VERSION;
    $$('.sidebar-note h4').forEach(el=>el.textContent='Phase '+VERSION);
    $$('*').forEach(el=>{ if(el.childNodes && el.childNodes.length===1 && el.childNodes[0].nodeType===3){ const t=el.textContent||''; if(/\b(?:23|24)\.0\.\d+\b/.test(t)) el.textContent=t.replace(/\b(?:23|24)\.0\.\d+\b/g,VERSION); }});
  }
  function getTestMode(){ return localStorage.getItem(TEST_MODE_KEY)==='true'; }
  function setTestMode(v){ localStorage.setItem(TEST_MODE_KEY, v?'true':'false'); ensureModelValidationUI(); renderUserAdminTestMode(); }
  function saved(){ try { if(typeof window.getSavedScenarios==='function') return window.getSavedScenarios()||[]; } catch(e){} try{return JSON.parse(localStorage.getItem(SCENARIO_KEY)||'[]')||[]}catch(e){return[]} }
  function writeSaved(arr){ localStorage.setItem(SCENARIO_KEY, JSON.stringify(arr)); try{ if(typeof window.renderSavedScenarios==='function') window.renderSavedScenarios(); }catch(e){} try{ if(typeof window.renderDashboardOpenTable==='function') window.renderDashboardOpenTable(); }catch(e){} }
  function download(name,text,type='text/plain;charset=utf-8'){ const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([text],{type})); a.download=name; document.body.appendChild(a); a.click(); setTimeout(()=>{URL.revokeObjectURL(a.href);a.remove()},500); }
  function ensureMenu(){
    const nav=$('.nav'); if(!nav) return;
    const reports=$('.nav-item[data-view="reports"]'); if(reports) reports.textContent='Portfolio Report';
    let btn=$('.nav-item[data-view="model-validation"]');
    if(!btn){ btn=document.createElement('button'); btn.className='nav-item'; btn.dataset.view='model-validation'; btn.textContent='Model Validation'; nav.appendChild(btn); btn.addEventListener('click',()=>{ if(typeof window.activateView==='function') window.activateView('model-validation'); else activateLocal('model-validation'); }); }
    btn.style.display=getTestMode()?'':'none';
  }
  function ensureView(){
    if($('#view-model-validation')) return;
    const section=document.createElement('section'); section.className='view hidden'; section.id='view-model-validation';
    section.innerHTML='<div class="section-header"><h2>Model Validation</h2><p>Controlled test portfolio and export tools for spreadsheet comparison and model validation.</p></div><div id="rt2403ModelValidationBody"></div>';
    const main=$('.main')||document.body; main.appendChild(section);
  }
  function activateLocal(view){ $$('.view').forEach(v=>v.classList.add('hidden')); const el=$('#view-'+view); if(el) el.classList.remove('hidden'); $$('.nav-item').forEach(b=>b.classList.toggle('active',b.dataset.view===view)); if(view==='model-validation') renderModelValidation(); }
  const oldActivate = window.activateView || (typeof activateView==='function'?activateView:null);
  if(oldActivate && !window.__rt2403ActivateWrapped){ window.__rt2403ActivateWrapped=true; window.activateView=function(view){ if(view==='model-validation'){ activateLocal(view); return; } const r=oldActivate.apply(this,arguments); setTimeout(()=>{ensureModelValidationUI();},50); return r; }; try{activateView=window.activateView}catch(e){} }
  function renderUserAdminTestMode(){
    const users=$('#view-users'); if(!users) return;
    let card=$('#rt2403TestModeCard');
    if(!card){ card=document.createElement('div'); card.className='card'; card.id='rt2403TestModeCard'; const firstCard=users.querySelector('.card'); users.insertBefore(card, firstCard||users.firstChild); }
    card.innerHTML='<div class="card-header"><h3>Test Mode</h3><span>Controls Model Validation visibility and test portfolio tools.</span></div><div class="form-grid"><label class="help-label">Enable Test Mode<select id="rt2403TestModeToggle"><option value="off">OFF</option><option value="on">ON</option></select></label><div class="note-box full-span">OFF hides Model Validation. ON adds the Model Validation menu item and enables deterministic test portfolio tools.</div></div>';
    const sel=$('#rt2403TestModeToggle'); if(sel){ sel.value=getTestMode()?'on':'off'; sel.onchange=()=>setTestMode(sel.value==='on'); }
  }
  function outcomesFor(base, runs=1000){ const rows=[]; for(let i=1;i<=runs;i++){ const u=((Math.sin((base+i)*12.9898)*43758.5453)%1+1)%1; const annual=Math.round(base*.72 + base*.58*u); const residual=Math.round(annual*.68); rows.push({iteration:i,annualLoss:annual,residualLoss:residual,hardCost:Math.round(annual*.65),softCost:Math.round(annual*.35),totalLoss:annual}); } return rows; }
  function makeScenario(n, product, component, exposure, residual, insurance){ const id='TEST-CORE-'+String(n).padStart(3,'0'); return { id, scenarioId:id, name:product+' - '+component, scenarioName:product+' - '+component, type:'complex', mode:'complex', scenarioStatus:'Open', status:'Open', productGroup:product, primaryProduct:product, totalExposure:exposure, residualRisk:residual, insuranceImpact:insurance, p90:Math.round(residual*1.65), highestDriver:component+' operational control gap', modelVersion:VERSION, simulationRuns:1000, riskItems:[{riskItemId:id+'-RISK-001',name:component+' compliance and operational failure',riskDomain:'Operational / Compliance'}], costs:[{costLossId:id+'-COST-001',costType:'Hard Loss',amountLikely:exposure}], mitigations:[{mitigationId:id+'-MIT-001',name:'Documented control validation',effectiveness:'Evidence based'}], insurance:[{policyName:product+' coverage tower',coverageAmount:insurance}], evidence:[{sourceType:'Internal',amount:exposure,description:'Deterministic validation estimate'}], acceptedRisk:[{authority:'Committee',amount:residual,notes:'Retained risk after controls and transfer'}], monteCarloOutcomes:outcomesFor(residual,1000), generatedSummary:'TEST DATA: deterministic core banking validation scenario for spreadsheet comparison.' }; }
  function testPortfolio(){
    return [
      makeScenario(1,'Deposits','Statement error remediation',814000,586080,227920),
      makeScenario(2,'Loans','HELOC disclosure timing',1265000,759000,506000),
      makeScenario(3,'Payments','ACH file exception processing',642000,372360,269640),
      makeScenario(4,'Digital Banking','Authentication outage response',980000,637000,343000),
      makeScenario(5,'BSA / OFAC','Screening queue backlog',720000,468000,252000),
      makeScenario(6,'Mortgage','Escrow analysis correction',540000,351000,189000),
      makeScenario(7,'Cards','Dispute handling delay',455000,286650,168350),
      makeScenario(8,'Core Operations','GL posting reconciliation failure',690000,414000,276000)
    ];
  }
  function loadTestPortfolio(){ const keep=saved().filter(s=>!String(s.id||s.scenarioId||'').startsWith('TEST-CORE-')); writeSaved(keep.concat(testPortfolio())); alert('Loaded deterministic Core Banking Test Portfolio: 8 scenarios.'); renderModelValidation(); try{ if(window.renderPortfolioReport2401) window.renderPortfolioReport2401(); }catch(e){} }
  function clearTestPortfolio(){ writeSaved(saved().filter(s=>!String(s.id||s.scenarioId||'').startsWith('TEST-CORE-'))); alert('Cleared TEST-CORE scenarios.'); renderModelValidation(); }
  function validationDataset(){ const rows=saved().filter(s=>String(s.id||s.scenarioId||'').startsWith('TEST-CORE-')); return rows.map(s=>({id:s.id||s.scenarioId,name:s.name||s.scenarioName,type:s.type||s.mode,productGroup:s.productGroup||s.primaryProduct,totalExposure:num(s.totalExposure),residualRisk:num(s.residualRisk),insuranceImpact:num(s.insuranceImpact),p90:num(s.p90),simulationRuns:num(s.simulationRuns||1000),modelVersion:s.modelVersion||VERSION})); }
  function exportDataset(){ download('risktool_validation_dataset.json', JSON.stringify(validationDataset(),null,2),'application/json;charset=utf-8'); }
  function csvLine(arr){return arr.map(v=>{const s=String(v??''); return /[",\n]/.test(s)?'"'+s.replace(/"/g,'""')+'"':s;}).join(',')}
  function downloadValidationPack(){
    const data=validationDataset();
    const csv=[csvLine(['id','name','type','productGroup','totalExposure','residualRisk','insuranceImpact','p90','simulationRuns','modelVersion'])].concat(data.map(r=>csvLine([r.id,r.name,r.type,r.productGroup,r.totalExposure,r.residualRisk,r.insuranceImpact,r.p90,r.simulationRuns,r.modelVersion]))).join('\n');
    const readme='RiskTool Model Validation Pack - '+VERSION+'\n\nUse these deterministic TEST-CORE scenarios to compare RiskTool portfolio totals against spreadsheet calculations. Required matches: Total Exposure, Residual Risk, and Insurance Impact.\n';
    const checklist='Monte Carlo Validation Checklist\n1. Load Core Banking Test Portfolio.\n2. Run Portfolio Report.\n3. Export Validation Dataset.\n4. Compare totals to spreadsheet_input_dataset.csv.\n5. Verify allowed simulation runs are 100, 1,000, 10,000, 100,000.\n';
    const bundle='--- model_validation_readme.txt ---\n'+readme+'\n--- spreadsheet_input_dataset.csv ---\n'+csv+'\n--- core_banking_test_scenarios.json ---\n'+JSON.stringify(testPortfolio(),null,2)+'\n--- expected_outputs_reference.csv ---\n'+csv+'\n--- monte_carlo_validation_checklist.txt ---\n'+checklist+'\n--- portfolio_comparison_template.csv ---\nMetric,RiskTool,Spreadsheet,Difference\nTotal Exposure,,,\nResidual Risk,,,\nInsurance Impact,,,\n';
    download('risktool_model_validation_pack.txt', bundle);
  }
  function renderModelValidation(){
    ensureView(); const body=$('#rt2403ModelValidationBody'); if(!body) return;
    const data=validationDataset(); const totals=data.reduce((t,r)=>(t.exposure+=r.totalExposure,t.residual+=r.residualRisk,t.insurance+=r.insuranceImpact,t),{exposure:0,residual:0,insurance:0});
    body.innerHTML='<div class="grid three"><div class="stat-card"><span>Test Scenarios</span><strong>'+data.length+'</strong></div><div class="stat-card"><span>Total Exposure</span><strong>'+money(totals.exposure)+'</strong></div><div class="stat-card"><span>Residual Risk</span><strong>'+money(totals.residual)+'</strong></div></div><div class="card"><div class="card-header"><h3>Validation Tools</h3><span>Deterministic test data for spreadsheet comparison.</span></div><div class="builder-actions"><button class="btn btn-primary" id="rt2403LoadTest">Load Core Banking Test Portfolio</button><button class="btn btn-danger" id="rt2403ClearTest">Clear Test Portfolio</button><button class="btn btn-secondary" id="rt2403Pack">Download Model Validation Pack</button><button class="btn btn-secondary" id="rt2403Export">Export Validation Dataset</button></div><div class="note-box">Test Mode keeps Model Validation visible. The test dataset uses TEST-CORE-### IDs and is deterministic so spreadsheet comparison is repeatable.</div></div><div class="card"><div class="card-header"><h3>Validation Dataset Preview</h3><span>Expected outputs reference</span></div><div class="table-wrap"><table><thead><tr><th>ID</th><th>Name</th><th>Product</th><th>Total Exposure</th><th>Residual Risk</th><th>Insurance Impact</th><th>Runs</th></tr></thead><tbody>'+(data.length?data.map(r=>'<tr><td>'+esc(r.id)+'</td><td>'+esc(r.name)+'</td><td>'+esc(r.productGroup)+'</td><td>'+money(r.totalExposure)+'</td><td>'+money(r.residualRisk)+'</td><td>'+money(r.insuranceImpact)+'</td><td>'+esc(r.simulationRuns)+'</td></tr>').join(''):'<tr><td colspan="7">No TEST-CORE validation scenarios loaded.</td></tr>')+'</tbody></table></div></div>';
    $('#rt2403LoadTest')?.addEventListener('click',loadTestPortfolio);
    $('#rt2403ClearTest')?.addEventListener('click',clearTestPortfolio);
    $('#rt2403Pack')?.addEventListener('click',downloadValidationPack);
    $('#rt2403Export')?.addEventListener('click',exportDataset);
  }
  function remove500Options(){ $$('select option[value="500"]').forEach(o=>o.remove()); }
  function ensureModelValidationUI(){ lockVersion(); ensureMenu(); ensureView(); renderUserAdminTestMode(); remove500Options(); if($('#view-model-validation') && !$('#view-model-validation').classList.contains('hidden')) renderModelValidation(); }
  document.addEventListener('click',e=>{ if(e.target.closest('.nav-item[data-view="users"]')) setTimeout(renderUserAdminTestMode,120); if(e.target.closest('.nav-item[data-view="model-validation"]')) setTimeout(renderModelValidation,60); },true);
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',()=>[50,250,800,1600].forEach(t=>setTimeout(ensureModelValidationUI,t))); else [50,250,800,1600].forEach(t=>setTimeout(ensureModelValidationUI,t));
  window.riskToolTestMode2403 = { loadTestPortfolio, clearTestPortfolio, exportDataset, downloadValidationPack, renderModelValidation };
})();
/* ===== END PHASE 24.0.08 TEST MODE + MODEL VALIDATION SHELL ===== */


/* ===== PHASE 24.0.08 SAVE + DASHBOARD REPORT STABILIZATION =====
   Purpose: restore scenario persistence and dashboard Open Report buttons without changing
   Portfolio Report, Test Mode, Model Validation, or workspace architecture.
*/
(function(){
  const PHASE = "24.0.08";
  const RT_LOCAL_KEY = (typeof STORAGE_KEY !== "undefined" && STORAGE_KEY) ? STORAGE_KEY : "risk_manager_scenarios_v1";

  function q(sel, root){ return (root || document).querySelector(sel); }
  function qa(sel, root){ return Array.from((root || document).querySelectorAll(sel)); }
  function esc(v){ return (typeof escapeHtml === "function") ? escapeHtml(String(v ?? "")) : String(v ?? "").replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])); }
  function moneyOrNumber(v){ return Number(v || 0).toLocaleString(); }

  function forceVersion2404(){
    try { window.RISKTOOL_RUNTIME_VERSION = PHASE; } catch(e) {}
    qa('#appVersion,.app-version,[data-version-label],[data-app-version],#versionDisplay,#footerVersion,#phaseVersion').forEach(el => { el.textContent = PHASE; });
    qa('.sidebar-note h4').forEach(el => { if (/Phase|Version/i.test(el.textContent || '')) el.textContent = 'Phase ' + PHASE; });
    if (document.title) document.title = 'RiskTool ' + PHASE;
  }

  function localReadSaved2404(){
    try {
      const parsed = JSON.parse(localStorage.getItem(RT_LOCAL_KEY) || '[]');
      return Array.isArray(parsed) ? parsed : [];
    } catch(e) { return []; }
  }
  function localWriteSaved2404(items){
    localStorage.setItem(RT_LOCAL_KEY, JSON.stringify(Array.isArray(items) ? items : []));
  }
  function scenarioId2404(existing){
    try { if (typeof generateScenarioId === 'function') return generateScenarioId(existing || []); } catch(e) {}
    return 'SCN-' + Date.now();
  }
  function summarizeForRows2404(s){
    const inherent = Number(s.inherent || s.total || s.totalExposure || s.hardCostLikely || s.betaExpectedValue || 0);
    const residual = Number(s.residual || s.residualRisk || Math.max(0, inherent - Number(s.insuranceImpact || 0)) || 0);
    return Object.assign({}, s, {
      id: String(s.id || scenarioId2404([])),
      name: String(s.name || s.scenarioName || s.projectOrProductName || 'Unnamed Scenario'),
      mode: String(s.mode || 'single'),
      productGroup: String(s.productGroup || s.primaryProduct || s.productFamily || ''),
      riskDomain: String(s.riskDomain || s.highestDriver || ''),
      scenarioStatus: String(s.scenarioStatus || s.status || 'Open'),
      identifiedDate: String(s.identifiedDate || ''),
      inherent,
      residual,
      totalExposure: Number(s.totalExposure || inherent || 0),
      residualRisk: Number(s.residualRisk || residual || 0)
    });
  }

  // Undo the prior workspace-gate override that returned [] unless a folder was connected.
  window.getSavedScenarios = getSavedScenarios = function(){
    return localReadSaved2404().map(summarizeForRows2404);
  };
  window.setSavedScenarios = setSavedScenarios = function(items){
    localWriteSaved2404((Array.isArray(items) ? items : []).map(summarizeForRows2404));
  };

  async function optionallyWriteWorkspaceCopy2404(scenario){
    // Best-effort only. Local persistence remains the primary working path for this stabilization patch.
    try {
      if (!(typeof workspaceFolderHandle !== 'undefined' && workspaceFolderHandle)) return;
      const dir = await workspaceFolderHandle.getDirectoryHandle('scenarios', { create: true });
      const safeId = String(scenario.id || scenarioId2404([])).replace(/[^a-zA-Z0-9_-]/g, '_');
      const fh = await dir.getFileHandle(safeId + '.json', { create: true });
      const writable = await fh.createWritable();
      await writable.write(JSON.stringify(scenario, null, 2));
      await writable.close();
      try {
        const all = getSavedScenarios();
        const idx = await dir.getFileHandle('index.json', { create: true });
        const iw = await idx.createWritable();
        await iw.write(JSON.stringify(all, null, 2));
        await iw.close();
      } catch(e) {}
    } catch(e) { console.warn('24.0.08 workspace copy skipped:', e); }
  }

  function readCurrentPayload2404(){
    const activeViewEl = q('.view.active');
    const viewName = activeViewEl?.id?.replace(/^view-/, '') || (typeof activeMode !== 'undefined' ? activeMode : 'single');
    if (viewName === 'beta' || (typeof activeMode !== 'undefined' && activeMode === 'beta')) {
      return (typeof getBetaPayload === 'function') ? getBetaPayload() : null;
    }
    if (viewName === 'complex' || (typeof activeMode !== 'undefined' && activeMode === 'complex')) {
      return (typeof getComplexPayload === 'function') ? getComplexPayload() : null;
    }
    return (typeof getSinglePayload === 'function') ? getSinglePayload() : null;
  }

  window.saveScenario = saveScenario = async function(event){
    if (event?.preventDefault) event.preventDefault();
    if (event?.stopPropagation) event.stopPropagation();
    const payload = readCurrentPayload2404();
    if (!payload) { alert('No active scenario data found to save.'); return null; }
    const saved = getSavedScenarios();
    if (!payload.id) payload.id = scenarioId2404(saved);
    const idField = payload.mode === 'complex' ? q('#complexScenarioId') : payload.mode === 'beta' ? q('#betaScenarioId') : q('#singleScenarioId');
    if (idField) idField.value = payload.id;

    let fullScenario = summarizeForRows2404(payload);
    try {
      const existing = saved.find(x => String(x.id || '') === String(fullScenario.id || ''));
      if (typeof applyOwnershipMetadata === 'function') fullScenario = applyOwnershipMetadata(fullScenario, existing);
    } catch(e) {}

    const idx = saved.findIndex(x => String(x.id || '') === String(fullScenario.id || ''));
    if (idx >= 0) saved[idx] = fullScenario; else saved.unshift(fullScenario);
    setSavedScenarios(saved);

    try { lastSummary = (typeof summarizePayload === 'function') ? summarizePayload(fullScenario) : fullScenario; } catch(e) { try { lastSummary = fullScenario; } catch(_){} }
    try { if (typeof renderScenarioSummary === 'function') renderScenarioSummary(lastSummary || fullScenario); } catch(e) {}
    try { if (typeof renderMonteCarloTable === 'function') renderMonteCarloTable(lastSummary || fullScenario); } catch(e) {}
    try { if (typeof renderCharts === 'function') renderCharts(lastSummary || fullScenario); } catch(e) {}
    try { renderSavedScenarios(); } catch(e) {}
    try { renderDashboardOpenTable(); } catch(e) {}
    try { if (typeof refreshLibraries === 'function') refreshLibraries(); } catch(e) {}
    optionallyWriteWorkspaceCopy2404(fullScenario);
    const status = q('#scenarioFileStatus') || q('#workspaceSetupStatus');
    if (status) status.textContent = 'Saved scenario changes: ' + fullScenario.id;
    return fullScenario;
  };

  window.saveBetaScenario = saveBetaScenario = saveScenario;

  window.renderSavedScenarios = renderSavedScenarios = function(){
    const tbody = q('#savedEvaluationsBody');
    const countEl = q('#savedCount');
    if (!tbody) return;
    const saved = getSavedScenarios();
    if (countEl) countEl.textContent = String(saved.length);
    if (!saved.length) { tbody.innerHTML = '<tr><td colspan="11">No saved scenarios yet.</td></tr>'; return; }
    tbody.innerHTML = saved.map(s => `<tr>
      <td>${esc(s.id)}</td><td>${esc(s.name)}</td>
      <td>${s.mode === 'single' ? 'Single' : s.mode === 'complex' ? 'Complex' : s.mode === 'beta' ? 'Beta' : 'Unknown'}</td>
      <td>${esc(s.productGroup)}</td><td>${esc(s.scenarioStatus)}</td>
      <td>${esc(s.assignedOwnerName || '')}</td><td>${esc(s.storageMode || 'Local Browser Storage')}</td>
      <td>${Number(s.inherent || 0)}</td><td>${Number(s.residual || 0)}</td><td>${esc(s.identifiedDate)}</td>
      <td><button class="btn btn-secondary small-btn" data-action="open" data-id="${esc(s.id)}">Open</button>
          <button class="btn btn-secondary small-btn" data-action="delete" data-id="${esc(s.id)}">Delete</button></td>
    </tr>`).join('');
    qa('button[data-action="open"]', tbody).forEach(btn => btn.addEventListener('click', () => { if (typeof openScenario === 'function') openScenario(btn.dataset.id); }));
    qa('button[data-action="delete"]', tbody).forEach(btn => btn.addEventListener('click', () => { if (typeof deleteScenario === 'function') deleteScenario(btn.dataset.id); }));
    try { if (typeof renderHeatMap === 'function') renderHeatMap(); } catch(e) {}
  };

  window.renderDashboardOpenTable = renderDashboardOpenTable = function(){
    const tbody = q('#dashboardOpenScenarioBody');
    if (!tbody) return;
    const rows = getSavedScenarios().filter(s => String(s.scenarioStatus || '').toLowerCase() !== 'closed')
      .sort((a,b) => Number(b.residual || b.inherent || 0) - Number(a.residual || a.inherent || 0));
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="11">No open scenarios available.</td></tr>'; return; }
    tbody.innerHTML = rows.map(s => `<tr>
      <td><button class="scenario-link" data-open-id="${esc(s.id)}">${esc(s.id)}</button></td>
      <td>${s.mode === 'single' ? 'Single Scenario' : s.mode === 'complex' ? 'Complex Scenario' : s.mode === 'beta' ? 'Beta Scenario' : 'Unknown'}</td>
      <td>${esc(s.name)}</td><td>${esc(s.scenarioStatus)}</td><td>${Number(s.inherent || 0)}</td>
      <td>${esc(s.identifiedDate)}</td><td>${esc(s.productGroup)}</td><td>${esc(s.riskDomain)}</td>
      <td>${esc(s.assignedOwnerName || '')}</td><td>${esc(s.storageMode || 'Local Browser Storage')}</td>
      <td><button class="btn btn-secondary small-btn" data-report-id="${esc(s.id)}">Open Report</button></td>
    </tr>`).join('');
    qa('[data-open-id]', tbody).forEach(btn => btn.addEventListener('click', () => { if (typeof openScenario === 'function') openScenario(btn.dataset.openId); }));
    qa('[data-report-id]', tbody).forEach(btn => btn.addEventListener('click', () => { if (typeof openScenarioReport === 'function') openScenarioReport(btn.dataset.reportId); }));
  };

  // Capture clicks even if another render function created the row without listeners.
  document.addEventListener('click', function(e){
    const reportBtn = e.target.closest && e.target.closest('[data-report-id]');
    if (reportBtn) { e.preventDefault(); e.stopPropagation(); if (typeof openScenarioReport === 'function') openScenarioReport(reportBtn.dataset.reportId); return; }
    const saveBtn = e.target.closest && e.target.closest('#saveScenarioBtn,[data-action="save-scenario"]');
    if (saveBtn) { saveScenario(e); }
  }, true);
  ...
  setInterval(forceVersion2404, 1000);
  setTimeout(function(){ try { renderSavedScenarios(); renderDashboardOpenTable(); forceVersion2404(); } catch(e){} }, 300);
})();
/* ===== END PHASE 24.0.08 SAVE + DASHBOARD REPORT STABILIZATION ===== */


/* ===== PHASE 24.0.08 FIX PACK =====
   Purpose: targeted fixes only.
   1) Dashboard "Open Report" now lands on the current Portfolio Report screen (rt2401 renderer)
      and drills into the selected scenario, instead of the legacy Report Options screen.
      Root cause: a later runtime-rebase patch reassigned window.activateView and dropped the
      Portfolio Report render hook, so programmatic navigation never rendered the report.
   2) New scenarios no longer default to "Closed" status. Scenario Status lists are alphabetized,
      so "Closed" was the silent default for new Single/Complex scenarios; the Dashboard Open
      Scenarios table filters out Closed rows, which made successful saves look like failures.
      New (unsaved) scenarios now default to "Open". User-chosen statuses are never overridden.
   3) The builder "Category" field now pulls the full Category Admin list. The category dropdown
      override only read user-added localStorage values (and window.productGroups, which was
      undefined because productGroups is a top-level let). It now merges DEFAULT_PRODUCT_GROUPS
      with stored values, exactly matching what Category Admin displays, and window.productGroups
      is kept in sync so the existing dropdown override uses the same complete list.
   4) Backstop: clicking "Not Now" on the post-login reconnect prompt always hides the
      #rtReconnectWorkspace29 overlay (older close helpers referenced a misspelled id).
*/
(function(){
  const PHASE = '24.0.08';
  ...
  /* ---------- Fix 3: Category field = full Category Admin list ---------- */
  function mergedCategoryList(){
    let vals = [];
    try { if (Array.isArray(DEFAULT_PRODUCT_GROUPS)) vals = vals.concat(DEFAULT_PRODUCT_GROUPS); } catch(e) {}
    ['risk_manager_product_groups_v431','risk_manager_product_groups'].forEach(k => {
      try { const x = JSON.parse(localStorage.getItem(k) || '[]'); if (Array.isArray(x)) vals = vals.concat(x); } catch(e) {}
    });
    return Array.from(new Set(vals.map(v => String(v || '').trim()).filter(Boolean)))
      .sort((a,b) => a.localeCompare(b, undefined, { sensitivity: 'base' }));
  }
  function syncCategoryGlobals(){
    // The rt68 category override reads window.productGroups; the top-level `let productGroups`
    // never attached to window, so keep an explicit, complete copy here.
    try { window.productGroups = mergedCategoryList(); } catch(e) {}
  }
  function fillCategorySelect(sel, list){
    if (!sel || !list.length) return;
    const current = sel.value || sel.dataset.rt68Value || '';
    const escFn = (typeof escapeHtml === 'function') ? escapeHtml : (v => String(v).replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c])));
    sel.innerHTML = list.map(v => '<option value="' + escFn(v) + '">' + escFn(v) + '</option>').join('');
    if (current && list.includes(current)) sel.value = current;
    sel.dataset.rt68Value = sel.value;
  }
  function applyCategoryFix(){
    syncCategoryGlobals();
    const list = mergedCategoryList();
    ['singleRiskDomain','complexRiskDomain','betaRiskDomain','complexSectionProduct','riskItemProduct']
      .forEach(id => fillCategorySelect(q('#' + id), list));
  }

  /* ---------- Fix 2: sensible default status for NEW scenarios ---------- */
  function applyStatusDefaults(){
    [['singleScenarioStatus','singleScenarioId'],
     ['complexScenarioStatus','complexScenarioId'],
     ['betaScenarioStatus','betaScenarioId']].forEach(([selId, idId]) => {
      const sel = q('#' + selId);
      if (!sel || sel.dataset.rt2405Touched === '1') return;
      const idField = q('#' + idId);
      if (idField && String(idField.value || '').trim()) return; // editing an existing scenario: never override
      const opts = Array.from(sel.options).map(o => o.value);
      if (opts.includes('Open') && (!sel.value || sel.value === 'Closed')) sel.value = 'Open';
    });
  }
  document.addEventListener('change', e => {
    const t = e.target;
    if (t && /ScenarioStatus$/.test(t.id || '')) t.dataset.rt2405Touched = '1';
  }, true);

  /* ---------- re-apply after every library refresh (admin adds, saves, imports) ---------- */
  try {
    const prevRefresh = (typeof window.refreshLibraries === 'function') ? window.refreshLibraries
      : (typeof refreshLibraries === 'function' ? refreshLibraries : null);
    if (prevRefresh) {
      window.refreshLibraries = function(){
        const r = prevRefresh.apply(this, arguments);
        try { applyCategoryFix(); applyStatusDefaults(); } catch(e) {}
        return r;
      };
      try { refreshLibraries = window.refreshLibraries; } catch(e) {}
    }
  } catch(e) {}

  /* ---------- Fix 1: Open Report -> Portfolio Report + scenario drilldown ---------- */
  function drillIntoScenario(id){
    const view = q('#view-reports');
    if (!view) return false;
    const row = qa('.rt2401-row', view).find(tr => String(tr.dataset.rt2401Id || '') === String(id));
    if (!row) return false;
    const outcomesBtn = row.querySelector('[data-rt2401-outcomes]');
    if (outcomesBtn) outcomesBtn.click(); else row.click();
    try { (q('#rt2401Drilldown') || row).scrollIntoView({ behavior: 'smooth', block: 'start' }); } catch(e) {}
    return true;
  }
  function openScenarioReport2405(id){
    try { if (typeof activateView === 'function') activateView('reports'); } catch(e) {}
    const render = () => { try { if (typeof window.renderPortfolioReport2401 === 'function') window.renderPortfolioReport2401(); } catch(e) {} };
    render();
    setTimeout(() => { if (!drillIntoScenario(id)) { render(); drillIntoScenario(id); } }, 200);
    setTimeout(() => { drillIntoScenario(id); }, 500);
  }
  window.openScenarioReport = openScenarioReport2405;
  try { openScenarioReport = openScenarioReport2405; } catch(e) {}

  /* ---------- Fix 4: reconnect overlay backstop ---------- */
  document.addEventListener('click', e => {
    if (e.target.closest && e.target.closest('#rtSkipWorkspace29')) {
      setTimeout(() => { const m = q('#rtReconnectWorkspace29'); if (m) m.style.display = 'none'; }, 50);
    }
  }, true);

  /* ---------- boot ---------- */
  function boot(){ try { applyCategoryFix(); applyStatusDefaults(); } catch(e) {} }
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => [120, 450, 1300, 2600].forEach(t => setTimeout(boot, t)));
  } else {
    [120, 450, 1300, 2600].forEach(t => setTimeout(boot, t));
  }
})();
/* ===== END PHASE 24.0.08 FIX PACK ===== */


/* ===== PHASE 24.0.08 STABILIZATION AUDIT PATCH =====
   Scope: repair core broken behavior only. No enhancements.
   Fixes:
   1) One runtime version: 24.0.08.
   2) Category dropdowns use the full Category Admin category set.
   3) Save buttons persist the complete scenario payload for Single, Complex, and Beta.
   4) Dashboard Open Report routes to the current Portfolio Report renderer, not the legacy report.
*/
(function(){
  'use strict';
  const VERSION = '24.0.08';
  const SCENARIO_KEY = (typeof STORAGE_KEY !== 'undefined' && STORAGE_KEY) ? STORAGE_KEY : 'risk_manager_scenarios_v431';
  const LEGACY_KEY = (typeof LEGACY_STORAGE_KEY !== 'undefined' && LEGACY_STORAGE_KEY) ? LEGACY_STORAGE_KEY : 'risk_manager_saved_evaluations_v2';
  ...
  function lockVersion(){
    try { window.RISKTOOL_RUNTIME_VERSION = VERSION; } catch(e) {}
    qa('#appVersion,.app-version,[data-version-label]').forEach(el => { el.textContent = VERSION; });
    qa('.sidebar-note h4').forEach(el => { el.textContent = 'Phase ' + VERSION; });
  }

  function readList(key){
    try { const x = JSON.parse(localStorage.getItem(key) || '[]'); return Array.isArray(x) ? x : []; } catch(e) { return []; }
  }
  function writeScenarioList(items){
    const list = Array.isArray(items) ? items : [];
    try { localStorage.setItem(SCENARIO_KEY, JSON.stringify(list)); } catch(e) { console.error(e); }
    // keep legacy key synced because older report/dashboard patches still read it in some paths
    try { localStorage.setItem(LEGACY_KEY, JSON.stringify(list)); } catch(e) {}
  }
  function scenarioList(){
    let list = [];
    try { if (typeof getSavedScenarios === 'function') list = getSavedScenarios() || []; } catch(e) {}
    if (!Array.isArray(list) || !list.length) list = readList(SCENARIO_KEY);
    if (!Array.isArray(list) || !list.length) list = readList(LEGACY_KEY);
  ...
  function fullCategoryList(){
    let vals = [];
    try { if (Array.isArray(DEFAULT_PRODUCT_GROUPS)) vals = vals.concat(DEFAULT_PRODUCT_GROUPS); } catch(e) {}
    try { if (Array.isArray(DEFAULT_PRODUCTS)) vals = vals.concat(DEFAULT_PRODUCTS); } catch(e) {}
    try { if (Array.isArray(productGroups)) vals = vals.concat(productGroups); } catch(e) {}
    try { if (Array.isArray(products)) vals = vals.concat(products); } catch(e) {}
    [
      'risk_manager_product_groups_v431',
      'risk_manager_products_v431',
      'risk_manager_product_groups',
      'risk_manager_products'
    ].forEach(k => { vals = vals.concat(readList(k)); });
    // preserve anything already present in the visible selects
    ['singleRiskDomain','complexRiskDomain','betaRiskDomain','singlePrimaryProduct','complexPrimaryProduct','complexSectionProduct','riskItemProduct'].forEach(id => {
      const sel = q('#' + id);
      if (sel) vals = vals.concat(Array.from(sel.options).map(o => o.value || o.textContent));
    });
    return Array.from(new Set(vals.map(v => String(v || '').trim()).filter(Boolean)))
      .sort((a,b) => a.localeCompare(b, undefined, {sensitivity:'base'}));
  }
  function fillSelect(id, list){
    const sel = q('#' + id); if (!sel || !list.length) return;
    const current = sel.value || sel.dataset.rtSavedValue || '';
    sel.innerHTML = list.map(v => '<option value="' + esc(v) + '">' + esc(v) + '</option>').join('');
    if (current && list.includes(current)) sel.value = current;
    else if (!sel.value && list.includes('Core')) sel.value = 'Core';
    sel.dataset.rtSavedValue = sel.value;
  }
  function repairCategories(){
    const list = fullCategoryList();
    try { window.productGroups = list.slice(); } catch(e) {}
    try { if (Array.isArray(productGroups)) productGroups = list.slice(); } catch(e) {}
    // These are scenario-level Category fields even though older ids still say RiskDomain/PrimaryProduct.
    ['singleRiskDomain','complexRiskDomain','betaRiskDomain','singlePrimaryProduct','complexPrimaryProduct','complexSectionProduct','riskItemProduct'].forEach(id => fillSelect(id, list));
  }

  function isLoggedIn(){
    try { return typeof isUserLoggedIn !== 'function' || isUserLoggedIn(); } catch(e) { return true; }
  }
  function showLogin(){ try { if (typeof showLoginGate === 'function') showLoginGate(); } catch(e) {} }
  function currentBuilder(){
    if (q('#view-beta.active')) return 'beta';
    if (q('#view-complex.active')) return 'complex';
    if (q('#view-single.active')) return 'single';
    try { return String(activeMode || 'single'); } catch(e) { return 'single'; }
  }
  function makeId(list){
    try { if (typeof generateScenarioId === 'function') return generateScenarioId(list); } catch(e) {}
    const d = new Date();
    const stamp = String(d.getFullYear()) + String(d.getMonth()+1).padStart(2,'0') + String(d.getDate()).padStart(2,'0');
    return stamp + '-' + String(list.length + 1).padStart(5,'0');
  }
  function getPayloadFor(mode){
    if (mode === 'beta') return getBetaPayload();
    if (mode === 'complex') {
      // Save what is already on screen; do not force-add a blank component.
      return getComplexPayload();
    }
    return getSinglePayload();
  }
  function idSelectorFor(mode){ return mode === 'beta' ? '#betaScenarioId' : mode === 'complex' ? '#complexScenarioId' : '#singleScenarioId'; }
  function enrichRecord(payload, existing){
    let p = Object.assign({}, payload || {});
    try { if (typeof applyOwnershipMetadata === 'function') p = applyOwnershipMetadata(p, existing); } catch(e) {}
    let summary = {};
    try { if (typeof summarizePayload === 'function') summary = summarizePayload(p) || {}; } catch(e) {}
    // summary has calculated values; payload has the full editable detail. Keep both.
    let merged = Object.assign({}, p, summary, {
      id: p.id || summary.id,
      mode: p.mode || summary.mode || currentBuilder(),
      name: p.name || summary.name || 'Unnamed Scenario',
      productGroup: p.productGroup || summary.productGroup || p.primaryProduct || '',
      riskDomain: p.riskDomain || summary.riskDomain || p.primaryProduct || '',
      primaryProduct: p.primaryProduct || summary.primaryProduct || p.riskDomain || '',
      scenarioStatus: p.scenarioStatus || summary.scenarioStatus || 'Open',
      scenarioSource: p.scenarioSource || summary.scenarioSource || '',
      insurance: Array.isArray(p.insurance) ? p.insurance : (Array.isArray(summary.insurance) ? summary.insurance : []),
      hardFacts: Array.isArray(p.hardFacts) ? p.hardFacts : (Array.isArray(summary.hardFacts) ? summary.hardFacts : []),
      mitigations: Array.isArray(p.mitigations) ? p.mitigations : (Array.isArray(summary.mitigations) ? summary.mitigations : []),
      items: Array.isArray(p.items) ? p.items : (Array.isArray(summary.items) ? summary.items : []),
      productSections: Array.isArray(p.productSections) ? p.productSections : (Array.isArray(summary.productSections) ? summary.productSections : []),
      acceptedRiskEntries: Array.isArray(p.acceptedRiskEntries) ? p.acceptedRiskEntries : (Array.isArray(summary.acceptedRiskEntries) ? summary.acceptedRiskEntries : [])
    });
    try { if (typeof normalizeScenario === 'function') merged = Object.assign({}, normalizeScenario(merged), merged); } catch(e) {}
    return merged;
  }
  function refreshAfterSave(){
    ['renderSavedScenarios','renderDashboardOpenTable','renderHeatMap','updateMonteCarloStatus'].forEach(fn => { try { if (typeof window[fn] === 'function') window[fn](); } catch(e) {} });
    try { if (typeof renderSavedScenarios === 'function') renderSavedScenarios(); } catch(e) {}
    try { if (typeof renderDashboardOpenTable === 'function') renderDashboardOpenTable(); } catch(e) {}
    lockVersion(); repairCategories();
  }
  function saveCurrentScenario(e, forcedMode){
    if (e) { e.preventDefault(); e.stopImmediatePropagation(); }
    if (!isLoggedIn()) { showLogin(); return false; }
    repairCategories();
    const mode = forcedMode || currentBuilder();
    let payload;
    try { payload = getPayloadFor(mode); } catch(err) { console.error(err); alert('Scenario could not be read from the screen: ' + (err.message || err)); return false; }
    const list = scenarioList();
    if (!payload.id) {
      payload.id = makeId(list);
      const idEl = q(idSelectorFor(mode)); if (idEl) idEl.value = payload.id;
    }
    const existing = list.find(x => String(x.id || '') === String(payload.id || ''));
    const record = enrichRecord(payload, existing);
    const idx = list.findIndex(x => String(x.id || '') === String(record.id || ''));
    if (idx >= 0) list[idx] = record; else list.unshift(record);
    writeScenarioList(list);
    try { if (typeof setSavedScenarios === 'function') setSavedScenarios(list); } catch(e) {}
    refreshAfterSave();
    try { if (mode === 'beta' && typeof setBetaOutputs === 'function') setBetaOutputs(record); } catch(e) {}
    try { if (typeof activateView === 'function') activateView(mode); } catch(e) {}
    alert((mode.charAt(0).toUpperCase() + mode.slice(1)) + ' scenario saved.');
    return false;
  }

  function renderDashboardRows(){
    const tbody = q('#dashboardOpenScenarioBody'); if (!tbody) return;
    const rows = scenarioList().filter(s => String(s.scenarioStatus || 'Open').toLowerCase() !== 'closed')
      .sort((a,b) => Number(b.residual || b.inherent || b.betaP90 || 0) - Number(a.residual || a.inherent || a.betaP90 || 0));
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="11">No open scenarios available.</td></tr>'; return; }
    tbody.innerHTML = rows.map(s => '<tr>'+
      '<td><button class="scenario-link" data-open-id="'+esc(s.id)+'">'+esc(s.id)+'</button></td>'+
      '<td>'+esc(s.mode === 'complex' ? 'Complex Scenario' : s.mode === 'beta' ? 'Beta Scenario' : 'Single Scenario')+'</td>'+
      '<td>'+esc(s.name || 'Unnamed')+'</td><td>'+esc(s.scenarioStatus || 'Open')+'</td><td>'+esc(s.inherent || '')+'</td>'+
      '<td>'+esc(s.identifiedDate || '')+'</td><td>'+esc(s.productGroup || s.primaryProduct || '')+'</td><td>'+esc(s.riskDomain || s.primaryProduct || '')+'</td>'+
      '<td>'+esc(s.assignedOwnerName || '')+'</td><td>'+esc(s.storageMode || 'Local Workspace')+'</td>'+
      '<td><button class="btn btn-secondary small-btn" type="button" data-report-id="'+esc(s.id)+'">Open Report</button></td></tr>').join('');
  }
  try { window.renderDashboardOpenTable = renderDashboardOpenTable = renderDashboardRows; } catch(e) { window.renderDashboardOpenTable = renderDashboardRows; }

  function openCurrentPortfolioReport(id){
    try { if (typeof activateView === 'function') activateView('reports'); } catch(e) {}
    const doRender = () => { try { if (typeof window.renderPortfolioReport2401 === 'function') window.renderPortfolioReport2401(); } catch(e) {} };
    doRender();
    const drill = () => {
      doRender();
      const view = q('#view-reports'); if (!view) return;
      const row = qa('.rt2401-row,.rt78-row', view).find(tr => String(tr.dataset.rt2401Id || tr.dataset.id || '') === String(id));
      if (row) {
        const btn = row.querySelector('[data-rt2401-outcomes],[data-view-outcomes]');
        if (btn) btn.click(); else row.click();
        try { row.scrollIntoView({behavior:'smooth', block:'center'}); } catch(e) {}
      }
    };
    setTimeout(drill, 150); setTimeout(drill, 500);
  }
  window.openScenarioReport = openCurrentPortfolioReport;
  try { openScenarioReport = openCurrentPortfolioReport; } catch(e) {}

  function wire(){
    lockVersion(); repairCategories();
    const saveBtn = q('#saveScenarioBtn'); if (saveBtn) { saveBtn.type = 'button'; saveBtn.onclick = e => saveCurrentScenario(e); }
    const betaBtn = q('#saveBetaScenarioBtn'); if (betaBtn) { betaBtn.type = 'button'; betaBtn.onclick = e => saveCurrentScenario(e, 'beta'); }
    ['#addComplexScenarioBtn','#rt54SaveAll'].forEach(sel => { const b=q(sel); if (b) { b.type='button'; b.onclick = e => saveCurrentScenario(e, 'complex'); } });
    renderDashboardRows();
  }

  document.addEventListener('click', function(e){
    const t = e.target;
    if (t.closest && t.closest('#saveScenarioBtn')) return saveCurrentScenario(e);
    if (t.closest && t.closest('#saveBetaScenarioBtn')) return saveCurrentScenario(e, 'beta');
    if (t.closest && t.closest('#addComplexScenarioBtn,#rt54SaveAll')) return saveCurrentScenario(e, 'complex');
    const rpt = t.closest && t.closest('[data-report-id]');
    if (rpt) { e.preventDefault(); e.stopImmediatePropagation(); openCurrentPortfolioReport(rpt.dataset.reportId); return false; }
    setTimeout(wire, 80);
  }, true);

  try {
    const priorRefresh = (typeof refreshLibraries === 'function') ? refreshLibraries : null;
    if (priorRefresh) {
      window.refreshLibraries = refreshLibraries = function(){ const r = priorRefresh.apply(this, arguments); setTimeout(repairCategories, 0); return r; };
    }
  } catch(e) {}

  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => [50,250,900,1800].forEach(t => setTimeout(wire,t)));
  else [50,250,900,1800].forEach(t => setTimeout(wire,t));
  setInterval(lockVersion, 1200);
})();
/* ===== END PHASE 24.0.08 STABILIZATION AUDIT PATCH ===== */


/* ===== PHASE 24.0.08 SAVE SYSTEM HARD RESET =====
   Scope: stabilization only.
   Fixes:
   1) Save buttons no longer depend on workspace-folder connection.
   2) getSavedScenarios/setSavedScenarios always use browser storage as the source of truth.
   3) Dashboard Open Report routes to the current Portfolio Report screen.
   4) Version display is locked to 24.0.08.
*/
(function(){
  'use strict';
  const VERSION = '24.0.08';
  const SCENARIO_KEY = 'risk_manager_scenarios_v431';
  const LEGACY_KEY = 'risk_manager_saved_evaluations_v2';
  const q = (s, r=document) => r.querySelector(s);
  const qa = (s, r=document) => Array.from(r.querySelectorAll(s));
  const esc = v => String(v ?? '').replace(/[&<>"']/g, ch => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));
  ...
  function lockVersion(){
    try { window.RISKTOOL_RUNTIME_VERSION = VERSION; } catch(e) {}
    ['appVersion','versionDisplay','footerVersion','phaseVersion'].forEach(id => { const el = q('#'+id); if (el) el.textContent = VERSION; });
    qa('.app-version,[data-version-label]').forEach(el => el.textContent = VERSION);
    qa('.sidebar-note h4').forEach(el => el.textContent = 'Phase ' + VERSION);
  }
  function read(key){ try { const x = JSON.parse(localStorage.getItem(key) || '[]'); return Array.isArray(x) ? x : []; } catch(e){ return []; } }
  function write(list){
    const arr = Array.isArray(list) ? list : [];
    localStorage.setItem(SCENARIO_KEY, JSON.stringify(arr));
    localStorage.setItem(LEGACY_KEY, JSON.stringify(arr));
  }
  function saved(){
    const primary = read(SCENARIO_KEY);
    if (primary.length) return primary;
    const legacy = read(LEGACY_KEY);
    if (legacy.length) { write(legacy); return legacy; }
    return [];
  }

  // Hard reset scenario storage APIs after all older workspace-folder patches have loaded.
  window.getSavedScenarios = function(){ return saved(); };
  window.setSavedScenarios = function(items){ write(items); return saved(); };
  try { getSavedScenarios = window.getSavedScenarios; } catch(e) {}
  try { setSavedScenarios = window.setSavedScenarios; } catch(e) {}

  function makeId(list){
    const d = new Date();
    const stamp = String(d.getFullYear()) + String(d.getMonth()+1).padStart(2,'0') + String(d.getDate()).padStart(2,'0');
    const nums = (list||[]).map(x => String(x.id||'')).filter(x => x.startsWith(stamp+'-')).map(x => Number(x.split('-')[1])).filter(Number.isFinite);
    return stamp + '-' + String((nums.length ? Math.max(...nums) : 0) + 1).padStart(5,'0');
  }
  ...
  function getPayload(mode){
    if (mode === 'beta' && typeof getBetaPayload === 'function') return getBetaPayload();
    if (mode === 'complex' && typeof getComplexPayload === 'function') return getComplexPayload();
    if (typeof getSinglePayload === 'function') return getSinglePayload();
    throw new Error('No scenario payload function is available for ' + mode);
  }
  function idEl(mode){ return q(mode === 'beta' ? '#betaScenarioId' : mode === 'complex' ? '#complexScenarioId' : '#singleScenarioId'); }
  function enrich(payload, existing, mode){
    let p = Object.assign({}, payload || {});
    try { if (typeof applyOwnershipMetadata === 'function') p = applyOwnershipMetadata(p, existing); } catch(e) {}
    let sum = {};
    try { if (typeof summarizePayload === 'function') sum = summarizePayload(p) || {}; } catch(e) {}
    const merged = Object.assign({}, p, sum, {
      id: p.id || sum.id,
      mode: p.mode || sum.mode || mode,
      name: p.name || sum.name || 'Unnamed Scenario',
      scenarioStatus: p.scenarioStatus || sum.scenarioStatus || 'Open',
      scenarioSource: p.scenarioSource || sum.scenarioSource || '',
      productGroup: p.productGroup || sum.productGroup || p.primaryProduct || p.riskDomain || '',
      primaryProduct: p.primaryProduct || sum.primaryProduct || p.productGroup || p.riskDomain || '',
      riskDomain: p.riskDomain || sum.riskDomain || p.productGroup || p.primaryProduct || '',
      storageMode: 'Local Browser Storage',
      lastSavedAt: new Date().toISOString(),
      items: Array.isArray(p.items) ? p.items : (Array.isArray(sum.items) ? sum.items : []),
      productSections: Array.isArray(p.productSections) ? p.productSections : (Array.isArray(sum.productSections) ? sum.productSections : []),
      hardFacts: Array.isArray(p.hardFacts) ? p.hardFacts : (Array.isArray(sum.hardFacts) ? sum.hardFacts : []),
      mitigations: Array.isArray(p.mitigations) ? p.mitigations : (Array.isArray(sum.mitigations) ? sum.mitigations : []),
      insurance: Array.isArray(p.insurance) ? p.insurance : (Array.isArray(sum.insurance) ? sum.insurance : []),
      acceptedRiskEntries: Array.isArray(p.acceptedRiskEntries) ? p.acceptedRiskEntries : (Array.isArray(sum.acceptedRiskEntries) ? sum.acceptedRiskEntries : [])
    });
    return merged;
  }
  function refresh(){
    ...
    try { if (typeof renderSavedScenarios === 'function') renderSavedScenarios(); } catch(e) {}
    try { if (typeof renderDashboardOpenTable === 'function') renderDashboardOpenTable(); } catch(e) {}
    try { if (typeof window.renderPortfolioReport2401 === 'function') window.renderPortfolioReport2401(); } catch(e) {}
    lockVersion();
  }
  function doSave(event, forcedMode){
    if (event) { event.preventDefault(); event.stopPropagation(); if (event.stopImmediatePropagation) event.stopImmediatePropagation(); }
    const mode = forcedMode || activeBuilder();
    let payload;
    try { payload = getPayload(mode); } catch(err) { console.error(err); alert('Save failed: unable to read scenario fields. ' + (err.message || err)); return false; }
    const list = saved();
    if (!payload.id) { payload.id = makeId(list); const el = idEl(mode); if (el) el.value = payload.id; }
    const idx = list.findIndex(x => String(x.id||'') === String(payload.id||''));
    const existing = idx >= 0 ? list[idx] : null;
    const record = enrich(payload, existing, mode);
    if (idx >= 0) list[idx] = record; else list.unshift(record);
    write(list);
    refresh();
    const msg = 'Saved ' + (mode === 'beta' ? 'Beta' : mode === 'complex' ? 'Complex' : 'Single') + ' scenario: ' + record.id;
    const status = q('#scenarioFileStatus') || q('#workspaceSetupStatus') || q('#userAdminStatus');
    if (status) status.textContent = msg;
    alert(msg);
    return false;
  }

  window.saveScenario = function(event){ return doSave(event); };
  window.saveBetaScenario = function(event){ return doSave(event, 'beta'); };
  try { saveScenario = window.saveScenario; } catch(e) {}
  try { saveBetaScenario = window.saveBetaScenario; } catch(e) {}

  function renderDashboardRows(){
    const tbody = q('#dashboardOpenScenarioBody'); if (!tbody) return;
    const rows = saved().filter(s => String(s.scenarioStatus || 'Open').toLowerCase() !== 'closed');
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="11">No open scenarios available.</td></tr>'; return; }
    tbody.innerHTML = rows.map(s => '<tr>'+
      '<td><button class="scenario-link" data-open-id="'+esc(s.id)+'">'+esc(s.id)+'</button></td>'+
      '<td>'+esc(s.mode === 'complex' ? 'Complex Scenario' : s.mode === 'beta' ? 'Beta Scenario' : 'Single Scenario')+'</td>'+
  ...
  window.renderDashboardOpenTable = renderDashboardRows;
  try { renderDashboardOpenTable = renderDashboardRows; } catch(e) {}

  function openPortfolio(id){
    try { if (typeof activateView === 'function') activateView('reports'); } catch(e) {}
    const r = () => {
      try { if (typeof window.renderPortfolioReport2401 === 'function') window.renderPortfolioReport2401(); } catch(e) {}
      const row = qa('.rt2401-row,.rt78-row').find(tr => String(tr.dataset.rt2401Id || tr.dataset.id || '') === String(id));
      if (row) { const btn = row.querySelector('[data-rt2401-outcomes],[data-view-outcomes]'); if (btn) btn.click(); else row.click(); }
    };
    setTimeout(r, 100); setTimeout(r, 400);
  }
  window.openScenarioReport = openPortfolio;
  try { openScenarioReport = openPortfolio; } catch(e) {}

  function wire(){
    lockVersion();
    const saveBtn = q('#saveScenarioBtn'); if (saveBtn) { saveBtn.type='button'; saveBtn.onclick = e => doSave(e); }
    const betaBtn = q('#saveBetaScenarioBtn'); if (betaBtn) { betaBtn.type='button'; betaBtn.onclick = e => doSave(e, 'beta'); }
    const complexBtn = q('#addComplexScenarioBtn'); if (complexBtn) { complexBtn.type='button'; complexBtn.onclick = e => doSave(e, 'complex'); }
    const rtSaveAll = q('#rt54SaveAll'); if (rtSaveAll) { rtSaveAll.type='button'; rtSaveAll.onclick = e => doSave(e, 'complex'); }
    renderDashboardRows();
  }
  document.addEventListener('click', function(e){
    const target = e.target;
    if (target.closest && target.closest('#saveScenarioBtn')) return doSave(e);
    if (target.closest && target.closest('#saveBetaScenarioBtn')) return doSave(e, 'beta');
    if (target.closest && target.closest('#addComplexScenarioBtn,#rt54SaveAll')) return doSave(e, 'complex');
    const rpt = target.closest && target.closest('[data-report-id]');
    if (rpt) { e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation(); openPortfolio(rpt.dataset.reportId); return false; }
  }, true);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => [50,250,750,1500].forEach(t => setTimeout(wire,t)));
  else [50,250,750,1500].forEach(t => setTimeout(wire,t));
  setInterval(lockVersion, 1000);
})();
/* ===== END PHASE 24.0.08 SAVE SYSTEM HARD RESET ===== */


/* ===== PHASE 24.0.08 SAVE SYSTEM REPAIR =====
   Stabilization only. This patch does not enhance reporting or validation.
   Fix target: scenario Save buttons not persisting even when workspace folder is connected.
   Root-control approach:
   - Save is intercepted on pointerdown/click before older handlers can mis-route it.
   - Full scenario payload + calculated summary is persisted to localStorage.
   - If workspace-folder mode is connected, the same full list is written to workspaceScenarioCache
     and flushed to the selected folder.
   - Dashboard and Saved Scenarios tables are refreshed from the same list.
*/
(function(){
  const VERSION = '24.0.08';
  const SCENARIO_KEY = 'risk_manager_scenarios_v431';
  ...
  function lockVersion2408(){
    try { window.RISKTOOL_RUNTIME_VERSION = VERSION; } catch(e) {}
    ['appVersion','versionDisplay','footerVersion','phaseVersion'].forEach(id => { const el=q('#'+id); if(el) el.textContent=VERSION; });
    qa('.app-version,[data-version-label]').forEach(el => el.textContent = VERSION);
    qa('.sidebar-note h4').forEach(el => { if (/Phase\s+/i.test(el.textContent || '')) el.textContent = 'Phase ' + VERSION; });
  }
  ...
  function dedupeById(list){
    ...
    (Array.isArray(list) ? list : []).forEach(item => {
      ...
      const id = String(item.id || item.scenarioId || '').trim();
      if (!id) return;
  ...
  function readScenarioList2408(){
    let combined = [];
    try { if (Array.isArray(workspaceScenarioCache)) combined = combined.concat(workspaceScenarioCache); } catch(e) {}
    combined = combined.concat(safeJsonRead(SCENARIO_KEY), safeJsonRead(LEGACY_KEY));
    return dedupeById(combined);
  }
  async function writeScenarioList2408(list){
    const arr = dedupeById(Array.isArray(list) ? list : []);
    localStorage.setItem(SCENARIO_KEY, JSON.stringify(arr));
    localStorage.setItem(LEGACY_KEY, JSON.stringify(arr));
    try { workspaceScenarioCache = arr.slice(); } catch(e) {}
    try {
      if (typeof workspaceFolderModeEnabled === 'function' && workspaceFolderModeEnabled() && typeof workspaceFolderHandle !== 'undefined' && workspaceFolderHandle && typeof flushWorkspaceFiles === 'function') {
        await flushWorkspaceFiles();
      }
    } catch(e) { console.warn('24.0.08 workspace flush skipped:', e); }
    return arr;
  }

  window.getSavedScenarios = function(){ return readScenarioList2408(); };
  window.setSavedScenarios = function(items){ writeScenarioList2408(items); return readScenarioList2408(); };
  try { getSavedScenarios = window.getSavedScenarios; } catch(e) {}
  try { setSavedScenarios = window.setSavedScenarios; } catch(e) {}

  function nextScenarioId2408(list){
    const d = new Date();
    const stamp = d.getFullYear().toString() + String(d.getMonth()+1).padStart(2,'0') + String(d.getDate()).padStart(2,'0');
    const nums = (list||[]).map(x => String(x.id || '')).filter(x => x.startsWith(stamp + '-')).map(x => Number(x.split('-')[1])).filter(Number.isFinite);
    return stamp + '-' + String((nums.length ? Math.max(...nums) : 0) + 1).padStart(5,'0');
  }
  function activeMode2408(){
    if (q('#view-beta.active')) return 'beta';
    if (q('#view-complex.active')) return 'complex';
    if (q('#view-single.active')) return 'single';
    try { if (activeMode === 'beta' || activeMode === 'complex' || activeMode === 'single') return activeMode; } catch(e) {}
    const hash = String(location.hash || '').toLowerCase();
    if (hash.includes('beta')) return 'beta';
    if (hash.includes('complex')) return 'complex';
    return 'single';
  }
  function getPayload2408(mode){
    if (mode === 'beta' && typeof getBetaPayload === 'function') return getBetaPayload();
    if (mode === 'complex' && typeof getComplexPayload === 'function') return getComplexPayload();
    if (typeof getSinglePayload === 'function') return getSinglePayload();
    throw new Error('Unable to read scenario fields for ' + mode + '.');
  }
  function setIdField2408(mode, id){
    const el = q(mode === 'beta' ? '#betaScenarioId' : mode === 'complex' ? '#complexScenarioId' : '#singleScenarioId');
    if (el) el.value = id;
  }
  function buildFullRecord2408(payload, existing, mode){
    let owned = Object.assign({}, payload || {});
    try { if (typeof applyOwnershipMetadata === 'function') owned = applyOwnershipMetadata(owned, existing || null); } catch(e) {}
    let summary = {};
    try { if (typeof summarizePayload === 'function') summary = summarizePayload(owned) || {}; } catch(e) { console.warn('24.0.08 summarize skipped:', e); }
    const record = Object.assign({}, owned, summary);
    record.id = owned.id || summary.id;
    record.mode = owned.mode || summary.mode || mode;
    record.name = owned.name || summary.name || 'Unnamed Scenario';
    record.scenarioStatus = owned.scenarioStatus || summary.scenarioStatus || (mode === 'beta' ? 'Draft' : 'Open');
    record.scenarioSource = owned.scenarioSource || summary.scenarioSource || '';
    record.productGroup = owned.productGroup || summary.productGroup || owned.primaryProduct || owned.riskDomain || '';
    record.primaryProduct = owned.primaryProduct || summary.primaryProduct || owned.productGroup || '';
    record.riskDomain = owned.riskDomain || summary.riskDomain || owned.productGroup || '';
    record.storageMode = (function(){ try { return (typeof workspaceFolderModeEnabled === 'function' && workspaceFolderModeEnabled() && typeof workspaceFolderHandle !== 'undefined' && workspaceFolderHandle) ? 'Workspace Folder + Browser Storage' : 'Local Browser Storage'; } catch(e){ return 'Local Browser Storage'; } })();
    record.lastSavedAt = new Date().toISOString();
    ['items','productSections','insurance','hardFacts','mitigations','acceptedRiskEntries','monteCarloMethodRows','monteCarloInputRows','monteCarloOutputRows','horizonRows','randomOutcomeRows'].forEach(k => {
      if (Array.isArray(owned[k])) record[k] = owned[k].slice();
      else if (Array.isArray(summary[k])) record[k] = summary[k].slice();
      else if (!Array.isArray(record[k])) record[k] = [];
    });
    return record;
  }
  function renderDashboard2408(){
    const tbody = q('#dashboardOpenScenarioBody');
    if (!tbody) return;
    const rows = readScenarioList2408().filter(s => String(s.scenarioStatus || 'Open').toLowerCase() !== 'closed')
      .sort((a,b) => Number(b.residual || b.inherent || 0) - Number(a.residual || a.inherent || 0));
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="11">No open scenarios available.</td></tr>'; return; }
    tbody.innerHTML = rows.map(s => '<tr>'+
      '<td><button class="scenario-link" type="button" data-open-id="'+esc(s.id)+'">'+esc(s.id)+'</button></td>'+
      '<td>'+esc(s.mode === 'complex' ? 'Complex Scenario' : s.mode === 'beta' ? 'Beta Scenario' : 'Single Scenario')+'</td>'+
      '<td>'+esc(s.name || 'Unnamed')+'</td><td>'+esc(s.scenarioStatus || 'Open')+'</td><td>'+esc(s.inherent || '')+'</td>'+
      '<td>'+esc(s.identifiedDate || '')+'</td><td>'+esc(s.productGroup || s.primaryProduct || '')+'</td><td>'+esc(s.riskDomain || '')+'</td>'+
      '<td>'+esc(s.assignedOwnerName || '')+'</td><td>'+esc(s.storageMode || 'Local Browser Storage')+'</td>'+
      '<td><button class="btn btn-secondary small-btn" type="button" data-report-id="'+esc(s.id)+'">Open Report</button></td></tr>').join('');
  }
  function renderSaved2408(){
    const tbody = q('#savedEvaluationsBody');
    const rows = readScenarioList2408();
    const count = q('#savedCount'); if (count) count.textContent = String(rows.length);
    const betaCount = q('#betaCount'); if (betaCount) betaCount.textContent = String(rows.filter(x => x.mode === 'beta').length);
    if (!tbody) return;
    if (!rows.length) { tbody.innerHTML = '<tr><td colspan="11">No saved scenarios yet.</td></tr>'; return; }
    tbody.innerHTML = rows.map(s => '<tr>'+
      '<td>'+esc(s.id)+'</td><td>'+esc(s.name || 'Unnamed')+'</td><td>'+esc(s.mode || '')+'</td><td>'+esc(s.productGroup || s.primaryProduct || '')+'</td><td>'+esc(s.scenarioStatus || '')+'</td>'+
      '<td>'+esc(s.assignedOwnerName || '')+'</td><td>'+esc(s.storageMode || '')+'</td><td>'+esc(s.inherent || '')+'</td><td>'+esc(s.residual || '')+'</td><td>'+esc(s.identifiedDate || '')+'</td>'+
      '<td><button class="btn btn-secondary small-btn" type="button" data-open-id="'+esc(s.id)+'">Open</button> <button class="btn btn-secondary small-btn" type="button" data-delete-id="'+esc(s.id)+'">Delete</button></td></tr>').join('');
  }
  window.renderDashboardOpenTable = renderDashboard2408;
  window.renderSavedScenarios = renderSaved2408;
  try { renderDashboardOpenTable = renderDashboard2408; } catch(e) {}
  try { renderSavedScenarios = renderSaved2408; } catch(e) {}

  async function saveNow2408(event, forcedMode){
    if (event) { event.preventDefault(); event.stopPropagation(); if (event.stopImmediatePropagation) event.stopImmediatePropagation(); }
    if (window.__rt2408SaveLock) return false;
    window.__rt2408SaveLock = true;
    try {
      const mode = forcedMode || activeMode2408();
      const payload = getPayload2408(mode);
      const list = readScenarioList2408();
      if (!payload.id) { payload.id = nextScenarioId2408(list); setIdField2408(mode, payload.id); }
      const idx = list.findIndex(x => String(x.id || '') === String(payload.id || ''));
      const existing = idx >= 0 ? list[idx] : null;
      const record = buildFullRecord2408(payload, existing, mode);
      if (idx >= 0) list[idx] = record; else list.unshift(record);
      await writeScenarioList2408(list);
      renderSaved2408(); renderDashboard2408();
      try { if (typeof refreshLibraries === 'function') refreshLibraries(); } catch(e) {}
      try { if (typeof renderHeatMap === 'function') renderHeatMap(); } catch(e) {}
      lockVersion2408();
      const msg = 'Saved scenario ' + record.id + ' (' + (record.mode || mode) + ').';
      const status = q('#scenarioFileStatus') || q('#workspaceSetupStatus') || q('#userAdminStatus');
      if (status) status.textContent = msg;
      console.info('RiskTool 24.0.08 save complete', record, readScenarioList2408());
      alert(msg);
    } catch(err) {
      console.error('RiskTool 24.0.08 save failed', err);
      alert('Save failed: ' + (err && err.message ? err.message : err));
    } finally {
      setTimeout(() => { window.__rt2408SaveLock = false; }, 250);
    }
    return false;
  }
  window.RT2408_save = saveNow2408;
  window.saveScenario = function(event){ return saveNow2408(event); };
  window.saveBetaScenario = function(event){ return saveNow2408(event, 'beta'); };
  try { saveScenario = window.saveScenario; } catch(e) {}
  try { saveBetaScenario = window.saveBetaScenario; } catch(e) {}
  ...
  function wire2408(){
    lockVersion2408();
    const pairs = [
      ['#saveScenarioBtn', null],
      ['#saveBetaScenarioBtn', 'beta'],
      ['#addComplexScenarioBtn', 'complex'],
      ['#rt54SaveAll', 'complex']
    ];
    pairs.forEach(([sel, mode]) => {
      ...
      if (!b) return;
      b.type = 'button';
      b.setAttribute('data-rt2408-save', mode || 'active');
      b.onclick = e => saveNow2408(e, mode);
    });
    renderSaved2408(); renderDashboard2408();
  }
  function isSaveTarget2408(target){
    return target && target.closest && target.closest('#saveScenarioBtn,#saveBetaScenarioBtn,#addComplexScenarioBtn,#rt54SaveAll,[data-rt2408-save]');
  }
  ['pointerdown','click'].forEach(evtName => {
    document.addEventListener(evtName, function(e){
      const btn = isSaveTarget2408(e.target);
      if (!btn) return;
      const marker = btn.getAttribute('data-rt2408-save');
      const forced = marker === 'beta' ? 'beta' : marker === 'complex' ? 'complex' : null;
      return saveNow2408(e, forced);
    }, true);
  });
  document.addEventListener('click', function(e){
    const rpt = e.target.closest && e.target.closest('[data-report-id]');
    if (rpt) {
      e.preventDefault(); e.stopPropagation(); if (e.stopImmediatePropagation) e.stopImmediatePropagation();
      try { if (typeof activateView === 'function') activateView('reports'); } catch(_) {}
      setTimeout(() => { try { if (typeof window.renderPortfolioReport2401 === 'function') window.renderPortfolioReport2401(); } catch(_) {} }, 100);
      return false;
    }
  }, true);
  if (document.readyState === 'loading') document.addEventListener('DOMContentLoaded', () => [50,250,800,1600,3200].forEach(t => setTimeout(wire2408,t)));
  else [50,250,800,1600,3200].forEach(t => setTimeout(wire2408,t));
  setInterval(lockVersion2408, 1000);
})();
/* ===== END PHASE 24.0.08 SAVE SYSTEM REPAIR ===== */
